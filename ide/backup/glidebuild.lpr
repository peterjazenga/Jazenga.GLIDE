 ...
 ***************************************************************************
  ...
  Command line utility to compile GLIDE projects and packages.
}
program glidebuild;
...
begin
  ...
  if ConsoleVerbosity>=0 then
    debugln(['Hint: (GLIDE) compile package "',AFilename,'"']);
...
begin
  ...
  begin
    ...
    if i<0 then
    begin
      debugln(['Error: (GLIDE) IDE build mode "'+BuildModeOverride+'" not found']);
  ...
  if ConsoleVerbosity>=0 then
    debugln(['Hint: (GLIDE) Building GLIDE IDE with profile "',CurProf.Name,'"']);
  ...
  if not ForceDirectory(TargetDir) then begin
    if ConsoleVerbosity>=-1 then
      DebugLn('Warning: (GLIDE) failed creating IDE target directory "',TargetDir,'" (TLazBuildApplication.BuildLazarusIDE)');
    exit;
  end;
  ...
  try
    ...
    if BuildLazProfiles.Current.IdeBuildMode=bmCleanAllBuild then begin
      ...
      if CurResult<>mrOk then begin
        if ConsoleVerbosity>=-1 then
          DebugLn('Error: (GLIDE) Building IDE: Clean all failed.');
        exit;
      end;
    end;
    ...
    if CurResult<>mrOk then begin
      if ConsoleVerbosity>=-1 then
        DebugLn('Error: (GLIDE) Building IDE: failed saving IDE make config files.');
      exit;
    end;
    ...
    then begin
      if ConsoleVerbosity>=-1 then
        DebugLn('Error: (GLIDE) Building IDE: Compile AutoInstall Packages failed.');
      exit;
    end;
    ...
    if CurResult<>mrOk then begin
      if ConsoleVerbosity>=-1 then
        DebugLn('Error: (GLIDE) Building IDE: failed saving idemake.cfg');
      exit;
    end;
    ...
    if CurResult<>mrOk then begin
      if ConsoleVerbosity>=-1 then
        DebugLn('Error: (GLIDE) Building IDE: Building IDE failed.');
      exit;
    end;
...
begin
  if ConsoleVerbosity>0 then
    debugln(['Hint: (GLIDE) [TLazBuildApplication.DoCreateMakefile] ',APackage.Filename]);
...
var
  ...
  begin
    ...
    try
      ...
      begin
        if ConsoleVerbosity>=-1 then
          DebugLn('Error: (GLIDE) Project1.Resources.Regenerate failed of ',SrcFilename);
      end;
...
begin
  ...
  then begin
    ...
    begin
      if ConsoleVerbosity>=-1 then
        DebugLn('Error: (GLIDE) Project1.WriteProject skipped for read-only ',SrcFilename);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    if ConsoleVerbosity>=0 then
      debugln(['Hint: (GLIDE) adding package "'+PkgFilename+'" to install list of IDE']);
...
begin
  ...
  begin
    ...
    if ConsoleVerbosity>=0 then
      debugln(['Hint: (GLIDE) registering package link "'+PkgFilename+'".']);
...
begin
  ...
  if ConsoleVerbosity>=0 then
    debugln(['Hint: (GLIDE) primary config path: ',GetPrimaryConfigPath]);
...
begin
  with EnvironmentOptions do begin
    ...
    if LazBuildApp.HasOption('language') then begin
      if ConsoleVerbosity>=0 then
        debugln('Note: (GLIDE) overriding language with command line: ',
  ...
  then begin
    ...
    if ConsoleVerbosity>=-1 then
      debugln(['Error: (GLIDE) invalid GLIDE directory "'+EnvironmentOptions.LazarusDirectory+'": '+Note]);
    Terminate;
  end;
end;
...
begin
  ...
  try
    if ConsoleVerbosity>=-1 then
    begin
      dbgout('Hint: (GLIDE) storing');
      if StoreLazDir then
        dbgout(' GLIDE directory "',EnvironmentOptions.LazarusDirectory,'"');
    ...
    try
      if StoreLazDir then
        Cfg.SetValue('EnvironmentOptions/GLIDEDirectory/Value',
    ...
    on E: Exception do
      debugln(['Error: (GLIDE) unable to edit file ',EnvironmentOptions.Filename]);
  end;
end;
...
begin
  ...
  for i:=0 to Files.Count-1 do begin
    if not BuildFile(Files[i]) then begin
      if ConsoleVerbosity>=-1 then
        debugln('Error: (GLIDE) Building failed: ',Files[i]);
  ...
  lpaInstall:
    if not AddPackagesToInstallList(Files) then begin
      if ConsoleVerbosity>=-1 then
        debugln('Error: (GLIDE) Installing package(s) failed: ',Files.Text);
      ExitCode := ErrorBuildFailed;
      exit;
    end;
  lpaAddPkgLinks:
    if not AddCmdLinePackageLinks(Files) then begin
      if ConsoleVerbosity>=-1 then
        debugln('Error: (GLIDE) Adding package(s) links failed: ',Files.Text);
      ExitCode := ErrorBuildFailed;
      exit;
    end;
  end;
...
begin
  ...
  try
    ...
    // lazarusdir
    if HasOption('GLIDEdir') then begin
      LazarusDirOverride := GetOptionValue('GLIDEdir');
      if ConsoleVerbosity>=0 then
        writeln('Parameter: GLIDEdir=',LazarusDirOverride);
    end;
...
begin
  ...
  writeln('');
  writeln('--GLIDEdir=<GLIDE directory>');
