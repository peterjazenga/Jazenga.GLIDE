...
begin
  ...
  if Sender is TIDEMenuItem then begin
    ...
    DebugLn(['Hint: (GLIDE) TSourceNotebook.EncodingClicked NewEncoding=',NewEncoding]);
    if SrcEdit.CodeBuffer<>nil then begin
      ...
      if NewEncoding<>SrcEdit.CodeBuffer.DiskEncoding then begin
        DebugLn(['Hint: (GLIDE) TSourceNotebook.EncodingClicked Old=',OldEncoding,' New=',NewEncoding]);
        ...
        if CurResult=mrYes then begin
          ...
          DebugLn(['Hint: (GLIDE) TSourceNotebook.EncodingClicked Change file to ',SrcEdit.CodeBuffer.DiskEncoding]);
          if (not SrcEdit.CodeBuffer.IsVirtual)
          and (LazarusIDE.DoSaveEditorFile(SrcEdit, []) <> mrOk)
          then begin
            DebugLn(['Hint: (GLIDE) TSourceNotebook.EncodingClicked LazarusIDE.DoSaveEditorFile failed']);
          end;
