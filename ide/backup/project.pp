  ...
  begin
    ...
    if Assigned(fOnFileBackup) then begin
      if fOnFileBackup(FXMLConfig.Filename)=mrAbort then begin
        debugln(['Error: (GLIDE) [TProject.SaveToLPI] backup of "'+FXMLConfig.Filename+'" failed.']);
        exit;
      end;
    end;
    ...
    if FoundInUnits<>nil then begin
      for i:=0 to FoundInUnits.Count-1 do begin
        ...
        if ConsoleVerbosity>=0 then
          debugln(['Note: (GLIDE) [TProject.UpdateIsPartOfProjectFromMainUnit] used unit ',FoundInUnits[i],' not marked in lpi. Setting IsPartOfProject flag.']);
        if AnUnitInfo=nil then begin
          NewUnitInfo:=TUnitInfo.Create(nil);
