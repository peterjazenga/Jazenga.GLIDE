...
begin
  ...
  begin
    Debugln('Hint: (GLIDE) [TMainIDE.ParseCmdLineOptions] PrimaryConfigPath="',GetPrimaryConfigPath,'"');
    Debugln('Hint: (GLIDE) [TMainIDE.ParseCmdLineOptions] SecondaryConfigPath="',GetSecondaryConfigPath,'"');
  end;
end;
...
begin
  ...
  begin
    debugln('Hint: (GLIDE) [TMainIDE.LoadGlobalOptions] overriding language with command line: ',
  ...
  begin
    debugln('Hint: (GLIDE) [TMainIDE.LoadGlobalOptions] overriding Lazarusdir with command line: ',
  ...
  then begin
    ...
    // => warn
    debugln(['Hint: (GLIDE) [TMainIDE.LoadGlobalOptions]']);
    debugln(['Hint: (GLIDE) LastCalled="',LastCalled,'"']);
    debugln(['Hint: (GLIDE) CurPrgName="',CurPrgName,'"']);
    debugln(['Hint: (GLIDE) AltPrgName="',AltPrgName,'"']);
...
begin
  ...
  then begin
    debugln(['Warning: (GLIDE) incompatible Lazarus directory: ',EnvironmentOptions.GetParsedLazarusDirectory]);
    ShowSetupDialog:=true;
  end;
  ...
  then begin
    debugln(['Warning: (GLIDE) invalid compiler: ',EnvironmentOptions.GetParsedCompilerFilename]);
    ShowSetupDialog:=true;
  end;
  ...
  begin
    ...
    then begin
      debugln(['Warning: (GLIDE) invalid fpc source directory: ',EnvironmentOptions.GetParsedFPCSourceDirectory]);
      ShowSetupDialog:=true;
    end;
  end;
  ...
  then begin
    debugln(['Warning: (GLIDE) missing GDB exe',EnvironmentOptions.GetParsedLazarusDirectory]);
    ShowSetupDialog:=true;
  end;
  ...
  then begin
    debugln(['Warning: (GLIDE) incompatible make utility: ',EnvironmentOptions.GetParsedMakeFilename]);
    ShowSetupDialog:=true;
  end;
...
begin
  ...
  if ConsoleVerbosity>0 then
    DebugLn('Hint: (GLIDE) [TMainIDE.Destroy]');
  ...
  if ConsoleVerbosity>=0 then
    DebugLn('Hint: (GLIDE) [TMainIDE.Destroy] B  -> inherited Destroy... ',ClassName);
  ...
  if ConsoleVerbosity>=0 then
    DebugLn('Hint: (GLIDE) [TMainIDE.Destroy] END');
end;
...
procedure TMainIDE.SetupStartProject;
  ...
  begin
    debugln(['Hint: (GLIDE) AskIfLoadLastFailingProject START']);
    ...
    debugln(['Hint: (GLIDE) AskIfLoadLastFailingProject END ',dbgs(Result)]);
  end;
  ...
  if CmdLineFiles<>nil then begin
    ...
    Begin
      ...
      if not FileExistsCached(AFilename) then begin
        debugln(['Warning: (GLIDE) command line file not found: "',AFilename,'"']);
        continue;
      end;
      if Project1=nil then begin
...
begin
  ...
  while i>=0 do begin
    ...
    and ((AForm.Owner=MainIDEBar) or (AForm.Owner=Self)) then begin
      DebugLn(['Hint: (GLIDE) [TMainIDE.FreeIDEWindows] ',dbgsName(AForm)]);
      AForm.Free;
    end;
...
begin
  ...
  if POFilename='' then begin
    DebugLn(['Warning: (GLIDE) [TMainIDE.UpdateProjectPOFile] unable to get project target filename']);
    exit;
  end;
...
begin
  if ConsoleVerbosity>0 then
    DebugLn(['Hint: (GLIDE) TMainIDE.LoadDesktopSettings']);
...
begin
  if ConsoleVerbosity>0 then
    DebugLn(['Hint: (GLIDE) TMainIDE.SaveDesktopSettings']);
...
begin
  if ConsoleVerbosity>0 then
    DebugLn(['Hint: (GLIDE) TMainIDE.OnLoadIDEOptions: ', AOptions.ClassName]);
...
begin
  if ConsoleVerbosity>0 then
    DebugLn(['Hint: (GLIDE) TMainIDE.OnSaveIDEOptions: ', AOptions.ClassName]);
  // ToDo: Figure out why this is not called with TEnvironmentOptions.
  if AOptions is TEnvironmentOptions then
...
begin
  ...
  try
    ...
    begin
      if ConsoleVerbosity>0 then
        DebugLn(['Hint: (GLIDE) TMainIDE.DoOpenIDEOptions: Options saved, updating TaskBar.']);
...
var
  ...
  begin
    ...
    try
      ...
      if not CTResult then begin
        DebugLn(['Error: (GLIDE) [TMainIDE.DoFixupComponentReferences.FindUsedUnits] failed parsing ',
                 aUnitInfo.Filename]);
    ...
    for i:=0 to UnitFilenames.Count-1 do begin
      ...
      if FileExistsCached(LFMFilename) then begin
        ...
        if ModalResult<>mrOk then begin
          debugln('Error: (GLIDE) [TMainIDE.DoFixupComponentReferences] Failed loading ',LFMFilename);
  ...
  begin
    ...
    DebugLn(['Warning: (GLIDE) FindUnitFilename missing: ',aComponentName]);
    Result:='';
  end;
  ...
  begin
    ...
    if UnitFilename='' then begin
      DebugLn(['Error: (GLIDE) [TMainIDE.DoFixupComponentReferences.LoadDependencyHidden] failed to find lfm for "',RefRootName,'"']);
      exit(mrCancel);
    end;
    // ToDo: use UnitResources
    LFMFilename:=ChangeFileExt(UnitFilename,'.lfm');
    ...
    begin
      ...
      if ModalResult<>mrOk then begin
        debugln('Error: (GLIDE) [TMainIDE.DoFixupComponentReferences] Failed loading ',UnitFilename);
        exit(mrCancel);
      end;
...
begin
  ...
  if RootUnitInfo=nil then begin
    debugln(['Warning: (GLIDE) [TMainIDE.DoFixupComponentReferences] component without unitinfo: ',DbgSName(RootComponent),' CurRoot=',DbgSName(CurRoot)]);
    exit;
  end;
  ...
  try
    ...
    for i:=0 to RootComponents.Count-1 do begin
      ...
      for j:=0 to ReferenceRootNames.Count-1 do begin
        ...
        DebugLn(['Note: (GLIDE) [TMainIDE.DoFixupComponentReferences] UNRESOLVED BEFORE loading ',j,' Root=',dbgsName(CurRoot),' RefRoot=',RefRootName,' Refs="',Trim(ReferenceInstanceNames.Text),'"']);
        ...
        if LoadResult=mrRetry then begin
          ...
          DebugLn(['Error: (GLIDE) [TMainIDE.DoFixupComponentReferences] failed loading component ',RefRootName]);
          Result:=mrCancel;
        end;
      end;
    end;
    ...
    try
      ...
      on E: Exception do begin
        DebugLn(['Error: (GLIDE) [TMainIDE.DoFixupComponentReferences] GlobalFixupReferences ',E.Message]);
    ...
    for i:=0 to RootComponents.Count-1 do begin
      ...
      for j:=0 to ReferenceRootNames.Count-1 do begin
        ...
        ReferenceInstanceNames.Clear;
        GetFixupInstanceNames(CurRoot,RefRootName,ReferenceInstanceNames);
        DebugLn(['Warning: (GLIDE) [TMainIDE.DoFixupComponentReferences] UNRESOLVED AFTER loading ',j,' ',dbgsName(CurRoot),' RefRoot=',RefRootName,' Refs="',Trim(ReferenceInstanceNames.Text),'"']);
...
begin
  ...
  if CurResult=mrAbort then begin
    if ConsoleVerbosity>0 then
      debugln(['Error: (GLIDE) [TMainIDE.DoSaveAll] DoCallModalFunctionHandler(lihtSavingAll) failed']);
    exit(mrAbort);
  end;
  ...
  if CurResult<>mrOK then begin
    if ConsoleVerbosity>0 then
      debugln(['Error: (GLIDE) [TMainIDE.DoSaveAll] DoSaveProject failed']);
  end;
  SaveEnvironment(true);
  SaveIncludeLinks;
  PkgBoss.SaveSettings;
  ...
  if CurResult<>mrOK then begin
    if ConsoleVerbosity>0 then
      debugln(['Error: (GLIDE) [TMainIDE.DoSaveAll] DoCallModalFunctionHandler(lihtSavedAll) failed']);
  end;
...
begin
  ...
  if DiskFilename<>AFilename then begin
    ...
    DebugLn(['Warning: (GLIDE) [TMainIDE.DoOpenProjectFile] Fixing file name: ',AFilename,' -> ',DiskFilename]);
    AFilename:=DiskFilename;
  end;

  // check if it is a directory
  if DirPathExistsCached(AFileName) then begin
    debugln(['Error: (GLIDE) [TMainIDE.DoOpenProjectFile] file is a directory']);
...
begin
  ...
  if not (ToolStatus in [itNone,itDebugger,itBuilder]) then begin
    if ConsoleVerbosity>0 then
      DebugLn('Error: (GLIDE) [TMainIDE.DoSaveForBuild] ToolStatus forbids it: ',dbgs(ToolStatus));
    Result:=mrAbort;
    exit;
  end;
  if Project1=nil then Begin
  ...
  if Result<>mrOk then begin
    if ConsoleVerbosity>0 then
      DebugLn('Error: (GLIDE) [TMainIDE.DoSaveForBuild] project saving failed');
    exit;
  end;
  ...
  if Result<>mrOK then
    if ConsoleVerbosity>0 then
      debugln(['Error: (GLIDE) [TMainIDE.DoSaveForBuild] PkgBoss.DoSaveAllPackages failed']);
...
begin
  if DoAbortBuild(true)<>mrOK then begin
    debugln(['Error: (GLIDE) [TMainIDE.DoBuildProject] DoAbortBuild failed']);
    exit(mrCancel);
  end;

  Result:=SourceFileMgr.PrepareForCompileWithMsg;
  if Result<>mrOk then begin
    debugln(['Error: (GLIDE) [TMainIDE.DoBuildProject] PrepareForCompile failed']);
    exit;
  end;
  ...
  begin
    ...
    if Result<>mrOk then begin
      debugln(['Error: (GLIDE) [TMainIDE.DoBuildProject] CheckCompileReasons negative']);
      exit;
    end;
  end;
  ...
  try
    ...
    if Result<>mrOk then begin
      debugln(['Error: (GLIDE) [TMainIDE.DoBuildProject] DoSaveForBuild failed']);
      exit;
    end;
    ...
    if Result<>mrOk then begin
      debugln(['Error: (GLIDE) [TMainIDE.DoBuildProject] handler lihtProjectBuilding negative']);
      exit;
    end;
    ...
    try
      ...
      if not (pbfDoNotCompileDependencies in Flags) then begin
        ...
        if Result<>mrOk then begin
          debugln(['Error: (GLIDE) [TMainIDE.DoBuildProject] handler lihtProjectDependenciesCompiling negative']);
          exit;
        end;
        ...
        if Result <> mrOk then
        begin
          debugln(['Error: (GLIDE) [TMainIDE.DoBuildProject] PkgBoss.DoCompileProjectDependencies failed']);
          exit;
        end;
        Result:=DoCallModalFunctionHandler(lihtProjectDependenciesCompiled);
        if Result<>mrOk then begin
          debugln(['Error: (GLIDE) [TMainIDE.DoBuildProject] handler lihtProjectDependenciesCompiled negative']);
          exit;
        end;
      end;
      ...
      if Result<>mrOk then
      begin
        debugln(['Error: (GLIDE) [TMainIDE.DoBuildProject] DoWarnAmbiguousFiles negative']);
        exit;
      end;
      ...
      if (AReason in Project1.CompilerOptions.CompileReasons) then begin
        ...
        and (not (pfAlwaysBuild in Project1.Flags)) then begin
          if Result=mrNo then begin
            debugln(['Error: (GLIDE) [TMainIDE.DoBuildProject] MainBuildBoss.DoCheckIfProjectNeedsCompilation nothing to be done']);
            Result:=mrOk;
          ...
          begin
            debugln(['Error: (GLIDE) [TMainIDE.DoBuildProject] MainBuildBoss.DoCheckIfProjectNeedsCompilation failed']);
            exit;
          end;
        end;
      end;
      ...
      if not (pbfSkipTools in Flags) then begin
        ...
        if (AReason in ToolBefore.CompileReasons) then begin
          ...
          if Result<>mrOk then
          begin
            debugln(['Error: (GLIDE) [TMainIDE.DoBuildProject] CompilerOptions.ExecuteBefore.Execute failed']);
            exit;
          end;
        end;
      end;
      ...
      and (not DirPathExistsCached(UnitOutputDirectory)) then begin
        ...
        if Result<>mrOk then begin
          debugln(['Error: (GLIDE) [TMainIDE.DoBuildProject] ForceDirectoryInteractive "',UnitOutputDirectory,'" failed']);
          exit;
        end;
      end;
      ...
      and (not DirPathExistsCached(TargetExeDirectory)) then begin
        ...
        if Result<>mrOk then begin
          debugln(['Error: (GLIDE) [TMainIDE.DoBuildProject] ForceDirectoryInteractive "',TargetExeDirectory,'" failed']);
          exit;
        end;
      end;
      ...
      then begin
        ...
        if not (Result in [mrOk,mrIgnore]) then begin
          debugln(['Error: (GLIDE) [TMainIDE.DoBuildProject] CreateApplicationBundle "',TargetExeName,'" failed']);
          exit;
        end;
        Result:=CreateAppBundleSymbolicLink(TargetExeName);
        if not (Result in [mrOk,mrIgnore]) then begin
          debugln(['Error: (GLIDE) [TMainIDE.DoBuildProject] CreateAppBundleSymbolicLink "',TargetExeName,'" failed']);
          exit;
        end;
      end;
      ...
      then begin
        debugln(['Error: (GLIDE) [TMainIDE.DoBuildProject] ProjResources.Regenerate failed']);
        exit;
      end;
      ...
      and (not (pbfDoNotCompileProject in Flags)) then begin
        ...
        if Result<>mrOk then begin
          debugln(['Error: (GLIDE) [TMainIDE.DoBuildProject] SaveStateFile before compile failed']);
          exit;
        end;
        ...
        Result:=TheCompiler.Compile(Project1,
                                ...
                                aCompileHint);
        if ConsoleVerbosity>=0 then
          debugln(['Hint: (GLIDE) [TMainIDE.DoBuildProject] compiler time in s: ',(Now-StartTime)*86400]);
        ...
        if Result<>mrOk then begin
          ...
          debugln(['Error: (GLIDE) [TMainIDE.DoBuildProject] Compile failed']);
          exit;
        end;
        ...
        if Result<>mrOk then begin
          debugln(['Error: (GLIDE) [TMainIDE.DoBuildProject] SaveStateFile after compile failed']);
          exit;
        end;
        ...
        if Result<>mrOk then begin
          debugln(['Error: (GLIDE) [TMainIDE.DoBuildProject] UpdateProjectPOFile failed']);
          exit;
        end;
      end;
      ...
      if not (pbfSkipTools in Flags) then begin
        ...
        if (AReason in ToolAfter.CompileReasons) then begin
          ...
          if Result<>mrOk then
          begin
            debugln(['Error: (GLIDE) [TMainIDE.DoBuildProject] CompilerOptions.ExecuteAfter.Execute failed']);
            exit;
          end;
        end;
      end;
...
begin
  ...
  then begin
    debugln(['Error: (GLIDE) [TMainIDE.DoInitProjectRun] Project can not run:',
  ...
  // Build project first
  if ConsoleVerbosity>0 then
    debugln('Hint: (GLIDE) TMainIDE.DoInitProjectRun Check build ...');
  ...
  if ConsoleVerbosity>0 then
    DebugLn(['Hint: (GLIDE) TMainIDE.DoInitProjectRun ProgramFilename=',ProgramFilename]);
...
begin
  DebugLn('Hint: (GLIDE) [TMainIDE.DoRunProject] INIT');
  ...
  debugln('Hint: (GLIDE) [TMainIDE.DoRunProject] Debugger=',EnvironmentOptions.DebuggerConfig.DebuggerClass);

  Result := mrCancel;

  Result := DebugBoss.StartDebugging;

  DebugLn('Hint: (GLIDE) [TMainIDE.DoRunProject] END');
end;
...
{$endif}
  ...
  begin
    ...
    try
      ...
      DebugLn('Hint: (GLIDE) CmdLine=[',StartLazProcess.Executable,' ',MergeCmdLineParams(StartLazProcess.Parameters),']');
...
begin
  DebugLn(['Hint: (GLIDE) TMainIDE.DoRestart ']);
  CanClose:=true;
...
procedure TMainIDE.DoExecuteRemoteControl;
  ...
  begin
    ...
    DebugLn(['Hint: (GLIDE) TMainIDE.DoExecuteRemoteControl.OpenFiles ProjectLoaded=',ProjectLoaded]);
    ...
    for i:=Files.Count-1 downto 0 do begin
      ...
      if (CompareFileExt(AProjectFilename,'.lpi',false)=0) then begin
        ...
        AProjectFilename:=CleanAndExpandFilename(AProjectFilename);
        if FileExistsUTF8(AProjectFilename) then begin
          DebugLn(['Hint: (GLIDE) TMainIDE.DoExecuteRemoteControl.OpenFiles AProjectFilename="',AProjectFilename,'"']);
          if (Project1<>nil)
    ...
    if Files<>nil then begin
      for i:=0 to Files.Count-1 do begin
        ...
        DebugLn(['Hint: (GLIDE) TMainIDE.DoExecuteRemoteControl.OpenFiles AFilename="',AFilename,'"']);
        if CompareFileExt(AFilename,'.lpk',false)=0 then begin
  ...
  then begin
    ...
    try
      ...
      try
        ...
        DebugLn(['Error: (GLIDE) TMainIDE.DoExecuteRemoteControl reading file failed: ',Filename]);
      end;
      DeleteFileUTF8(Filename);
...
begin
  ...
  if Result<>mrOk then begin
    DebugLn('Error: (GLIDE) TMainIDE.DoBuildLazarus: failed because saving failed');
    exit;
  end;

  Result:=DoCallModalFunctionHandler(lihtLazarusBuilding);
  if Result<>mrOk then begin
    debugln(['Error: (GLIDE) TMainIDE.DoBuildLazarusSub handler lihtLazarusBuilding negative']);
    exit;
  end;
  ...
  try
    ...
    and (BuildLazProfiles.Current.IdeBuildMode<>bmBuild) then begin
      ...
      if BuildLazProfiles.Current.IdeBuildMode=bmCleanAllBuild then begin
        ...
        if Result<>mrOk then begin
          DebugLn('Error: (GLIDE) TMainIDE.DoBuildLazarus: Clean all failed.');
          exit;
        end;
      end;
    end;
    ...
    if Result<>mrOk then begin
      DebugLn('Error: (GLIDE) TMainIDE.DoBuildLazarusSub: Compile AutoInstall Packages failed.');
      exit;
    end;
    ...
    Result:=PkgBoss.DoSaveAutoInstallConfig;
    if Result<>mrOk then begin
      DebugLn('Error: (GLIDE) TMainIDE.DoBuildLazarus: Save AutoInstall Config failed.');
      exit;
    end;
    ...
    Result:=MainBuildBoss.CheckUnitPathForAmbiguousPascalFiles(
                     ...
                     CompiledUnitExt,'IDE');
    if Result<>mrOk then begin
      DebugLn('Error: (GLIDE) TMainIDE.DoBuildLazarus: Check UnitPath for ambiguous pascal files failed.');
      exit;
    end;
    ...
    if Result<>mrOk then begin
      DebugLn('Error: (GLIDE) TMainIDE.DoBuildLazarus: Save IDEMake options failed.');
      exit;
    end;
  ...
  try
    ...
    try
      ...
      if BuildFileDialog.ShowModal<>mrOk then begin
        DebugLn(['Error: (GLIDE) TMainIDE.DoConfigBuildFile cancelled']);
        Result:=mrCancel;
        exit;
      end;
...
begin
  ...
  begin
    ...
    DebugLn(['Error: (GLIDE) TMainIDE.OnPropHookShowMethod failed finding the method in code']);
    DoJumpToCodeToolBossError;
...
begin
  if ConsoleVerbosity>0 then
    DebugLn('Hint: (GLIDE) TMainIDE.DesignerPasteComponent A');
  ...
  try
    ...
    if NewComponents.Count=0 then begin
      DebugLn('Error: (GLIDE) TMainIDE.DesignerPasteComponent FAILED FormEditor1.CreateChildComponentFromStream');
      exit;
    end;
...
begin
  ...
  if SysUtils.CompareText(Param,'SrcPath')=0 then
    ...
    debugln('Warning: (GLIDE) TMainIDE.MacroFunctionProject: ',FuncData^.Result);
  end;
end;
...
begin
  ...
  if NewSource=nil then begin
    DebugLn(['Error: (GLIDE) TMainIDE.DoJumpToCodePosition missing NewSource']);
    DumpStack;
    exit;
  end;
...
begin
  ...
  begin
    SourceFileMgr.UpdateSourceNames;
    if ConsoleVerbosity>0 then
      debugln('Note: (GLIDE) TMainIDE.DoJumpToCodeToolBossError No errormessage');
    exit;
  end;
...
begin
  ...
  try
    ...
    if Result<>mrOk then begin
      debugln('Error: (GLIDE) TMainIDE.DoFindRenameIdentifier failed: user cancelled dialog');
      exit;
    end;
    ...
    if Result<>mrOk then begin
      debugln('Error: (GLIDE) TMainIDE.DoFindRenameIdentifier unable to add user defined extra files');
      exit;
    end;
    ...
    if CodeToolBoss.ErrorMessage<>'' then
      DoJumpToCodeToolBossError;
    if Result<>mrOk then begin
      debugln('Error: (GLIDE) TMainIDE.DoFindRenameIdentifier GatherIdentifierReferences failed');
      exit;
    end;
    ...
    if Result<>mrOk then begin
      debugln('Error: (GLIDE) TMainIDE.DoFindRenameIdentifier GatherFPDocReferences failed');
      exit;
    end;
    {$ENDIF}
    ...
    if Options.Rename then begin
      ...
      try
        ...
        then begin
          DoJumpToCodeToolBossError;
          debugln('Error: (GLIDE) TMainIDE.DoFindRenameIdentifier unable to commit');
          Result:=mrCancel;
          exit;
        end;
      finally
    ...
    while UsedByDependency<>nil do begin
      ...
      then begin
        ...
        if InheritedComponent<>nil then begin
          ...
          try
            if ConsoleVerbosity>0 then
              DebugLn(['Hint: (GLIDE) RenameInheritedComponents ',dbgsName(InheritedComponent),' Owner=',dbgsName(InheritedComponent.Owner)]);
  ...
  begin
    ...
    try
      ...
      for i:=0 to PropCount-1 do begin
        ...
        // auto rename it
        if ConsoleVerbosity>0 then
          DebugLn(['Hint: (GLIDE) RenameMethods OldMethodName="',DefaultName,'" NewMethodName="',NewMethodName,'"']);
        ...
        if CTResult then begin
          ...
          DebugLn(['Error: (GLIDE) TMainIDE.DesignerRenameComponent.RenameMethods failed OldMethodName="',CurMethodName,'" NewMethodName="',NewMethodName,'" Error=',CodeToolBoss.ErrorMessage]);
        end;
      end;
      ApplyCodeToolChanges;
    finally
...
begin
  DebugLn('Hint: (GLIDE) TMainIDE.DesignerRenameComponent Old=',AComponent.Name,':',AComponent.ClassName,' New=',NewName,' Owner=',dbgsName(AComponent.Owner));
...
begin
  ADesigner:=TDesigner(Sender);
  GetDesignerUnit(ADesigner,ASrcEdit,AnUnitInfo);
  debugln('Hint: (GLIDE) TMainIDE.DesignerSaveAsXML ',AnUnitInfo.Filename);
...
begin
  ...
  try
    ...
    if r then begin
      ...
      DebugLn(['Error: (GLIDE) TMainIDE.OnPropHookCreateMethod failed adding method "'+ShortMethodName+'" to source']);
      DoJumpToCodeToolBossError;
...
begin
  if ConsoleVerbosity>0 then
    DebugLn('Hint: (GLIDE) TMainIDE.OnPropHookPersistentAdded A ',dbgsName(APersistent));
  ADesigner:=nil;
  if APersistent is TComponent then
  ...
  if (RegComp=nil) and (AComponent<>nil) then begin
    ...
    if ClassUnitInfo=nil then begin
      DebugLn('Error: (GLIDE) TMainIDE.OnPropHookPersistentAdded ',APersistent.ClassName,
              ' not registered');
      exit;
    end;
  end;
...
begin
  ...
  DebugLn('Hint: (GLIDE) TMainIDE.OnPropHookDeletePersistent A ',dbgsName(APersistent));
  if APersistent is TComponent then begin
    AComponent:=TComponent(APersistent);
...
begin
  ...
  if AnUnitInfo=nil then begin
    DebugLn(['Error: (GLIDE) TMainIDE.OnPropHookObjectPropertyChanged LookupRoot not found']);
    exit;
  end;
  ...
  if (NewObject is TComponent) then begin
    ...
    if ReferenceUnitInfo=nil then begin
      DebugLn(['Error: (GLIDE) TMainIDE.OnPropHookObjectPropertyChanged reference LookupRoot not found']);
      exit;
    end;
...
begin
  ...
  AnUnitInfo:=Project1.UnitWithComponent(TComponent(GlobalDesignHook.LookupRoot));
  if AnUnitInfo=nil then begin
    DebugLn(['Error: (GLIDE) TMainIDE.OnPropHookAddDependency LookupRoot not found']);
    exit;
  end;
...
begin
  ...
  debugln(['Hint: (GLIDE) TMainIDE.mnuEditSortBlockClicked ',DbgSName(FindOwnerControl(GetFocus))]);
