      ...
      begin
        ...
        if (Ext='.ppu') or (Ext='.o') or (Ext='.rst') or (Ext='.rsj') then begin
          if not DeleteFileUTF8(Filename) then
            debugln(['Error : (GLIDE) Clean directory: failed to delete file "',Filename,'"']);
        end;
      end;
...
begin
  ...
  if not FileExistsUTF8(RevisionIncFile) then begin
    debugln(['Note: (GLIDE) revision.inc file missing: ',RevisionIncFile]);
    ...
    try
      ...
      on E: Exception do begin
        debugln(['Warning: (GLIDE) unable to write ',RevisionIncFile,': ',E.Message]);
      end;
    end;
    sl.Free;
  end;
...
begin
  if FileExistsUTF8(fTargetFilename) then begin
    if not DeleteFileUTF8(fTargetFilename) then begin
      debugln(['Error: (GLIDE) Building IDE failed. Unable to delete "',fTargetFilename,'"']);
      exit;
    end;
  end;
  ...
  if FileExistsUTF8(BackupFilename) then begin
    if not RenameFileUTF8(BackupFilename,fTargetFilename) then begin
      debugln(['Error: (GLIDE) Building IDE failed. Unable to restore backup file "',BackupFilename,'" to "',fTargetFilename,'"']);
    end;
  end;
end;
...
begin
  ...
  if (fProfile.TargetDirectory<>'') then begin
    ...
    if fTargetDir='' then begin
      debugln('Error: (GLIDE) [CalcTargets] error resolving macros in TargetDirectory=',fProfile.TargetDirectory);
      Exit(mrAbort);
    end;
    fUnitOutDir:=AppendPathDelim(fTargetDir)+'units';
    debugln('Hint: (GLIDE) [CalcTargets] TargetDirectory=',fTargetDir);
    debugln('Hint: (GLIDE) [CalcTargets] UnitsTargetDirectory=',fUnitOutDir);
  end else begin
    ...
    begin
      ...
      debugln('Hint: (GLIDE) [CalcTargets] Cross Compiling TargetOS=',fProfile.FPCTargetOS,' TargetCPU=',
      ...
      begin
        if not DirectoryIsWritableCached(fTargetDir) then begin
          ...
          fUpdateRevInc:=false;
          fTargetDir:=AppendPathDelim(GetPrimaryConfigPath)+'bin';
          debugln('Hint: (GLIDE) [CalcTargets] Lazarus directory is readonly, using fallback target directory: ',fTargetDir);
          fUnitOutDir:=AppendPathDelim(GetPrimaryConfigPath)+'units'
                  +PathDelim+fTargetCPU+'-'+fTargetOS+PathDelim+TargetLCLPlatform;
        end else begin
  ...
  if blfBackupOldExe in Flags then begin
    ...
    if FileExistsUTF8(AltFilename) then begin
      if DeleteFileUTF8(AltFilename) then
        debugln(['Note: (GLIDE) deleted file "',AltFilename,'"'])
      else
        debugln(['Warning: (GLIDE) unable to delete file "',AltFilename,'"']);
    end;
    ...
    if FileExistsUTF8(BackupFilename) then begin
      if DeleteFileUTF8(BackupFilename) then begin
        debugln(['Note: (GLIDE) deleted backup "',BackupFilename,'"']);
      end else begin
        ...
        if FileExistsUTF8(Backup2Filename) then begin
          if DeleteFileUTF8(Backup2Filename) then
            debugln(['Note: (GLIDE) deleted backup "',Backup2Filename,'"'])
          else
            debugln(['Warning: (GLIDE) unable to delete old backup file "'+Backup2Filename+'"']);
        end;
        if not FileExistsUTF8(Backup2Filename) then begin
          if RenameFileUTF8(BackupFilename,Backup2Filename) then
            debugln(['Note: (GLIDE) renamed old backup file "'+BackupFilename+'" to "',Backup2Filename,'"'])
          else
            debugln(['Warning: (GLIDE) unable to rename old backup file "'+BackupFilename+'" to "',Backup2Filename,'"']);
        end;
      end;
    end;
    if not FileExistsUTF8(BackupFilename) then begin
      if RenameFileUTF8(fTargetFilename,BackupFilename) then
        debugln(['Note: (GLIDE) renamed file "'+fTargetFilename+'" to "',BackupFilename,'"'])
      else
        debugln(['Warning: (GLIDE) unable to rename file "'+fTargetFilename+'" to "',BackupFilename,'"']);
    end;
...
begin
  ...
  if not FileExistsCached(BundleDir) then begin
    ...
    if not (Result in [mrOk,mrIgnore]) then begin
      debugln(['Error: (GLIDE) unable to create application bundle']);
    ...
    if not (Result in [mrOk,mrIgnore]) then begin
      debugln(['Error: (GLIDE) unable to create symlink in application bundle: ',fTargetFilename]);
      if IDEMessagesWindow<>nil then
