...
type
  ...
  public
    ...
    procedure SetupOnProgress(const EventSink: IProjectGroupEvents; const EventHandler: TIProjectGroupEventsOnProgress);
    // Call event handlers:
    procedure OnError(const ErrorCode: TErrorCode; const ErrorText: AnsiString);
    procedure OnProgress(const EventCode: TProgressEvent; const ProgressValue: LongInt; const EventText: AnsiString);
  end;
...
function ProjectIProjectLoadFromFile(const ItemHandle: IProject; const Path: PAnsiChar; out Result1: TBool): Boolean; cdecl;
function ProjectIProjectSaveToFile(const ItemHandle: IProject; const Path: PAnsiChar; out Result1: TBool): Boolean; cdecl;
// Properties:
...
begin
  ...
  try
    if TObject(ItemHandle) is TProject then
    begin
      Result1 := TProject(ItemHandle).LoadFromFile(Path);
      Result := True;
    end;
  except
  end;
...
begin
  ...
  try
    if TObject(ItemHandle) is TProject then
    begin
      Result1 := TProject(ItemHandle).SaveToFile(Path);
      Result := True;
    end;
  except
  end;
...
var
  String__ProjectIProjectConnectionStringValue: AnsiString;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        Length__Value := Length(String__ProjectIProjectConnectionStringValue);
      end else
      begin
        System.Move(String__ProjectIProjectConnectionStringValue[1], Value^, Min(Length__Value, Length(String__ProjectIProjectConnectionStringValue)));
      end;
      Result := True;
    end;
  except
...
procedure TIProjectGroupEvents.OnError(const ErrorCode: TErrorCode; const ErrorText: AnsiString);
begin
  if Assigned(FOnErrorEventHandler) then
  begin
    FOnErrorEventHandler(FOnErrorEventSink, ErrorCode, PAnsiChar(ErrorText));
  end;
end;

procedure TIProjectGroupEvents.OnProgress(const EventCode: TProgressEvent; const ProgressValue: LongInt; const EventText: AnsiString);
begin
  if Assigned(FOnProgressEventHandler) then
  begin
    FOnProgressEventHandler(FOnProgressEventSink, EventCode, ProgressValue, PAnsiChar(EventText));
  end;
end;
