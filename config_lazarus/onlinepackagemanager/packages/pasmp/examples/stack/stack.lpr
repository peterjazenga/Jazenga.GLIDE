...
var IntegerStack:TPasMPBoundedStack;
...
begin
 ...
 while not Terminated do begin
  while not (Terminated or IntegerStack.Push(Value)) do begin
   TPasMP.Yield;
  end;
  inc(Value);
 end;
...
begin
 while not Terminated do begin
  if IntegerStack.Pop(Value) then begin
   write(#13,Value);
  end;
 end;
end;
...
begin
 ...
 IntegerStack:=TPasMPBoundedStack.Create(65536,SizeOf(longint));
 try
