...
procedure TcyNavPanel.WndProc(var Message: TLMessage);     
var
  aControl: TControl;
  ScreenPos: TPoint;
...
begin
  ...
  if Message.Msg = WM_LBUTTONUP then // Only works for TGraphicControls ...    
  begin
    AllowDisabled := true;
    aControl := Self.ControlAtPos(SmallPointToPoint(TLMMouse(Message).Pos), AllowDisabled, true);   
