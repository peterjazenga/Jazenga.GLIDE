...
type
  ...
  TDirectInput8Create = function(hinst: THandle; dwVersion: Cardinal; const riidltf: TGUID; out ppvOut; punkOuter: IUnknown): HResult; stdcall;
  TDirectInputCreateA = function(hinst: THandle; dwVersion: DWORD; out ppDI: IDirectInputA; punkOuter: IUnknown): HResult; stdcall;
  TDirectInputCreateW = function(hinst: THandle; dwVersion: DWORD; out ppDI: IDirectInputW; punkOuter: IUnknown): HResult; stdcall;
  TDirectInputCreate = function(hinst: THandle; dwVersion: DWORD; out ppDI: IDirectInput; punkOuter: IUnknown): HResult; stdcall;
  TDirectInputCreateEx = function(hinst : THandle; dwVersion: DWORD; const riidltf: TGUID; out ppvOut; punkOuter: IUnknown): HResult; stdcall;

var
  DirectInput8Create: TDirectInput8Create;
  DirectInputCreateA: TDirectInputCreateA;
  DirectInputCreateW: TDirectInputCreateW;
  DirectInputCreate: TDirectInputCreate;
  DirectInputCreateEx: TDirectInputCreateEx;
...
type
  TjoyConfigChanged = function(dwFlags: DWORD): MMRESULT; stdcall;
var
  joyConfigChanged: TjoyConfigChanged;
...
begin
  ...
  begin
    ...
    if (DirectInputLib<>0) then
    begin
      DirectInputCreateA:= TDirectInputCreateA(GetProcAddress(DirectInputLib, 'DirectInputCreateA'));
      DirectInputCreateW:= TDirectInputCreateW(GetProcAddress(DirectInputLib, 'DirectInputCreateW'));
      {$IFDEF UNICODE}
      DirectInputCreate:= TDirectInputCreate(GetProcAddress(DirectInputLib, 'DirectInputCreateW'));
      {$ELSE}
      DirectInputCreate:= TDirectInputCreate(GetProcAddress(DirectInputLib, 'DirectInputCreateA'));
      {$ENDIF}

      DirectInputCreateEx:= TDirectInputCreateEx(GetProcAddress(DirectInputLib, 'DirectInputCreateEx'));
    end;

    DirectInput8Lib:= LoadLibrary(DirectInput8Dll);
    if (DirectInput8Lib<>0) then
    begin
      DirectInput8Create:= TDirectInput8Create(GetProcAddress(DirectInput8Lib, 'DirectInput8Create'));
    end;

    WinMMLib:= LoadLibrary(WinMMDll);
    if (WinMMLib <> 0) then
    begin
      joyConfigChanged:= TjoyConfigChanged(GetProcAddress(WinMMLib, 'joyConfigChanged'));
    end;

    JoyCPLLib:= LoadLibrary(JoyCPL);
    if (JoyCPLLib <> 0) then
    begin
      ShowJoyCPL:= TShowJoyCPL(GetProcAddress(JoyCPLLib, 'ShowJoyCPL'));
    end;
