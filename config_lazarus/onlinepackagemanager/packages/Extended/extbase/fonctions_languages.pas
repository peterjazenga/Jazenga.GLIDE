...
{$IFDEF VERSIONS}
  gver_fonctions_languages : T_Version = ( Component : 'Languages Management' ; FileUnit : 'fonctions_languages' ;
              			                 ...
              			                 BugsStory : 'Version 1.0.0.0 : Universal properties.' + #13#10
                                                           + 'Version 0.9.9.0 : Centralising getting Properties.' + #13#10
...
function fb_LoadProperties ( var SomeProperties : TStringListUTF8 ; const as_FilePath : String ; const ab_ErrorMessage : Boolean = False ):Boolean; overload;
function fb_LoadProperties ( var SomeProperties : TStringListUTF8; const as_DirPath, as_BeginFile, as_Lang : String ; const ab_ErrorMessage : Boolean = False ):Boolean; overload;
...
{$ENDIF}
  fonctions_string,
  {$IFDEF FPC}
  unite_messages, Translations, LazFileUtils,
  {$ELSE}
...
function fb_LoadProperties ( var SomeProperties : TStringListUTF8 ; const as_FilePath : String ; const ab_ErrorMessage : Boolean = False ):Boolean;
Begin
  FreeAndNil(SomeProperties);
  SomeProperties := TStringlistUTF8.Create ;
  if FileExistsUTF8 ( as_FilePath ) then
    try
      SomeProperties.LoadFromFile ( as_FilePath );
      Result := True;

    Except
      MyShowMessage('Cannot open property file :'+#13#10+as_FilePath);
    end
   Else
    Begin
     Result := False;
     if ab_ErrorMessage Then
       MyShowMessage(StringReplace ( GS_FICHIER_NON_TROUVE, CST_ARG, as_FilePath, [rfReplaceAll] ));
    end;

end;

function fb_LoadProperties ( var SomeProperties : TStringListUTF8; const as_DirPath, as_BeginFile, as_Lang : String ; const ab_ErrorMessage : Boolean = False ):Boolean;
Begin
  Result := fb_LoadProperties (SomeProperties,as_DirPath+as_BeginFile+GS_INNER_LANG_SEPARATOR+as_Lang+GS_EXT_LANGUAGES, ab_ErrorMessage);
End;
