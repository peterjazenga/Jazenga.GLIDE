...
procedure p_setLabelColorEnter ( const FLabel : TCustomLabel; const FLabelColor : TColor ; const FAlwaysSame : boolean  );
procedure p_setCompColorEnter ( const Fcomponent : TControl; const FFocusColor : TColor ; const FAlwaysSame : boolean  );
procedure p_setLabelColorExit  ( const FLabel : TCustomLabel; const FAlwaysSame : boolean  );
...
uses fonctions_proprietes, Forms;
...
begin
  ...
  and Assigned(APopUpMenu) Then
    Begin
     ...
     if Result Then
       Begin
        lp_pos:=AControl.ClientToScreen( lp_pos );
        lp_pos.y := lp_pos.y + AControl.Height;
