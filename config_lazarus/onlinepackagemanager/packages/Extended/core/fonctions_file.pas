...
{$IFDEF VERSIONS}
  gVer_fonctions_file : T_Version = ( Component : 'Gestion des fichiers' ; FileUnit : 'fonctions_erreurs' ;
           ...
           BugsStory :
           'Version 1.0.7.0 : Renaming fb_eraseDir to DeleteDirUTF8'+#13#10+
           'Version 1.0.6.0 : Creating fs_getBackupFileName'  +#13#10 +
           'Version 1.0.5.0 : Adding FileWriteln and FileCreateUTF8File.' +#13#10+
           'Version 1.0.4.0 : Adding FileReadln and FileCreateDeleteUTF8.'  +#13#10+
           'Version 1.0.3.0 : Adding ExtractDirName and ExtractSubDir.' +#13#10 +
           'Version 1.0.2.0 : UTF8 not tested.' +#13#10+
           'Version 1.0.1.0 : adding Windows drive verifying function.' +#13#10+
...
function  fb_FindFiles( const astl_FilesList: TStrings; as_StartDir : String;
                        ...
                        const as_FileMask: String=CST_FILTER_ALL; const as_DirMask: String=CST_FILTER_ALL):Boolean;
// const PreserveTime: To Do
Function CopyFileUTF8 ( const as_Source, as_Destination : String ; const PreserveTime: boolean=False; const ab_AppendFile : Boolean=False; const ab_CreateBackup : Boolean = False ):Integer;
    ...
    while Count>0 do
     Begin
       ...
       if DirectoryExistsUTF8(as_Dir+ls_file)
        ...
        Else
          inc ( ai64_size, FileSizeUtf8(as_Dir+ls_file));
     end;
  finally
    lstl_Files.Destroy;
  end;
...
begin
  ...
  { Build a list of the files in directory as_StartDir
     (not the directories!)                         }
  try
    IsFound := FindFirstUTF8(as_StartDir + CST_FILTER_ALL, faDirectory, SR) = 0 ;
    while IsFound do
     begin
...
begin
  ...
  { Build a list of the files in directory as_StartDir
     (not the directories!)                         }
  try
    IsFound := FindFirstUTF8(as_StartDir + CST_FILTER_ALL, faAnyFile-faDirectory, SR) = 0 ;
    while IsFound do
     begin
      if (( SR.Name <> '.' ) and ( SR.Name <> '..' ))
...
{$IFDEF WINDOWS}
function fs_verifyAndReplaceDriveLetter ( const as_path : String ):String;
var qwspace : {$IFDEF FPC}{$IFDEF CPU64}Pint64{$ELSE}PLongInt{$ENDIF}{$ELSE}int64{$ENDIF};
    qwspacefree : {$IFDEF FPC}{$IFDEF CPU64}Pint64{$ELSE}PLongInt{$ENDIF}{$ELSE}int64{$ENDIF};
...
Begin
  ...
  while (Windows.GetDriveType(@Result[1]) <> 1) and
    GetDiskFreeSpaceEx(@Result[1],qwspacefree,qwspace, nil )
    and ( qwspace <= 0 ) do
  {$ENDIF}
   Begin
...
Begin
  ...
  try
    try
      ...
      if as_message > '' then
        begin
          MyShowMessage(as_message + CST_ENDOFLINE + E.Message);
          Abort;
        end;
    end;
  finally
...
Begin
  ...
  try
    try
      ...
      if as_message > '' then
        begin
          MyShowMessage(as_message + as_filePath + CST_ENDOFLINE + E.Message);
          Abort;
        end;
    end;
  finally
