...
{ TFWDBEdit }
   ...
   TFWDBDateTimePicker = class ( {$IFDEF FPC}TFWDateTimePicker{$ELSE}TJvDBDateTimePicker{$ENDIF}, IFWComponent, IFWComponentEdit )
      ...
      protected
         procedure MouseDown( Button : TMouseButton; Shift : TShiftState; X,Y : Integer); override;
         procedure UpdateDate(const CallChangeFromSetDateTime: Boolean = False); override;
...
begin
  with FDataLink do
  if DateTime <> FDataLink.Field.asInteger Then
    Begin
      Edit ;
...
procedure TFWDBDateTimePicker.UpdateDate(const CallChangeFromSetDateTime: Boolean = False);
begin
 Inherited;
...
begin
  if assigned ( FDataLink.Dataset ) Then
    FDataLink.DataSet.BeforeDelete:=FBeforeDelete;
  inherited Destroy;
  FDataLink.Free;
  FDataLink := nil;
end;
...
begin
  if not (FDataLink.DataSourceFixed and (csLoading in ComponentState)) then
   Begin
     ...
     if assigned ( FDataLink.DataSet ) Then
      Begin
        FBeforeDelete:=FDataLink.DataSet.BeforeDelete;
        FDataLink.DataSet.BeforeDelete:=BeforeDelete;
      end;
   end;
  if AValue <> nil then AValue.FreeNotification(Self);
end;
...
function TFWDBFileEdit.GetText: TCaption;
begin
  Result:=Inherited Text;
end;

procedure TFWDBFileEdit.SetText(const AValue: TCaption);
begin
  if Avalue <> Text Then
    Begin
      FileName:=AValue;
      Inherited Text := Avalue;
      if assigned ( Field.DataSet ) Then
        Begin
          Field.DataSet.Edit;
          Field.Value:=AValue;
        end;
...
procedure TFWDBFileEdit.BeforeDelete(ADataset: TDataset);
begin
