...
const
  gVer_fonctions_erreurs : T_Version = ( Component : 'Gestion des erreurs' ; FileUnit : 'fonctions_erreurs' ;
                        			                 ...
                        			                 BugsStory :'Version 1.1.4.0 : Restructure.' + #13#10 +
                        			                	    'Version 1.1.3.1 : Mise à jour sur la déconnexion OLE.' + #13#10 +
                        			                 ...
                        			                 UnitType : 1 ;
                        			                 Major : 1 ; Minor : 1 ; Release : 4 ; Build : 0 );
{$ENDIF}
function f_GereException ( const aexc_exception : Exception  ; const adat_Dataset : TComponent ) : TClass; overload ;
...
function f_GereException ( const aexc_exception : Exception  ; const adat_Dataset : TComponent ; const aaco_connecteur : TCustomConnection ; const ab_PasseErreur : Boolean ) : TClass; overload ;
...
// adat_Dataset   : la dataset de la connexion ado
// ab_PasseErreur : Option passer l'erreur
procedure p_GereException ( adat_Dataset : TDataSet ; aexc_exception : EDatabaseError  ; var AAction : TDataAction );
Begin
  AAction := daRetry;
  if assigned ( adat_Dataset )
{$IFDEF EADO}
  and ( adat_Dataset is TCustomADODataset )  Then
    // Exception avec dataset ado
    f_GereException ( aexc_exception, adat_Dataset,( adat_Dataset as TCustomADODataset ).Connection, False );
  Else
{$ELSE}
  Then
{$ENDIF}
      // Exception sans dataset
      f_GereException ( aexc_exception, nil, nil, False );
End ;


function f_GereException ( const aexc_exception : Exception ; const adat_Dataset : TComponent ; const aaco_connecteur : TCustomConnection ; const ab_PasseErreur : Boolean ) : TClass;
...
{$ENDIF}
  ge_DataSetErrorEvent := {$IFNDEF FPC}@{$ENDIF}p_GereException;
finalization
