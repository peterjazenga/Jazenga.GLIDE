...
{$IFDEF VERSIONS}
      gVer_TSvgFormInfoIni : T_Version = ( Component : 'Composant TOnFormInfoIni' ;
                                           ...
                                           BugsStory : '1.1.4.1 : Always ItemIndex on combo.' +#13#10 +
                                                       '1.1.4.0 : Panels verify.' +#13#10 +
                                                       '1.1.3.0 : IniReadWriteComponent event.' +#13#10 +
                                                       '1.1.2.0 : IniLoaded and IniWrited events.' +#13#10 +
                                                       '1.1.1.0 : UTF8 Directory with upgrading.' +#13#10 +
                                           ...
                                           UnitType : 3 ;
                                           Major : 1 ; Minor : 1 ; Release : 4 ; Build : 1 );
...
type
  ...
  TOnIniComponentEdit = procedure ( const AComponent : TComponent  ; const ATypeEditInProgress : TSaveEdit; const AInifile : TCustomInifile ; var KeepOnComponent : Boolean ) of object;
...
type
  ...
  private
    ...
    FOnReadWriteComponent : TOnIniComponentEdit;
    FPanelFilter : Boolean;
    FOnFormDestroy,
    FOnFormShow   ,
    FOnFormCreate : TNotifyEvent ;
    FOnIniLoad,FOnIniLoaded, FOnIniWrite, FOnIniWrited : TEventIni;
...
//    procedure loaded; override;
    function GetfeSauveEdit(const AComponent : TComponent ; const aSauveObjet:TSaveEdits;const aTypeEdit :TSaveEdit):Boolean ; virtual;
...
//    property SavePosObjects: Boolean read FSavePosObjects write FSavePosObjects default False;
    ...
    property PanelFilter : Boolean read FPanelFilter write FPanelFilter default False;
...
//    property SameMonitor: Boolean read FSameMonitor  write FSameMonitor default False;
    property OnIniLoaded  : TEventIni read FOnIniLoaded write FOnIniLoaded ;
    ...
    property OnIniWrited : TEventIni read FOnIniWrited write FOnIniWrited;
    ...
    property OnReadWriteComponent  : TOnIniComponentEdit read FOnReadWriteComponent  write FOnReadWriteComponent;
...
////////////////////////////////////////////////////////////////////////////////
function TOnFormInfoIni.GetfeSauveEdit(const AComponent : TComponent ; const aSauveObjet:TSaveEdits;const aTypeEdit :TSaveEdit):Boolean;
begin
  Result := False;
  if aTypeEdit in aSauveObjet then
    Result := True;
  if assigned ( FOnReadWriteComponent ) Then
   FOnReadWriteComponent ( AComponent, aTypeEdit, FIniFile, Result );
end;
...
var
  ...
  function GetSauveEdit(const aTypeEdit :TSaveEdit):Boolean ;
   Begin
     Result := GetfeSauveEdit( lcom_Component, FSaveEdits, aTypeEdit );
   End;
  ...
  Begin
    ...
    if (lcom_Component is TBaseVirtualTree )
     and   GetSauveEdit(feTVirtualTrees) then
        begin
    ...
    if (lcom_Component is TCustomListView)
     and   GetSauveEdit(feTListView) then
        begin
  ...
  function ReadAtComponent:Boolean;
   Begin
     Result := False;
     if Assigned(FOnReadComponent) Then
       Begin  // filtering event
         ...
         if not ab_keepon
          Then
           Begin
             if lcom_Component is TCustomPanel then
              Result:=True; // will skip controls of Tpanel
            Exit;
           End;
       end;
  ...
  End;
  procedure ReadControl ( const AComPanel : TWinControl );
  var k : Integer;
  Begin
    for k := 0 to AComPanel.ControlCount - 1 do
      try
        lcom_Component := AComPanel.Controls[k];
        if ReadAtComponent Then
         Continue;
        if   (lcom_Component is TWinControl )
        and ((lcom_Component as TWinControl ).controlCount > 0 ) Then
         ReadControl(lcom_Component as TWinControl);
      except
      end;
...
begin
  ...
  if Assigned(FInifile) then
    try
      ...
      If Assigned ( FOnIniLoaded ) Then
        FOnIniLoaded ( FInifile, ab_keepon );
    end;
 p_Freeini;
end;
...
var ls_Temp : String;
  function GetSauveEdit(const aTypeEdit :TSaveEdit):Boolean ;
   Begin
     Result := GetfeSauveEdit( acom_Component, FSaveEdits, aTypeEdit );
   End;
  ...
  Begin
    ...
    // lecture des RadioBoutons
    if  GetSauveEdit ( feTSpeedButton )
  ...
  Begin
    Result := False;
    if GetSauveEdit(feTedit) and ((acom_Component is TCustomEdit)
    ...
    if GetSauveEdit(feTDateEdit) Then
  ...
  Begin
    Result := False;
    if GetSauveEdit (feTFileNameEdit)
  ...
  Begin
    Result := False;
    if GetSauveEdit (feTDirectoryEdit)
  ...
  Begin
    Result := False;
    {$IFDEF DELPHI}
    if GetSauveEdit(feTDateTimePicker) Then
  ...
  Begin
    ...
    if  GetSauveEdit (feTComboBox)
    and (acom_Component is TComboBox)    then
      begin
        p_ReadComboBoxItems(acom_Component, TCustomComboBox(acom_Component).Items );
        Result := True;
      end;
    if (   (GetSauveEdit (feTComboValue) and (acom_Component is TCustomComboBox))
        or (GetSauveEdit (feTListValue ) and (acom_Component is TCustomListBox )))
     then
      try
        ...
        if   assigned ( GetPropInfo ( acom_Component, CST_PROPERTY_ITEMINDEX ))
  ...
  Begin
    ...
    if GetSauveEdit ( feTPageControl )
...
//        WriteLn( af_Form.Name + '  ' +  lcom_Component.Name +' ' +IntToStr(TControl (lcom_Component).Left));
  ...
  Begin
    ...
    if GetSauveEdit (feTCurrencyEdit)
  ...
  Begin
    Result := False;
    if  GetSauveEdit ( feTCheck )
    and (    (lcom_Component is TCheckBox)
    ...
    if GetSauveEdit ( feTRadio )
    and (lcom_Component is TRadioButton) then
      begin
        FInifile.WriteBool(af_Form.name,lcom_Component.Name,TRadioButton(lcom_Component).Checked);
        Result := True;
      end;
    if GetSauveEdit ( feTSpeedButton )
    and (lcom_Component is TSpeedButton) then
      begin
        FInifile.WriteBool(af_Form.name,lcom_Component.Name,TSpeedButton(lcom_Component).Down);
        Result := True;
      end;
    if GetSauveEdit ( feTRadioGroup )
    and (lcom_Component is TRadioGroup)   then
      begin
  ...
  Begin
    Result := False;
    if GetSauveEdit (feTMemo)
    and (lcom_Component is TCustomMemo)     then
      begin
        SauveTStringsDansIni(FInifile, af_Form.Name + '-' +  lcom_Component.Name,TCustomMemo(lcom_Component).Lines);
        Result := True;
      end;
    if GetSauveEdit (feTRichEdit)
    and {$IFDEF FPC}(lcom_Component.ClassNameIs(CST_ONFORMINI_RICHVIEW) or lcom_Component.ClassNameIs(CST_ONFORMINI_RICHMEMO) {$ELSE} (lcom_Component is  TCustomRichEdit {$ENDIF})
    and ( fobj_getComponentObjectProperty(lcom_Component, CST_ONFORMINI_LINES ) <> nil)
     then
      begin
        SauveTStringsDansIni(FInifile, af_Form.Name + '-' +  lcom_Component.Name,fobj_getComponentObjectProperty(lcom_Component, CST_ONFORMINI_LINES ) as TStrings);
        Result := True;
      end;
    ...
    and (lcom_Component.CLassNameIs( CST_ONFORMINI_EXTCOLOR))
     then
      begin
        ...
        Exit;
      End;
    if (   (GetSauveEdit (feTComboValue) and (lcom_Component is TCustomComboBox))
        or (GetSauveEdit (feTListValue ) and (lcom_Component is TCustomListBox )))
     Then
      begin
        ...
        if   assigned ( GetPropInfo ( lcom_Component, CST_PROPERTY_ITEMINDEX ))
  ...
  Begin
    ...
    if GetSauveEdit ( feTPopup )
    and (lcom_Component is TPopupMenu )  then
      begin
  ...
  Begin
    ...
    and (lcom_Component is TListBox)         then
      begin
        SauveTStringsDansIni(FInifile, lcom_Component.Name,TCustomListBox(lcom_Component).Items);
        Result := True;
      end;
  {$IFDEF DELPHI}
    if GetSauveEdit(feTDateTimePicker)
    and (lcom_Component is TDateTimePicker)  then
      begin
  ...
  function WriteAtComponent : Boolean;
  Begin
    Result := False;
    if Assigned(FOnWriteComponent) Then
     Begin // filtering event
       ...
       if not ab_keepon
        Then
         Begin
          Result := True; //skip panel controls
          Exit;
         end;
     end;
  ...
  procedure WriteControl(const Acom_Panel: TWinControl);
  var k : Integer;
  Begin
    For k:=0 to Acom_Panel.ControlCount-1 do
      begin
        lcom_Component := Acom_Panel.Controls[k];
        if WriteAtComponent Then
         Continue;
        if   ( lcom_Component is TWinControl )
        and (( lcom_Component as TWinControl ).ControlCount > 0 ) Then
         WriteControl ( Acom_Panel as TWinControl );
        end;
...
begin
  ...
  If Assigned ( FOnIniWrited ) Then
    FOnIniWrited ( FInifile, ab_keepon );
