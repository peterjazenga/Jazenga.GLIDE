...
(* Const before type ignored *)
  ...
  var
    eglGetProcAddress : function(procname:{$IFDEF WINCE}PWideChar{$ELSE}PChar{$ENDIF}):__eglMustCastToProperFunctionPointerType;stdcall;
...
function glGetProcAddress(ahlib:thandle;ProcName:{$IFDEF WINCE}PWideChar{$ELSE}PChar{$ENDIF}):pointer;
...
implementation
  ...
  function glGetProcAddress(ahlib:thandle;ProcName:{$IFDEF WINCE}PWideChar{$ELSE}PChar{$ENDIF}):pointer;
    begin
      result:=GetProcAddress(ahlib,ProcName);      //9999
  ...
  procedure LoadEGL(lib : {$IFDEF WINCE}PWideChar{$ELSE}PChar{$ENDIF});
    begin
      FreeEGL;
      EGLLib:=LoadLibrary(lib); //9999
      if EGLLib=0 then
  ...
  procedure LoadGLESv2(lib : {$IFDEF WINCE}PWideChar{$ELSE}PChar{$ENDIF});
    begin
      FreeGLESv2;
