...
Type
  ...
  public
    //Procedure CreateForm(AClass: TCGIDataModuleClass; Var Reference: TCGIDataModule);
    Procedure CreateForm(InstanceClass: TCGIDataModuleClass; out Reference);
...
procedure TModuledCGIApplication.CreateForm(InstanceClass: TCGIDataModuleClass; out Reference);
  var Instance: TCGIDataModuleClass;
begin

  // Allocate the instance, without calling the constructor
  Instance := TCGIDataModuleClass(InstanceClass.NewInstance);


  // set the Reference before the constructor is called, so that
  // events and constructors can refer to it
  TCGIDataModuleClass(Reference) := Instance;

  //Reference:=AClass.Create(Self);
  If FMainModule=Nil then
    FMainModule:=TCGIDataModule(Reference);
end;
