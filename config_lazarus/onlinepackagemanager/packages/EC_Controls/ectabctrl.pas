...
type
  ...
  TECTabCtrlFlag = (ectfCloseBtnDown,     { Close button of any tab is pushed down }
                    ectfDeletingTab,      { Deleting tab, prevent cases when tab changes but TabIndex remains }
...
var i, aPrevID, aPrevIndex: Integer;
    bCanClose: Boolean;
begin
  bCanClose:=True;
  if assigned(OnCloseQuery) then OnCloseQuery(self, AIndex, bCanClose);
  if bCanClose then
    begin
      ...
      if AIndex=TabIndex then
        begin
          Flags:=Flags+[ectfDeletingTab, ectfKeepActiveVisi];
          j:=-1;
...
begin
  ...
  Tabs[AFolder].FoldedTabs.Add(Tabs[AIndex]);   //Insert(0, Tabs[AIndex]);
  ...
  begin
    ...
    if aTab.FoldedTabs.Count>0 then
      begin
        ...
        Canvas.DrawGlyph(aTab.DropDownRect, aGlyphDesign, eisEnabled);
...
var bAllow: Boolean;
begin
  if (FTabIndex=AValue) and not (ectfDeletingTab in Flags) then exit;
  bAllow:=True;
