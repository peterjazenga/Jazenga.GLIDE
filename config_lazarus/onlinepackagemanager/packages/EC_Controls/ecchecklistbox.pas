...
begin  { do not call inherited ! }
  ...
  aLeft:=ARect.Right-Indent-CheckColumns*aGlyphWidth-(CheckColumns-1)*Spacing-1;
  ...
  for i:=0 to CheckColumns-1 do
    begin
      ...
      if not bRightToLeft
        then anyRect.Left:=aLeft+i*(aGlyphWidth+Spacing)
        else anyRect.Left:=ARect.Right-ARect.Left-aLeft-i*Spacing-(i+1)*aGlyphWidth;
      ...
      if Grid and (bRightToLeft or (i<(CheckColumns-1))) then
        begin
          Canvas.MoveTo(anyRect.Left+aGlyphWidth+Spacing div 2, ARect.Top);
          Canvas.LineTo(Canvas.PenPos.X, ARect.Bottom);
        end;
    end;
          ...
          if not (FRightToLeft xor (Alignment=taRightJustify)) then
            begin
              while (X>=aLimit) and (i<aClmnsM1) do
               begin
                 inc(aLimit, FGlyphWidth+Spacing);
                 inc(i);
               end;
            end else
            begin
              while (X<aLimit) and (i<aClmnsM1) do
               begin
                 dec(aLimit, FGlyphWidth+Spacing);
                 inc(i);
               end;
            end;
        end;
