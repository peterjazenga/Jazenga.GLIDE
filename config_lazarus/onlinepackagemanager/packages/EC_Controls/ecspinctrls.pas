...
begin
  ...
  for i := 0 to ClientList.Count - 1 do
    if TControl(ClientList[i]) is TCustomSpinBtns
      then TCustomSpinBtns(TControl(ClientList[i])).FController := nil
      else if TControl(ClientList[i]) is TECSpinBtns
             then TECSpinBtns(TControl(ClientList[i])).FController := nil;
...
begin
  if TControl(AClient) is TCustomSpinBtns then ABtns := TCustomSpinBtns(TControl(AClient));
  if TControl(AClient) is TECSpinEdit then ABtns := TECSpinEdit(TControl(AClient)).Buttons;
