    ...
    begin
      ...
      case Index of
        ...
        2: begin
             ...
             aHook.PersistentDeleting(aECGCol);
             aECG.Columns.Delete(0);
             aHook.SelectOnlyThis(aECG.Columns);  { force the OI to refresh }
           end;
        3: begin
             i:=aECG.Columns.Count-1;
             aECGCol:=aECG.Columns[i];
             aHook.PersistentDeleting(aECGCol);
             aECG.Columns.Delete(i);
             aHook.SelectOnlyThis(aECG.Columns);  { force the OI to refresh }
           end;
        4: begin
             for i:=aECG.Columns.Count-1 downto 0 do
               begin
                 aECGCol:=aECG.Columns[i];
                 aHook.PersistentDeleting(aECGCol);
                 aECG.Columns.Delete(i);
...
begin
  ...
  if Index>0 then AnItem.Enabled:= (TECGrid(GetComponent).Columns.Count>0);
end;
