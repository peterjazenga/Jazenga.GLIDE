...
uses
  Types, Classes, ImagingTypes, ImagingCore;
...
begin
  ...
  begin
    ...
    if not Info.IsSpecial then
      Result := ImagingCore.GetScanLine(FPData.Bits, Info, FPData.Width, Index)
    else
      Result := FPData.Bits;
  end
  else
...
begin
  if Valid then
    ImagingCore.GetImageFormatInfo(FPData.Format, Result)
  else
...
begin
  Result := Assigned(FPData) and ImagingCore.TestImage(FPData^);
end;
...
begin
  if Valid and ImagingCore.ConvertImage(FPData^, Value) then
    DoDataSizeChanged;
end;
...
begin
  if Assigned(FPData) and ImagingCore.NewImage(AWidth, AHeight, AFormat, FPData^) then
    DoDataSizeChanged;
end;
...
begin
  if Valid and ImagingCore.ResizeImage(FPData^, NewWidth, NewHeight, Filter) then
    DoDataSizeChanged;
end;
...
begin
  ...
  begin
    ImagingCore.ResizeImageToFit(FPData^, FitWidth, FitHeight, Filter,
      DstImage.FPData^);
...
begin
  if Valid and ImagingCore.FlipImage(FPData^) then
    DoPixelsChanged;
end;

procedure TBaseImage.Mirror;
begin
  if Valid and ImagingCore.MirrorImage(FPData^) then
    DoPixelsChanged;
end;
...
begin
  if Valid then
  begin
    ImagingCore.RotateImage(FPData^, Angle);
    DoPixelsChanged;
  end;
end;
...
begin
  ...
  begin
    ImagingCore.CopyRect(FPData^, SrcX, SrcY, Width, Height, DstImage.FPData^, DstX, DstY);
...
begin
  if Valid and Assigned(DstImage) and DstImage.Valid then
  begin
    ImagingCore.StretchRect(FPData^, SrcX, SrcY, SrcWidth, SrcHeight,
...
begin
  if Valid then
  begin
    ImagingCore.ReplaceColor(FPData^, X, Y, Width, Height, OldColor, NewColor);
    DoPixelsChanged;
  end;
end;
...
begin
  if Valid then
  begin
    ImagingCore.SwapChannels(FPData^, SrcChannel, DstChannel);
    DoPixelsChanged;
  end;
end;
...
begin
  Result := Iff(Valid, ImagingCore.ImageToStr(FPData^), 'empty image');
end;

procedure TBaseImage.LoadFromFile(const FileName: string);
begin
  if Assigned(FPData) and ImagingCore.LoadImageFromFile(FileName, FPData^) then
    DoDataSizeChanged;
end;

procedure TBaseImage.LoadFromStream(Stream: TStream);
begin
  if Assigned(FPData) and ImagingCore.LoadImageFromStream(Stream, FPData^) then
    DoDataSizeChanged;
end;
...
begin
  if Valid then
    ImagingCore.SaveImageToFile(FileName, FPData^);
end;

procedure TBaseImage.SaveToStream(const Ext: string; Stream: TStream);
begin
  if Valid then
    ImagingCore.SaveImageToStream(Ext, Stream, FPData^);
end;
...
begin
  inherited Create;
  if ImagingCore.TestImage(AData) then
    begin
      ImagingCore.CloneImage(AData, FImageData);
      DoDataSizeChanged;
    end
  else
    Create;
...
begin
  ImagingCore.FreeImage(FImageData);
  inherited Destroy;
end;
...
var
  I: Integer;
begin
  ImagingCore.FreeImagesInArray(FDataArray);
  SetLength(FDataArray, Images);
  for I := 0 to GetImageCount - 1 do
    ImagingCore.NewImage(AWidth, AHeight, AFormat, FDataArray[I]);
...
begin
  ...
  begin
    ...
    if ImagingCore.TestImage(ADataArray[I]) then
      ImagingCore.CloneImage(ADataArray[I], FDataArray[I])
    else
      ImagingCore.NewImage(DefaultWidth, DefaultHeight, ifDefault, FDataArray[I]);
  end;
  if GetImageCount > 0 then
    SetActiveImage(0);
end;
...
begin
  ...
  begin
    ...
    for I := Value to GetImageCount - 1 do
      ImagingCore.FreeImage(FDataArray[I]);
    SetLength(FDataArray, Value);
  end;
  SetPointer;
end;
...
begin
  if (Index >= 0) and (Index < GetImageCount) then
    ImagingCore.CloneImage(Value, FDataArray[Index]);
end;
...
begin
  ...
  begin
    for I := 0 to Len - 1 do
      ImagingCore.CloneImage(Images[I], FDataArray[Index + I]);
  end;
end;
...
begin
  if PrepareInsert(Index, 1) then
    ImagingCore.NewImage(AWidth, AHeight, AFormat, FDataArray[Index]);
end;
...
begin
  ...
  begin
    ...
    ImagingCore.FreeImage(FDataArray[Index]);
    if Index < GetImageCount - 1 then
    begin
...
begin
  for I := 0 to GetImageCount - 1 do
    ImagingCore.ConvertImage(FDataArray[I], Format);
end;
...
begin
  for I := 0 to GetImageCount - 1 do
    ImagingCore.ResizeImage(FDataArray[I], NewWidth, NewHeight, Filter);
end;
...
begin
  ImagingCore.LoadMultiImageFromFile(FileName, FDataArray);
  SetActiveImage(0);
end;

procedure TMultiImage.LoadMultiFromStream(Stream: TStream);
begin
  ImagingCore.LoadMultiImageFromStream(Stream, FDataArray);
  SetActiveImage(0);
end;

procedure TMultiImage.SaveMultiToFile(const FileName: string);
begin
  ImagingCore.SaveMultiImageToFile(FileName, FDataArray);
end;

procedure TMultiImage.SaveMultiToStream(const Ext: string; Stream: TStream);
begin
  ImagingCore.SaveMultiImageToStream(Ext, Stream, FDataArray);
end;
