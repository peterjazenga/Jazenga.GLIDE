...
procedure TImagingCanvas.TranslateFPToNative(const Color: TColorFPRec;
  Native: Pointer);
begin
  ImagingCore.TranslatePixel(@Color, Native, ifA32R32G32B32F,  //=== ct9999 =============
...
begin
  ...
  begin
    ...
    case Bpp of
      ...
      for I := X1 to X2 do
      begin
        ImagingCore.CopyPixel(Color, PixelPtr, Bpp);   //=== ct9999 =====
        Inc(PixelPtr, Bpp);
       end;
    end;
  end;
...
begin
  if (X >= FClipRect.Left) and (Y >= FClipRect.Top) and
    (X < FClipRect.Right) and (Y < FClipRect.Bottom) then
  begin
    ImagingCore.CopyPixel(Pixel, GetPixelPointer(X, Y), Bpp);  //=== ct9999 ==========
  end;
end;
