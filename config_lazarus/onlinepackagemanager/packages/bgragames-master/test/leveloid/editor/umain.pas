...
uses
  ...
  ExtCtrls, ColorBox, uleveloid, bgPanel, BGRABitmap, BGRABitmapTypes, types;
...
begin
  ...
  begin
    ...
    clDialog.Color := BGRAToColor(TileMap.Map.BackgroundColor);

    if TileMap.Layers[0].Data.Count < (TileMap.Map.Width * TileMap.Map.Height) then
    begin
      TileMap.Layers[0].Data.Clear;
      for i := 0 to (TileMap.Map.Width * TileMap.Map.Height) do
        TileMap.Layers[0].Data.Add('-1');
    end;
  end
  else
  begin
    ShowMessage('Arrastra un archivo .leveloid al programa.');
    bgMap.OnRedraw := nil;
...
begin
  ...
  begin
    ...
    TileMap.Map.BackgroundColor := ColorToBGRA(clDialog.Color);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        lblID.Caption := 'id: ' + IntToStr(n);
        lblPOS.Caption := 'x: ' + IntToStr(x) + ' y: ' + IntToStr(y);
        CursorRect := r;
...
begin
  ...
  Bitmap.Rectangle(CursorRect.Left, CursorRect.Top, CursorRect.Right,
    CursorRect.Bottom, BGRAWhite, BGRA(255, 255, 255, 50), dmDrawWithTransparency);
end;
