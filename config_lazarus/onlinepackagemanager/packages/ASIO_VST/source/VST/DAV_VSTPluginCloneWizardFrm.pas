...
uses
  LCLIntf, LCLType, LMessages, Messages, Classes, Controls, Forms, StdCtrls, ComCtrls, ExtCtrls,
  Graphics, DAV_VSTPluginCloneConfig, Dialogs, FileUtil;
 ...
 case StepIndex of
    STEP_DESTINATION:
      begin
        ...
        else if not DirectoryExistsUTF8(edtProjectPath.Text) { *Converted from DirectoryExists*  } then
        begin
          if MessageDlg(RCProjectPathDoesNotExists, mtConfirmation, [mbYes,
            mbNo], 0) = mrYes then
          begin
            ForceDirectoriesUTF8(edtProjectPath.Text); { *Converted from ForceDirectories*  }
            Result := True;
          end;
        end
        else
    ...
    STEP_SELECT_PLUGIN:
      begin
        ...
        else if not FileExistsUTF8(EdClonedPlugin.Text) { *Converted from FileExists*  } then
