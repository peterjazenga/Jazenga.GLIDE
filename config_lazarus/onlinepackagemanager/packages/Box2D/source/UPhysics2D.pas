{ box2D 2.3.0 translation
  ...
  Translator: Qianyuan Wang(IoC¬O)
...
begin
   ...
   PInt32(Int32(m_stack) + SizeOf(Int32) * m_count)^ := element;
   Inc(m_count);
end;
...
begin
   //b2Assert(m_count > 0);
	 Dec(m_count);
	 Result := PInt32(Int32(m_stack) + SizeOf(Int32) * m_count)^;
end;
...
function Tb2Body.GetLinearVelocityFromLocalPoint( const localPoint: TVector2): TVector2;
  var xtv:TVector2;
begin
   xtv:=GetWorldPoint(localPoint);
   Result := GetLinearVelocityFromWorldPoint(xtv);
end;
...
begin
   {$IFDEF OP_OVERLOAD}
   xtv:=m_bodyA.GetWorldPoint(m_localAnchorA);
   Result := (xtv - m_groundAnchorA).Length;
   {$ELSE}
...
begin
   {$IFDEF OP_OVERLOAD}
   xtv:=m_bodyB.GetWorldPoint(m_localAnchorB);
   Result := (xtv - m_groundAnchorB).Length;
   {$ELSE}
...
function Tb2WheelJoint.GetJointTranslation: PhysicsFloat;
  var xtv0,xtv1,xtv2: TVector2;
begin
   {$IFDEF OP_OVERLOAD}
   xtv0:=m_bodyB.GetWorldPoint(m_localAnchorB);
   xtv1:= m_bodyA.GetWorldPoint(m_localAnchorA);
   xtv2:= m_bodyA.GetWorldVector(m_localXAxisA);

   Result := b2Dot(xtv0 - xtv1, xtv2);
   {$ELSE}
