...
begin
  ...
  if (Operation  = opInsert) and (AComponent is TfpgWidget) then
  begin
   ...
   //   UnLoadLib;
   // sakupdate;
 // ShowMessage('a component was added',  true);
 // fpgapplication.ProcessMessages;
  end;
   if (Operation = opRemove)  and (AComponent is TfpgWidget) then
   begin
  // fpgapplication.ProcessMessages;
  // ShowMessage('a component was removed', true)   ;
     //  saksuspend;
     //    UnLoadLib;
     //  sakupdate;
     InitSpeech.TimerCount.Enabled := false;
...
begin
  ...
  begin
    ...
    TheNotifier := TNotifier.create(fpgapplication);
    initspeech.voice_language := '';
    initspeech.voice_gender := '';
...
begin
  ...
  for f := 0 to fpgapplication.ComponentCount - 1 do
  begin
    SetLength(InitSpeech.AssistiveData, Length(InitSpeech.AssistiveData) + 1);
    InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1] :=
      TSAK_Assistive.Create();
   ...
   // with (fpgapplication.Components[f]) as TfpgForm do

      for i := 0 to fpgapplication.Components[f].ComponentCount - 1 do
      begin
        if (fpgapplication.Components[f].Components[i] is TfpgWidget) or (fpgapplication.Components[f].Components[i] is TfpgButton) or (fpgapplication.Components[f].Components[i] is TfpgMemo) or
          (fpgapplication.Components[f].Components[i] is TfpgEdit) or (fpgapplication.Components[f].Components[i] is TfpgStringGrid) or (fpgapplication.Components[f].Components[i] is TfpgCheckBox) or
          (fpgapplication.Components[f].Components[i] is TfpgRadiobutton) or (fpgapplication.Components[f].Components[i] is TfpgListBox) or (fpgapplication.Components[f].Components[i] is TfpgComboBox) or
          (fpgapplication.Components[f].Components[i] is TfpgPopupMenu) or (fpgapplication.Components[f].Components[i] is TfpgMenuItem) or (fpgapplication.Components[f].Components[i] is TfpgTrackBar) or (fpgapplication.Components[f].Components[i] is TfpgLabel)
            // or (fpgapplication.Components[f].Components[i] is TfpgFileDialog) or (fpgapplication.Components[f].Components[i] is TfpgSaveDialog)
        then
        begin

          if (fpgapplication.Components[f].Components[i] is TfpgPopupMenu) then
          begin
            SetLength(InitSpeech.AssistiveData, Length(InitSpeech.AssistiveData) + 1);
            ...
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].Description :=
              'Menu,';
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].TheObject :=
              TfpgPopupMenu(fpgapplication.Components[f].Components[i]);
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnClick :=
              TfpgPopupMenu(fpgapplication.Components[f].Components[i]).OnShow;
            TfpgPopupMenu(fpgapplication.Components[f].Components[i]).OnShow := @InitSpeech.SAKClick;

            //  TfpgPopupMenu(fpgapplication.Components[f].Components[i]).OnMouseMove;
            //  TfpgPopupMenu(fpgapplication.Components[f].Components[i]).OnMouseMove := @InitSpeech.SAKMouseMove;

            with (TfpgPopupMenu(fpgapplication.Components[f].Components[i]) as TfpgPopupMenu) do
              ...
              begin
                ...
                //     InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnMouseMove :=
                //  TfpgMenuItem(fpgapplication.Components[f].Components[i]).OnMouseMove;
                // TfpgMenuItem(fpgapplication.Components[f].Components[i]).OnMouseMove := @InitSpeech.SAKMouseMove;
              end;
          end

          else

         if (fpgapplication.Components[f].Components[i] is TfpgLabel) then
          begin
            SetLength(InitSpeech.AssistiveData, Length(InitSpeech.AssistiveData) + 1);
            ...
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].Description :=
              'Label,';
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].TheObject :=
              TfpgLabel(fpgapplication.Components[f].Components[i]);
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnClick :=
              TfpgLabel(fpgapplication.Components[f].Components[i]).OnClick;
            TfpgLabel(fpgapplication.Components[f].Components[i]).OnClick := @InitSpeech.SAKClick;
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnMouseMove :=
              TfpgLabel(fpgapplication.Components[f].Components[i]).OnMouseMove;
            TfpgLabel(fpgapplication.Components[f].Components[i]).OnMouseMove := @InitSpeech.SAKMouseMove;
          end
          else
          if (fpgapplication.Components[f].Components[i] is TfpgButton) then
          begin
            SetLength(InitSpeech.AssistiveData, Length(InitSpeech.AssistiveData) + 1);
            ...
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].Description :=
              'Button,';
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].TheObject :=
              TfpgButton(fpgapplication.Components[f].Components[i]);
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnClick :=
              TfpgButton(fpgapplication.Components[f].Components[i]).OnClick;
            TfpgButton(fpgapplication.Components[f].Components[i]).OnClick := @InitSpeech.SAKClick;
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnEnter :=
              TfpgButton(fpgapplication.Components[f].Components[i]).OnEnter;
            TfpgButton(fpgapplication.Components[f].Components[i]).OnEnter := @InitSpeech.SAKEnter;

            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnMouseMove :=
              TfpgButton(fpgapplication.Components[f].Components[i]).OnMouseMove;
            TfpgButton(fpgapplication.Components[f].Components[i]).OnMouseMove := @InitSpeech.SAKMouseMove;
          end
          else
          if (fpgapplication.Components[f].Components[i] is TfpgStringGrid) then
          begin
            SetLength(InitSpeech.AssistiveData, Length(InitSpeech.AssistiveData) + 1);
            ...
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].Description :=
              'Grid,';
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].TheObject :=
              TfpgStringGrid(fpgapplication.Components[f].Components[i]);
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnFocusChange :=
              TfpgStringGrid(fpgapplication.Components[f].Components[i]).OnFocusChange;
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnMouseDown :=
              TfpgStringGrid(fpgapplication.Components[f].Components[i]).OnMouseDown;
            TfpgStringGrid(fpgapplication.Components[f].Components[i]).OnFocusChange := @InitSpeech.SAKFocusChange;
            TfpgStringGrid(fpgapplication.Components[f].Components[i]).OnMouseDown := @InitSpeech.SAKMouseDown;

            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnEnter :=
              TfpgStringGrid(fpgapplication.Components[f].Components[i]).OnEnter;
            TfpgStringGrid(fpgapplication.Components[f].Components[i]).OnEnter := @InitSpeech.SAKEnter;

            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnMouseMove :=
              TfpgStringGrid(fpgapplication.Components[f].Components[i]).OnMouseMove;
            TfpgStringGrid(fpgapplication.Components[f].Components[i]).OnMouseMove := @InitSpeech.SAKMouseMove;
          end
          else
          if (fpgapplication.Components[f].Components[i] is TfpgTrackBar) then
          begin
            SetLength(InitSpeech.AssistiveData, Length(InitSpeech.AssistiveData) + 1);
            ...
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].Description :=
              'Track bar,';
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].TheObject :=
              TfpgTrackBar(fpgapplication.Components[f].Components[i]);
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnTrackbarChange :=
              TfpgTrackBar(fpgapplication.Components[f].Components[i]).OnChange;

            TfpgTrackBar(fpgapplication.Components[f].Components[i]).OnChange := @InitSpeech.SAKTrackBarChange;

            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnEnter :=
              TfpgTrackBar(fpgapplication.Components[f].Components[i]).OnEnter;

            TfpgTrackBar(fpgapplication.Components[f].Components[i]).OnEnter := @InitSpeech.SAKEnter;

            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnMouseMove :=
              TfpgTrackBar(fpgapplication.Components[f].Components[i]).OnMouseMove;
            TfpgTrackBar(fpgapplication.Components[f].Components[i]).OnMouseMove := @InitSpeech.SAKMouseMove;
          end
          else
          if (fpgapplication.Components[f].Components[i] is TfpgRadiobutton) then
          begin
            SetLength(InitSpeech.AssistiveData, Length(InitSpeech.AssistiveData) + 1);
            ...
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].Description :=
              'Radio Button,';
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].TheObject :=
              TfpgRadiobutton(fpgapplication.Components[f].Components[i]);

            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnChange :=
              TfpgRadiobutton(fpgapplication.Components[f].Components[i]).OnChange;

            TfpgRadiobutton(fpgapplication.Components[f].Components[i]).OnChange := @InitSpeech.SAKChange;


            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnEnter :=
              TfpgRadiobutton(fpgapplication.Components[f].Components[i]).OnEnter;

            TfpgRadiobutton(fpgapplication.Components[f].Components[i]).OnEnter := @InitSpeech.SAKEnter;

            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnMouseMove :=
              TfpgRadiobutton(fpgapplication.Components[f].Components[i]).OnMouseMove;
            TfpgRadiobutton(fpgapplication.Components[f].Components[i]).OnMouseMove := @InitSpeech.SAKMouseMove;
          end
          else

          if (fpgapplication.Components[f].Components[i] is TfpgCheckBox) then
          begin
            SetLength(InitSpeech.AssistiveData, Length(InitSpeech.AssistiveData) + 1);
            ...
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].Description :=
              'Check Box,';
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].TheObject :=
              TfpgCheckBox(fpgapplication.Components[f].Components[i]);
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnChange :=
              TfpgCheckBox(fpgapplication.Components[f].Components[i]).OnChange;
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnEnter :=
              TfpgCheckBox(fpgapplication.Components[f].Components[i]).OnEnter;

            TfpgCheckBox(fpgapplication.Components[f].Components[i]).OnChange := @InitSpeech.SAKChange;
            TfpgCheckBox(fpgapplication.Components[f].Components[i]).OnEnter := @InitSpeech.SAKEnter;
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnMouseMove :=
              TfpgCheckBox(fpgapplication.Components[f].Components[i]).OnMouseMove;
            TfpgCheckBox(fpgapplication.Components[f].Components[i]).OnMouseMove := @InitSpeech.SAKMouseMove;
          end
          else
          if (fpgapplication.Components[f].Components[i] is TfpgListBox) then
          begin
            SetLength(InitSpeech.AssistiveData, Length(InitSpeech.AssistiveData) + 1);
            ...
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].Description :=
              'List Box,';
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].TheObject :=
              TfpgListBox(fpgapplication.Components[f].Components[i]);
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnChange :=
              TfpgListBox(fpgapplication.Components[f].Components[i]).OnChange;

            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnEnter :=
              TfpgListBox(fpgapplication.Components[f].Components[i]).OnEnter;

            TfpgListBox(fpgapplication.Components[f].Components[i]).OnChange := @InitSpeech.SAKChange;
            TfpgListBox(fpgapplication.Components[f].Components[i]).OnEnter := @InitSpeech.SAKEnter;

            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnMouseMove :=
              TfpgListBox(fpgapplication.Components[f].Components[i]).OnMouseMove;
            TfpgListBox(fpgapplication.Components[f].Components[i]).OnMouseMove := @InitSpeech.SAKMouseMove;
          end
          else
          if (fpgapplication.Components[f].Components[i] is TfpgComboBox) then
          begin
            SetLength(InitSpeech.AssistiveData, Length(InitSpeech.AssistiveData) + 1);
            ...
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].Description :=
              'Combo Box,';
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].TheObject :=
              TfpgComboBox(fpgapplication.Components[f].Components[i]);
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnChange :=
              TfpgComboBox(fpgapplication.Components[f].Components[i]).OnChange;
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnEnter :=
              TfpgComboBox(fpgapplication.Components[f].Components[i]).OnEnter;
            TfpgComboBox(fpgapplication.Components[f].Components[i]).OnChange := @InitSpeech.SAKChange;
            TfpgComboBox(fpgapplication.Components[f].Components[i]).OnEnter := @InitSpeech.SAKEnter;

            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnMouseMove :=
              TfpgComboBox(fpgapplication.Components[f].Components[i]).OnMouseMove;
            TfpgComboBox(fpgapplication.Components[f].Components[i]).OnMouseMove := @InitSpeech.SAKMouseMove;
          end
          else
          if (fpgapplication.Components[f].Components[i] is TfpgMemo) then
          begin
            SetLength(InitSpeech.AssistiveData, Length(InitSpeech.AssistiveData) + 1);
            ...
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].Description :=
              'Memo,';
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].TheObject :=
              TfpgMemo(fpgapplication.Components[f].Components[i]);
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnEnter :=
              TfpgMemo(fpgapplication.Components[f].Components[i]).OnEnter;
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnKeyChar :=
              TfpgMemo(fpgapplication.Components[f].Components[i]).OnKeyChar;

            TfpgMemo(fpgapplication.Components[f].Components[i]).OnEnter := @InitSpeech.SAKEnter;
            TfpgMemo(fpgapplication.Components[f].Components[i]).OnKeyChar := @InitSpeech.SAKKeyChar;

            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnMouseMove :=
              TfpgMemo(fpgapplication.Components[f].Components[i]).OnMouseMove;
            TfpgMemo(fpgapplication.Components[f].Components[i]).OnMouseMove := @InitSpeech.SAKMouseMove;
          end
          else
          if (fpgapplication.Components[f].Components[i] is TfpgEdit) then
          begin
            SetLength(InitSpeech.AssistiveData, Length(InitSpeech.AssistiveData) + 1);
            ...
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].Description :=
              'Edit,';
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].TheObject :=
              TfpgEdit(fpgapplication.Components[f].Components[i]);
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnEnter :=
              TfpgEdit(fpgapplication.Components[f].Components[i]).OnEnter;
            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnKeyChar :=
              TfpgEdit(fpgapplication.Components[f].Components[i]).OnKeyChar;
            TfpgEdit(fpgapplication.Components[f].Components[i]).OnEnter := @InitSpeech.SAKEnter;
            TfpgEdit(fpgapplication.Components[f].Components[i]).OnKeyChar := @InitSpeech.SAKKeyChar;

            InitSpeech.AssistiveData[Length(InitSpeech.AssistiveData) - 1].OriOnMouseMove :=
              TfpgEdit(fpgapplication.Components[f].Components[i]).OnMouseMove;
            TfpgEdit(fpgapplication.Components[f].Components[i]).OnMouseMove := @InitSpeech.SAKMouseMove;
          end
          else
          if (fpgapplication.Components[f].Components[i] is TfpgWidget) then
          begin
              UpdateChild(fpgapplication.Components[f].Components[i])  ;
              end;
...
procedure SAKUpdate();
begin
  InitSpeech.initobject;
 // f := 0;
 // ChildComponentCount(fpgApplication);
 // InitSpeech.CompCount := f;
//  InitSpeech.timercount.Enabled := True;
end;
