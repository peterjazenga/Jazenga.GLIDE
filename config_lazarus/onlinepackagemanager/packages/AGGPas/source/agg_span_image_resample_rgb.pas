...
{ CONSTRUCT }
constructor span_image_resample_rgb_affine.Construct(alloc: span_allocator_ptr;
...
{ GENERATE }
function span_image_resample_rgb_affine.generate(x, y: int; len: unsigned): aggclr_ptr;
var
  fg: array[0..3] of int;
...
{ CONSTRUCT }
constructor span_image_resample_rgb.Construct(alloc: span_allocator_ptr;
  src: rendering_buffer_ptr; back_color: aggclr_ptr; interpolator: span_interpolator_ptr;
  filter: image_filter_lut_ptr; order: order_type);
begin
  inherited Construct(alloc, src, back_color, interpolator, filter);
...
{ GENERATE }
function span_image_resample_rgb.generate(x, y: int; len: unsigned): aggclr_ptr;
var
  span: aggclr_ptr;
