...
type
  ...
  //------------------------------------------------------------math_stroke
  math_stroke = object
    m_width, m_width_abs, m_width_eps: double;
    m_width_sign: int;
    m_miter_limit, m_inner_miter_limit, m_approx_scale: double;
    ...
    procedure calc_cap(vc: pod_bvector_ptr; v0, v1: vertex_dist_ptr; len: double);
    procedure calc_join(vc: pod_bvector_ptr; v0, v1, v2: vertex_dist_ptr; len1, len2: double);
    procedure add_vertex(vc: pod_bvector_ptr; x, y: double);
    procedure calc_arc(vc: pod_bvector_ptr; x, y, dx1, dy1, dx2, dy2: double);
    procedure calc_miter(vc: pod_bvector_ptr; v0, v1, v2: vertex_dist_ptr;
                         dx1, dy1, dx2, dy2: double; lj: line_join_e; mlimit, dbevel: double);
  end;

procedure stroke_calc_arc(out_vertices: pod_deque_ptr; x, y, dx1, dy1, dx2, dy2, Width, approximation_scale: double);
procedure stroke_calc_miter(out_vertices: pod_deque_ptr; v0, v1, v2: vertex_dist_ptr;
                            dx1, dy1, dx2, dy2, Width: double; line_join: unsigned;
                            miter_limit, approximation_scale: double);
procedure stroke_calc_cap(out_vertices: pod_deque_ptr; v0, v1: vertex_dist_ptr;
                          len: double; line_cap: unsigned; Width, approximation_scale: double);
procedure stroke_calc_join(out_vertices: pod_deque_ptr; v0, v1, v2: vertex_dist_ptr;
                           len1, len2, Width: double; line_join, inner_join: unsigned;
...
procedure math_stroke.calc_cap(vc: pod_bvector_ptr; v0, v1: vertex_dist_ptr; len: double);
var
...
procedure math_stroke.calc_join(vc: pod_bvector_ptr; v0, v1, v2: vertex_dist_ptr; len1, len2: double);
var
...
begin
  ...
  if (cp <> 0) and ((cp > 0) = (m_width > 0)) then
  begin
    // Inner join
    if len1 < len2 then
  ...
  begin
    ...
    if (m_line_join = round_join) or (m_line_join = bevel_join) then
    begin
      ...
      begin
        if calc_intersection(v0^.x + dx1, v0^.y - dy1, v1^.x + dx1, v1^.y - dy1, v1^.x +
          dx2, v1^.y - dy2, v2^.x + dx2, v2^.y - dy2, @dx, @dy) then
          add_vertex(vc, dx, dy)
        else
          add_vertex(vc, v1^.x + dx1, v1^.y - dy1);
...
procedure math_stroke.calc_arc(vc: pod_bvector_ptr; x, y, dx1, dy1, dx2, dy2: double);
var
  a1, a2, da: double;
...
procedure math_stroke.calc_miter(vc: pod_bvector_ptr; v0, v1, v2: vertex_dist_ptr;
  dx1, dy1, dx2, dy2: double; lj: line_join_e; mlimit, dbevel: double);
var
...
begin
  ...
  if calc_intersection(v0^.x + dx1, v0^.y - dy1, v1^.x + dx1, v1^.y - dy1, v1^.x + dx2, v1^.y - dy2,
    v2^.x + dx2, v2^.y - dy2, @xi, @yi) then
  begin
  ...
  begin
    ...
    if (cross_product(v0^.x, v0^.y, v1^.x, v1^.y, x2, y2) < 0.0) = (cross_product(v1^.x, v1^.y, v2^.x, v2^.y, x2, y2) < 0.0) then
    begin
...
procedure stroke_calc_miter(out_vertices: pod_deque_ptr; v0, v1, v2: vertex_dist_ptr;
  dx1, dy1, dx2, dy2, Width: double; line_join: unsigned; miter_limit, approximation_scale: double);
var
  pt: point_type;
...
begin
  ...
  begin
    ...
    if (((x2 - v0^.x) * dy1 - (v0^.y - y2) * dx1 < 0.0) <> ((x2 - v2^.x) * dy1 - (v2^.y - y2) * dx1 < 0.0)) then
    begin
      // This case means that the next segment continues
      // the previous one (straight line)
      //-----------------
