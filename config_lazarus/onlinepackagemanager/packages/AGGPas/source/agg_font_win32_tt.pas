...
{ CONSTRUCT }
constructor font_engine_win32_tt_base.Construct(flag32_ : boolean; dc : HDC; max_fonts : unsigned = 32 );
begin
 m_flag32:=flag32_;
 m_dc    :=dc;
...
{ RESOLUTION_ }
procedure font_engine_win32_tt_base.resolution_(dpi : unsigned );
begin
 m_resolution:=dpi;
...
{ HEIGHT_ }
procedure font_engine_win32_tt_base.height_(h : double );
begin
 m_height:=trunc(h );
...
{ WIDTH_ }
procedure font_engine_win32_tt_base.width_(w : double );
begin
 m_width:=trunc(w );
...
{ WEIGHT_ }
procedure font_engine_win32_tt_base.weight_(w : int );
begin
 m_weight:=w;
...
{ ITALIC_ }
procedure font_engine_win32_tt_base.italic_(it : boolean );
begin
 m_italic:=it;
...
{ CHAR_SET_ }
procedure font_engine_win32_tt_base.char_set_(c : DWORD );
begin
 m_char_set:=c;
...
{ PITCH_AND_FAMILY_ }
procedure font_engine_win32_tt_base.pitch_and_family_(p : DWORD );
begin
...
{ FLIP_Y_ }
procedure font_engine_win32_tt_base.flip_y_(flip : boolean );
begin
 m_flip_y:=flip;
...
{ HINTING_ }
procedure font_engine_win32_tt_base.hinting_(h : boolean );
begin
 m_hinting:=h;
...
{ CREATE_FONT }
function font_engine_win32_tt_base.create_font(typeface_ : PChar; ren_type : glyph_rendering ) : boolean;
var
 len : unsigned;
...
{ GAMMA_ }
procedure font_engine_win32_tt_base.gamma_(f : vertex_source_ptr );
begin
...
{ TRANSFORM_ }
procedure font_engine_win32_tt_base.transform_(mtx : trans_affine_ptr );
begin
...
{ _RESOLUTION }
function font_engine_win32_tt_base._resolution : unsigned;
begin
 result:=m_resolution;
...
{ _TYPEFACE }
function font_engine_win32_tt_base._typeface : PChar;
begin
...
{ _HEIGHT }
function font_engine_win32_tt_base._height : double;
begin
 result:=m_height;
...
{ _WIDTH }
function font_engine_win32_tt_base._width : double;
begin
 result:=m_width;
...
{ _WEIGHT }
function font_engine_win32_tt_base._weight : int;
begin
 result:=m_weight;
...
{ _ITALIC }
function font_engine_win32_tt_base._italic : boolean;
begin
 result:=m_italic;
...
{ _CHAR_SET }
function font_engine_win32_tt_base._char_set : DWORD;
begin
 result:=m_char_set;
...
{ _PITCH_AND_FAMILY }
function font_engine_win32_tt_base._pitch_and_family : DWORD;
begin
...
{ _HINTING }
function font_engine_win32_tt_base._hinting : boolean;
begin
 result:=m_hinting;
...
{ _FLIP_Y }
function font_engine_win32_tt_base._flip_y : boolean;
begin
 result:=m_flip_y;
...
{ FONT_SIGNATURE }
function font_engine_win32_tt_base.font_signature : PChar;
begin
...
{ CHANGE_STAMP }
function font_engine_win32_tt_base.change_stamp : int;
begin
...
{ PREPARE_GLYPH }
function font_engine_win32_tt_base.prepare_glyph(glyph_code : unsigned ) : boolean;
const
...
{ GLYPH_INDEX }
function font_engine_win32_tt_base.glyph_index : unsigned;
begin
...
{ DATA_SIZE }
function font_engine_win32_tt_base.data_size : unsigned;
begin
 result:=m_data_size;
...
{ DATA_TYPE }
function font_engine_win32_tt_base.data_type : unsigned;
begin
 result:=m_data_type;
...
{ BOUNDS }
function font_engine_win32_tt_base.bounds : rect_ptr;
begin
 result:=@m_bounds;
...
{ ADVANCE_X }
function font_engine_win32_tt_base.advance_x : double;
begin
 result:=m_advance_x;
...
{ ADVANCE_Y }
function font_engine_win32_tt_base.advance_y : double;
begin
 result:=m_advance_y;
...
{ WRITE_GLYPH_TO }
procedure font_engine_win32_tt_base.write_glyph_to(data : int8u_ptr );
begin
 if (data <> NIL ) and
    (m_data_size <> 0 ) then
  case m_data_type of
...
{ ADD_KERNING }
function font_engine_win32_tt_base.add_kerning(first ,second : unsigned; x ,y : double_ptr ) : boolean;
var
 end_ ,beg_ ,mid : int;
...
{ FLAG32 }
function font_engine_win32_tt_base.flag32 : boolean;
begin
 result:=m_flag32;
...
{ PAIR_LESS }
function font_engine_win32_tt_base.pair_less(v1 ,v2 : KERNINGPAIR_ptr ) : boolean;
begin
...
begin
 ...
 quick_sort(@pairs ,func_less(@_pair_less ));

end;

{ FIND_FONT }
function font_engine_win32_tt_base.find_font(name : PChar ) : int;
var
 i : unsigned;
 n : font_name_ptr;
...
{ CONSTRUCT }
constructor font_engine_win32_tt_int16.Construct(dc : HDC; max_fonts : unsigned = 32 );
begin
...
{ CONSTRUCT }
constructor font_engine_win32_tt_int32.Construct(dc : HDC; max_fonts : unsigned = 32 );
begin
