...
type
  ...
  rbox_ctrl_impl = object(ctrl)
    m_border_width, m_border_extra, m_text_thickness, m_text_height, m_text_width: double;
    m_items: array[0..31] of char_ptr;
    m_sizes: array[0..31] of int8u;
    m_num_items: unsigned;
    m_cur_item: int;
    m_xs1, m_ys1, m_xs2, m_ys2, m_dy: double;
  ...
  rbox_ctrl = object(rbox_ctrl_impl)
    m_background_color, m_border_color, m_text_color, m_inactive_color, m_active_color: aggclr;
...
constructor rbox_ctrl_impl.Construct(x1, y1, x2, y2: double; flip_y_: boolean = False);
begin
...
{ BORDER_WIDTH_ }
procedure rbox_ctrl_impl.border_width_(t: double; extra: double = 0.0);
begin
  m_border_width := t;
...
{ TEXT_THICKNESS_ }
procedure rbox_ctrl_impl.text_thickness_(t: double);
begin
  m_text_thickness := t;
...
{ TEXT_SIZE_ }
procedure rbox_ctrl_impl.text_size_(h: double; w: double = 0.0);
begin
  m_text_width := w;
  m_text_height := h;
...
{ ADD_ITEM }
procedure rbox_ctrl_impl.add_item(Text: PChar);
begin
...
{ _CUR_ITEM }
function rbox_ctrl_impl._cur_item: int;
begin
  Result := m_cur_item;
...
{ CUR_ITEM_ }
procedure rbox_ctrl_impl.cur_item_(i: int);
begin
  m_cur_item := i;
...
{ IN_RECT }
function rbox_ctrl_impl.in_rect(x, y: double): boolean;
begin
...
function rbox_ctrl_impl.on_mouse_button_down(x, y: double): boolean;
var
  i: unsigned;
...
{ ON_MOUSE_BUTTON_UP }
function rbox_ctrl_impl.on_mouse_button_up(x, y: double): boolean;
begin
  Result := False;
...
{ ON_MOUSE_MOVE }
function rbox_ctrl_impl.on_mouse_move(x, y: double; button_flag: boolean): boolean;
begin
  Result := False;
...
{ ON_ARROW_KEYS }
function rbox_ctrl_impl.on_arrow_keys(left, right, down, up: boolean): boolean;
begin
...
{ NUM_PATHS }
function rbox_ctrl_impl.num_paths: unsigned;
begin
  Result := 5;
...
{ REWIND }
procedure rbox_ctrl_impl.rewind(path_id: unsigned);
begin
  m_idx := path_id;
...
{ VERTEX }
function rbox_ctrl_impl.vertex(x, y: double_ptr): unsigned;
var
  cmd: unsigned;
...
{ CONSTRUCT }
constructor rbox_ctrl.Construct(x1, y1, x2, y2: double; flip_y_: boolean = False);
begin
  inherited Construct(x1, y1, x2, y2, flip_y_);
...
{ BACKGROUND_COLOR_ }
procedure rbox_ctrl.background_color_(c: aggclr_ptr);
begin
...
{ BORDER_COLOR_ }
procedure rbox_ctrl.border_color_(c: aggclr_ptr);
begin
  m_border_color := c^;
...
{ TEXT_COLOR_ }
procedure rbox_ctrl.text_color_(c: aggclr_ptr);
begin
  m_text_color := c^;
...
{ INACTIVE_COLOR_ }
procedure rbox_ctrl.inactive_color_(c: aggclr_ptr);
begin
  m_inactive_color := c^;
...
{ ACTIVE_COLOR_ }
procedure rbox_ctrl.active_color_(c: aggclr_ptr);
begin
  m_active_color := c^;
...
{ _COLOR }
function rbox_ctrl._color(i: unsigned): aggclr_ptr;
begin
  Result := m_colors[i];
