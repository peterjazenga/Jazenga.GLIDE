...
type
  ...
  spline_ctrl = object(spline_ctrl_impl)
    m_background_color, m_border_color, m_curve_color, m_inactive_pnt_color, m_active_pnt_color: aggclr;
...
constructor spline_ctrl_impl.Construct(x1, y1, x2, y2: double; num_pnt: unsigned; flip_y: boolean = False);
var
  i: unsigned;
...
{ BORDER_WIDTH_ }
procedure spline_ctrl_impl.border_width_(t: double; extra: double = 0.0);
begin
  m_border_width := t;
...
{ CURVE_WIDTH_ }
procedure spline_ctrl_impl.curve_width_(t: double);
begin
  m_curve_width := t;
...
{ POINT_SIZE_ }
procedure spline_ctrl_impl.point_size_(s: double);
begin
  m_point_size := s;
...
{ IN_RECT }
function spline_ctrl_impl.in_rect(x, y: double): boolean;
begin
...
function spline_ctrl_impl.on_mouse_button_down(x, y: double): boolean;
var
  i: unsigned;
...
{ ON_MOUSE_BUTTON_UP }
function spline_ctrl_impl.on_mouse_button_up(x, y: double): boolean;
begin
...
{ ON_MOUSE_MOVE }
function spline_ctrl_impl.on_mouse_move(x, y: double; button_flag: boolean): boolean;
var
  xp, yp: double;
...
{ ON_ARROW_KEYS }
function spline_ctrl_impl.on_arrow_keys(left, right, down, up: boolean): boolean;
var
  kx, ky: double;
...
{ ACTIVE_POINT_ }
procedure spline_ctrl_impl.active_point_(i: int);
begin
  m_active_pnt := i;
...
{ _SPLINE }
function spline_ctrl_impl._spline: double_ptr;
begin
...
{ _SPLINE8 }
function spline_ctrl_impl._spline8: int8u_ptr;
begin
...
{ _VALUE }
function spline_ctrl_impl._value(x: double): double;
begin
  x := m_spline.get(x);
...
{ VALUE_ }
procedure spline_ctrl_impl.value_(idx: unsigned; y: double);
begin
...
{ POINT_ }
procedure spline_ctrl_impl.point_(idx: unsigned; x, y: double);
begin
  if idx < m_num_pnt then
  begin
    set_xp(idx, x);
...
{ X_ }
procedure spline_ctrl_impl.x_(idx: unsigned; x: double);
begin
  m_xp[idx] := x;
...
{ Y_ }
procedure spline_ctrl_impl.y_(idx: unsigned; y: double);
begin
  m_yp[idx] := y;
...
{ _X }
function spline_ctrl_impl._x(idx: unsigned): double;
begin
  Result := m_xp[idx];
...
{ _Y }
function spline_ctrl_impl._y(idx: unsigned): double;
begin
  Result := m_yp[idx];
...
{ NUM_PATHS }
function spline_ctrl_impl.num_paths: unsigned;
begin
  Result := 5;
...
{ REWIND }
procedure spline_ctrl_impl.rewind(path_id: unsigned);
var
  i: unsigned;
...
{ VERTEX }
function spline_ctrl_impl.vertex(x, y: double_ptr): unsigned;
var
  cmd: unsigned;
...
{ CALC_XP }
function spline_ctrl_impl.calc_xp(idx: unsigned): double;
begin
...
{ CALC_YP }
function spline_ctrl_impl.calc_yp(idx: unsigned): double;
begin
...
{ SET_XP }
procedure spline_ctrl_impl.set_xp(idx: unsigned; val: double);
begin
  if val < 0.0 then
    val := 0.0;
...
{ SET_YP }
procedure spline_ctrl_impl.set_yp(idx: unsigned; val: double);
begin
  if val < 0.0 then
    val := 0.0;
...
{ CONSTRUCT }
constructor spline_ctrl.Construct(x1, y1, x2, y2: double; num_pnt: unsigned; flip_y: boolean = False);
begin
...
{ BACKGROUND_COLOR_ }
procedure spline_ctrl.background_color_(c: aggclr_ptr);
begin
...
{ BORDER_COLOR_ }
procedure spline_ctrl.border_color_(c: aggclr_ptr);
begin
  m_border_color := c^;
...
{ CURVE_COLOR_ }
procedure spline_ctrl.curve_color_(c: aggclr_ptr);
begin
  m_curve_color := c^;
...
{ INACTIVE_PNT_COLOR_ }
procedure spline_ctrl.inactive_pnt_color_(c: aggclr_ptr);
begin
...
{ ACTIVE_PNT_COLOR_ }
procedure spline_ctrl.active_pnt_color_(c: aggclr_ptr);
begin
...
{ _COLOR }
function spline_ctrl._color(i: unsigned): aggclr_ptr;
begin
  Result := m_colors[i];
