...
type
  ...
  slider_ctrl_impl = object(ctrl)
    m_border_width, m_border_extra, m_text_thickness, m_value, m_preview_value, m_min, m_max: double;
    m_num_steps: unsigned;
    m_descending: boolean;
    m_label: array[0..63] of byte;
    m_xs1, m_ys1, m_xs2, m_ys2, m_pdx: double;
  ...
  slider_ctrl = object(slider_ctrl_impl)
    m_background_color, m_triangle_color, m_text_color, m_pointer_preview_color, m_pointer_color: aggclr;
...
constructor slider_ctrl_impl.Construct(x1, y1, x2, y2: double; flip_y: boolean = False);
begin
...
{ BORDER_WIDTH_ }
procedure slider_ctrl_impl.border_width_(t: double; extra: double = 0.0);
begin
  m_border_width := t;
...
{ RANGE_ }
procedure slider_ctrl_impl.range_(min, max: double);
begin
  m_min := min;
  m_max := max;
...
{ NUM_STEPS_ }
procedure slider_ctrl_impl.num_steps_(num: unsigned);
begin
  m_num_steps := num;
...
{ LABEL_ }
procedure slider_ctrl_impl.label_(fmt: PChar);
var
  len: unsigned;
...
{ TEXT_THICKNESS_ }
procedure slider_ctrl_impl.text_thickness_(t: double);
begin
  m_text_thickness := t;
...
{ _DESCENDING }
function slider_ctrl_impl._descending: boolean;
begin
  Result := m_descending;
...
{ DESCENDING_ }
procedure slider_ctrl_impl.descending_(v: boolean);
begin
  m_descending := v;
...
{ _VALUE }
function slider_ctrl_impl._value: double;
begin
...
{ VALUE_ }
procedure slider_ctrl_impl.value_(v: double);
begin
...
{ IN_RECT }
function slider_ctrl_impl.in_rect(x, y: double): boolean;
begin
...
function slider_ctrl_impl.on_mouse_button_down(x, y: double): boolean;
var
  xp, yp: double;
...
{ ON_MOUSE_BUTTON_UP }
function slider_ctrl_impl.on_mouse_button_up(x, y: double): boolean;
begin
  m_mouse_move := False;
...
{ ON_MOUSE_MOVE }
function slider_ctrl_impl.on_mouse_move(x, y: double; button_flag: boolean): boolean;
var
  xp: double;
...
{ ON_ARROW_KEYS }
function slider_ctrl_impl.on_arrow_keys(left, right, down, up: boolean): boolean;
var
  d: double;
...
{ NUM_PATHS }
function slider_ctrl_impl.num_paths: unsigned;
begin
  Result := 6;
...
{ REWIND }
procedure slider_ctrl_impl.rewind(path_id: unsigned);
var
  i: unsigned;
...
{ VERTEX }
function slider_ctrl_impl.vertex(x, y: double_ptr): unsigned;
var
  cmd: unsigned;
...
{ NORMALIZE_VALUE }
function slider_ctrl_impl.normalize_value(preview_value_flag: boolean): boolean;
var
  ret: boolean;
  step: int;
...
{ CONSTRUCT }
constructor slider_ctrl.Construct(x1, y1, x2, y2: double; flip_y: boolean = False);
begin
  inherited Construct(x1, y1, x2, y2, flip_y);
...
{ BACKGROUND_COLOR_ }
procedure slider_ctrl.background_color_(c: aggclr_ptr);
begin
...
{ POINTER_COLOR_ }
procedure slider_ctrl.pointer_color_(c: aggclr_ptr);
begin
  m_pointer_color := c^;
...
{ _COLOR }
function slider_ctrl._color(i: unsigned): aggclr_ptr;
begin
  Result := m_colors[i];
