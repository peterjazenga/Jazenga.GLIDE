...
type
  ...
  simple_polygon_vertex_source = object(vertex_source)
    ...
    m_num_points, m_vertex: unsigned;
    m_roundoff, m_close: boolean;
...
constructor simple_polygon_vertex_source.Construct(polygon: double_ptr; np: unsigned; roundoff: boolean = False; Close: boolean = True);
begin
  m_polygon := polygon;
  m_num_points := np;
  m_vertex := 0;
...
{ CLOSE_ }
procedure simple_polygon_vertex_source.close_(f: boolean);
begin
  m_close := f;
...
{ _CLOSE }
function simple_polygon_vertex_source._close: boolean;
begin
  Result := m_close;
...
{ REWIND }
procedure simple_polygon_vertex_source.rewind(path_id: unsigned);
begin
  m_vertex := 0;
...
{ VERTEX }
function simple_polygon_vertex_source.vertex(x, y: double_ptr): unsigned ;
begin
...
{ CONSTRUCT }
constructor polygon_ctrl_impl.Construct(np: unsigned; point_radius: double = 5);
begin
...
{ _NUM_POINTS }
function polygon_ctrl_impl._num_points: unsigned;
begin
  Result := m_num_points;
...
{ _XN }
function polygon_ctrl_impl._xn(n: unsigned): double;
begin
...
{ _YN }
function polygon_ctrl_impl._yn(n: unsigned): double;
begin
...
{ XN_PTR }
function polygon_ctrl_impl.xn_ptr(n: unsigned): double_ptr;
begin
  Result := double_ptr(ptrcomp(m_polygon) + (n * 2) * sizeof(double));
...
{ YN_PTR }
function polygon_ctrl_impl.yn_ptr(n: unsigned): double_ptr;
begin
  Result := double_ptr(ptrcomp(m_polygon) + (n * 2 + 1) * sizeof(double));
...
{ _POLYGON }
function polygon_ctrl_impl._polygon: double_ptr;
begin
  Result := m_polygon;
...
{ LINE_WIDTH_ }
procedure polygon_ctrl_impl.line_width_(w: double);
begin
  m_stroke.width_(w);
...
{ _LINE_WIDTH }
function polygon_ctrl_impl._line_width: double;
begin
...
{ POINT_RADIUS_ }
procedure polygon_ctrl_impl.point_radius_(r: double);
begin
  m_point_radius := r;
...
{ _POINT_RADIUS }
function polygon_ctrl_impl._point_radius: double;
begin
...
{ IN_POLYGON_CHECK_ }
procedure polygon_ctrl_impl.in_polygon_check_(f: boolean);
begin
...
{ _IN_POLYGON_CHECK }
function polygon_ctrl_impl._in_polygon_check: boolean;
begin
...
{ CLOSE_ }
procedure polygon_ctrl_impl.close_(f: boolean);
begin
  m_vs.close_(f);
...
{ _CLOSE }
function polygon_ctrl_impl._close: boolean;
begin
  Result := m_vs._close;
...
{ NUM_PATHS }
function polygon_ctrl_impl.num_paths: unsigned;
begin
  Result := 1;
...
{ REWIND }
procedure polygon_ctrl_impl.rewind(path_id: unsigned);
begin
  m_status := 0;
...
{ VERTEX }
function polygon_ctrl_impl.vertex(x, y: double_ptr): unsigned;
var
  cmd: unsigned;
  r: double;
...
begin
  ...
  begin
    ...
    if (m_node >= 0) and (m_node = m_status) then
      r := r * 1.2;
...
{ IN_RECT }
function polygon_ctrl_impl.in_rect(x, y: double): boolean;
begin
  Result := False;
...
function polygon_ctrl_impl.on_mouse_button_down(x, y: double): boolean;
var
  i: unsigned;
  ret: boolean;
...
{ ON_MOUSE_BUTTON_UP }
function polygon_ctrl_impl.on_mouse_button_up(x, y: double): boolean;
var
  ret: boolean;
...
{ ON_MOUSE_MOVE }
function polygon_ctrl_impl.on_mouse_move(x, y: double; button_flag: boolean): boolean;
var
  ret: boolean;
...
{ ON_ARROW_KEYS }
function polygon_ctrl_impl.on_arrow_keys(left, right, down, up: boolean): boolean;
begin
  Result := False;
...
{ CHECK_EDGE }
function polygon_ctrl_impl.check_edge(i: unsigned; x, y: double): boolean;
var
  ret: boolean;
...
begin
  ...
  begin
    ...
    if (u1 > 0.0) and (u1 < 1.0) and (Sqrt(dx * dx + dy * dy) <= m_point_radius) then
      ret := True;
...
function polygon_ctrl_impl.point_in_polygon(x, y: double): boolean;
var
  j, k: unsigned;
...
begin
  ...
  begin
    ...
    if yflag0 <> yflag1 then
      ...
      if int((vty1 - y) * (vtx0 - vtx1) >= (vtx1 - x) * (vty0 - vty1)) = yflag1 then
...
{ CONSTRUCT }
constructor polygon_ctrl.Construct(np: unsigned; point_radius: double = 5);
begin
...
{ LINE_COLOR_ }
procedure polygon_ctrl.line_color_(c: aggclr_ptr);
begin
  m_color := c^;
...
{ _COLOR }
function polygon_ctrl._color(i: unsigned): aggclr_ptr;
begin
  Result := @m_color;
