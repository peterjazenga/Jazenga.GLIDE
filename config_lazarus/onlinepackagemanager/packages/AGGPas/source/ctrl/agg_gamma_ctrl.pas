...
type
  ...
  gamma_ctrl_impl = object(ctrl)
    ...
    m_border_width, m_border_extra, m_curve_width, m_grid_width, m_text_thickness,
    m_point_size, m_text_height, m_text_width, m_xc1, m_yc1, m_xc2, m_yc2, m_xs1,
    m_ys1, m_xs2, m_ys2, m_xt1, m_yt1, m_xt2, m_yt2: double;
    ...
    m_xp1, m_yp1, m_xp2, m_yp2: double;
    m_p1_active: boolean;
  ...
  gamma_ctrl = object(gamma_ctrl_impl)
    m_background_color, m_border_color, m_curve_color, m_grid_color, m_inactive_pnt_color,
    m_active_pnt_color, m_text_color: aggclr;
...
constructor gamma_ctrl_impl.Construct(x1, y1, x2, y2: double; flip_y: boolean = False);
begin
...
{ BORDER_WIDTH_ }
procedure gamma_ctrl_impl.border_width_(t: double; extra: double = 0.0);
begin
  m_border_width := t;
...
{ CURVE_WIDTH_ }
procedure gamma_ctrl_impl.curve_width_(t: double);
begin
  m_curve_width := t;
...
{ GRID_WIDTH_ }
procedure gamma_ctrl_impl.grid_width_(t: double);
begin
  m_grid_width := t;
...
{ TEXT_THICKNESS_ }
procedure gamma_ctrl_impl.text_thickness_(t: double);
begin
  m_text_thickness := t;
...
{ TEXT_SIZE_ }
procedure gamma_ctrl_impl.text_size_(h: double; w: double = 0.0);
begin
  m_text_width := w;
  m_text_height := h;
...
{ POINT_SIZE_ }
procedure gamma_ctrl_impl.point_size_(s: double);
begin
  m_point_size := s;
...
{ IN_RECT }
function gamma_ctrl_impl.in_rect(x, y: double): boolean;
begin
...
function gamma_ctrl_impl.on_mouse_button_down(x, y: double): boolean;
begin
  inverse_transform_xy(@x, @y);
  calc_points;
...
{ ON_MOUSE_BUTTON_UP }
function gamma_ctrl_impl.on_mouse_button_up(x, y: double): boolean;
begin
...
{ ON_MOUSE_MOVE }
function gamma_ctrl_impl.on_mouse_move(x, y: double; button_flag: boolean): boolean;
begin
  inverse_transform_xy(@x, @y);
...
{ ON_ARROW_KEYS }
function gamma_ctrl_impl.on_arrow_keys(left, right, down, up: boolean): boolean;
var
...
{ GAMMA }
function gamma_ctrl_impl.gamma: char_ptr;
begin
...
{ _Y }
function gamma_ctrl_impl._y(x: double): double;
begin
...
{ FUNC_OPERATOR_GAMMA }
function gamma_ctrl_impl.func_operator_gamma(x: double): double;
begin
  Result := m_gamma_spline._y(x);
...
{ NUM_PATHS }
function gamma_ctrl_impl.num_paths: unsigned;
begin
  Result := 7;
...
{ REWIND }
procedure gamma_ctrl_impl.rewind(path_id: unsigned);
var
  kx1, ky1, kx2, ky2: double;
...
{ VERTEX }
function gamma_ctrl_impl.vertex(x, y: double_ptr): unsigned;
var
  cmd: unsigned;
...
begin
  ...
  case m_idx of
    ...
    1:
    begin
      if (m_vertex = 0) or (m_vertex = 4) or (m_vertex = 8) then
        cmd := path_cmd_move_to;
    ...
    3:
    begin
      if (m_vertex = 0) or (m_vertex = 4) or (m_vertex = 8) or (m_vertex = 14) then
        cmd := path_cmd_move_to;
...
{ CONSTRUCT }
constructor gamma_ctrl.Construct(x1, y1, x2, y2: double; flip_y: boolean = False);
begin
  inherited Construct(x1, y1, x2, y2, flip_y);
...
{ BACKGROUND_COLOR_ }
procedure gamma_ctrl.background_color_(c: aggclr_ptr);
begin
...
{ BORDER_COLOR_ }
procedure gamma_ctrl.border_color_(c: aggclr_ptr);
begin
  m_border_color := c^;
...
{ CURVE_COLOR_ }
procedure gamma_ctrl.curve_color_(c: aggclr_ptr);
begin
  m_curve_color := c^;
...
{ GRID_COLOR_ }
procedure gamma_ctrl.grid_color_(c: aggclr_ptr);
begin
  m_grid_color := c^;
...
{ INACTIVE_PNT_COLOR_ }
procedure gamma_ctrl.inactive_pnt_color_(c: aggclr_ptr);
begin
...
{ ACTIVE_PNT_COLOR_ }
procedure gamma_ctrl.active_pnt_color_(c: aggclr_ptr);
begin
...
{ TEXT_COLOR_ }
procedure gamma_ctrl.text_color_(c: aggclr_ptr);
begin
  m_text_color := c^;
...
{ _COLOR }
function gamma_ctrl._color(i: unsigned): aggclr_ptr;
begin
  Result := m_colors[i];
