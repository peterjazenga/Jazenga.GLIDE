...
begin
  ...
  begin
    if this^.m_base_length > 1e-10 then
      x^ :=
        x^ * (vertex_dist_ptr(this^.m_src_vertices.array_operator(this^.m_src_vertices.size - 1))^.dist / this^.m_base_length);
    ...
    begin
      ...
      dd :=
        vertex_dist_ptr(this^.m_src_vertices.array_operator(1))^.dist - vertex_dist_ptr(this^.m_src_vertices.array_operator(0))^.dist;
    ...
    begin
      ...
      dd :=
        vertex_dist_ptr(this^.m_src_vertices.array_operator(j))^.dist - vertex_dist_ptr(this^.m_src_vertices.array_operator(i))^.dist;
...
{ BASE_LENGTH_ }
procedure trans_single_path.base_length_(v: double);
begin
  m_base_length := v;
...
{ _BASE_LENGTH }
function trans_single_path._base_length: double;
begin
...
{ PRESERVE_X_SCALE_ }
procedure trans_single_path.preserve_x_scale_(f: boolean);
begin
...
{ _PRESERVE_X_SCALE }
function trans_single_path._preserve_x_scale: boolean ;
begin
...
{ MOVE_TO }
procedure trans_single_path.move_to(x, y: double);
var
  vd: vertex_dist;
...
{ LINE_TO }
procedure trans_single_path.line_to(x, y: double);
var
  vd: vertex_dist;
...
begin
  if (m_status = making_path) and (m_src_vertices.size > 1) then
  begin
    ...
    if m_src_vertices.size > 2 then
      ...
      begin
        d :=
          vertex_dist_ptr(m_src_vertices.array_operator(m_src_vertices.size - 3))^.dist + vertex_dist_ptr(
          m_src_vertices.array_operator(m_src_vertices.size - 2))^.dist;
...
{ ADD_PATH }
procedure trans_single_path.add_path(vs: vertex_source_ptr; path_id: unsigned = 0);
var
  x, y: double;
  cmd: unsigned;
...
{ TOTAL_LENGTH }
function trans_single_path.total_length: double;
begin
