...
type
  iterator_x23 = object
    den, den_step, nom_x, nom_x_step, nom_y, nom_y_step: double;
...
begin
  ...
  nom_x := double_ptr(ptrcomp(m) + 0 * sizeof(double))^ + double_ptr(ptrcomp(m) + 1 * sizeof(double))^ *
    tx + double_ptr(ptrcomp(m) + 2 * sizeof(double))^ * ty;
  nom_x_step := double_ptr(ptrcomp(m) + 1 * sizeof(double))^ * step;

  nom_y := double_ptr(ptrcomp(m) + 3 * sizeof(double))^ + double_ptr(ptrcomp(m) + 4 * sizeof(double))^ *
    tx + double_ptr(ptrcomp(m) + 5 * sizeof(double))^ * ty;
...
begin
  ...
  Pointer(transform) := @_transform23;
...
{ QUAD_TO_QUAD }
procedure trans_perspective23.quad_to_quad(src, dst: double_ptr);
var
  left: double_88;
  right: double_81;
...
{ RECT_TO_QUAD }
procedure trans_perspective23.rect_to_quad(x1, y1, x2, y2: double; quad: double_ptr);
var
  src: double_8;
...
{ QUAD_TO_RECT }
procedure trans_perspective23.quad_to_rect(quad: double_ptr; x1, y1, x2, y2: double);
var
  dst: double_8;
...
{ IS_VALID }
function trans_perspective23.is_valid: boolean;
begin
  Result := m_valid;
...
{ BEGIN_ }
function trans_perspective23.begin_(x, y, step: double): iterator_x23;
begin
...
begin
  ...
  Pointer(transform_2x2) := @_transform_2x2;
  Pointer(inverse_transform) := @_inverse_transform;
  Pointer(transform_affine) := @_transform_affine;
...
begin
  Result :=
    sx * (sy * w2 - ty * w1) + shx * (ty * w0 - shy * w2) + tx * (shy * w1 - sy * w0);
...
begin
  Result :=
    (Abs(sx) > epsilon) and (Abs(sy) > epsilon) and (Abs(w2) > epsilon);
...
begin
  Result :=
    is_equal_eps(sx, 1.0, epsilon) and is_equal_eps(shy, 0.0, epsilon) and is_equal_eps(w0, 0.0, epsilon) and
    is_equal_eps(shx, 0.0, epsilon) and is_equal_eps(sy, 1.0, epsilon) and is_equal_eps(w1, 0.0, epsilon) and
    is_equal_eps(tx, 0.0, epsilon) and is_equal_eps(ty, 0.0, epsilon) and is_equal_eps(w2, 1.0, epsilon);
...
begin
  Result :=
    is_equal_eps(sx, m^.sx, epsilon) and is_equal_eps(shy, m^.shy, epsilon) and is_equal_eps(w0, m^.w0, epsilon) and
    is_equal_eps(shx, m^.shx, epsilon) and is_equal_eps(sy, m^.sy, epsilon) and is_equal_eps(w1, m^.w1, epsilon) and
    is_equal_eps(tx, m^.tx, epsilon) and is_equal_eps(ty, m^.ty, epsilon) and is_equal_eps(w2, m^.w2, epsilon);
