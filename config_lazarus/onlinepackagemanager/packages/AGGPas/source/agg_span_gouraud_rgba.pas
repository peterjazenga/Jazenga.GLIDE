...
type
  ...
  rgba_calc = object
    ...
    m_r1, m_g1, m_b1, m_a1, m_dr, m_dg, m_db, m_da, m_r, m_g, m_b, m_a, m_x: int;
  ...
  span_gouraud_rgba = object(span_gouraud)
    ...
    m_rgba1, m_rgba2, m_rgba3: rgba_calc;
    ...
    // Agg 2.4 impl
    constructor Construct_(c1, c2, c3: aggclr_ptr; x1, y1, x2, y2, x3, y3: double; d: double = 0);
...
function rgba_calc.round(v: double): int;
begin
  if v < 0.0 then
    Result := trunc(v - 0.5)
  else
...
{ INIT }
procedure rgba_calc.init(c1, c2: coord_type_ptr);
var
  dy: double;
...
{ CALC }
procedure rgba_calc.calc(y: double);
var
  k: double;
...
{ PREPARE }
procedure span_gouraud_rgba.prepare(max_span_len: unsigned);
var
...
{ GENERATE }
function span_gouraud_rgba.generate(x, y: int; len: unsigned): aggclr_ptr;
const
...
{ CONSTRUCT_ }
constructor span_gouraud_rgba.Construct_(c1, c2, c3: aggclr_ptr; x1, y1, x2, y2, x3, y3: double;
  d: double = 0);
begin
...
begin
  ...
  m_swap :=
    cross_product(coord[0].x, coord[0].y, coord[2].x, coord[2].y, coord[1].x, coord[1].y) < 0.0;
