...
type
  ...
  rasterizer_outline_aa = object
    ...
    m_accurate_join, m_round_cap: boolean;
    ...
    procedure render_all_paths(vs: vertex_source_ptr; colors: aggclr_ptr;
...
{ CONSTRUCT }
constructor rasterizer_outline_aa.Construct(ren: renderer_outline_ptr);
begin
  m_src_vertices.Construct(sizeof(line_aa_vertex), 6, func_vertex_sequence(@line_aa_vertex_func_operator));
...
{ DRAW }
procedure rasterizer_outline_aa.draw(dv: draw_vars_ptr; start, end_: unsigned);
var
  i: unsigned;
...
begin
  ...
  begin
    ...
    begin
      ...
      dv^.flags :=
        dv^.flags or (unsigned(dv^.curr.diagonal_quadrant = dv^.Next.diagonal_quadrant) shl 1);
...
{ ACCURATE_JOIN_ }
procedure rasterizer_outline_aa.accurate_join_(v: boolean);
begin
...
{ _ACCURATE_JOIN }
function rasterizer_outline_aa._accurate_join: boolean;
begin
...
{ ROUND_CAP_ }
procedure rasterizer_outline_aa.round_cap_(v: boolean);
begin
  m_round_cap := v;
...
{ _ROUND_CAP }
function rasterizer_outline_aa._round_cap: boolean;
begin
  Result := m_round_cap;
...
{ MOVE_TO }
procedure rasterizer_outline_aa.move_to(x, y: int);
var
  vt: line_aa_vertex;
...
{ LINE_TO }
procedure rasterizer_outline_aa.line_to(x, y: int);
var
  vt: line_aa_vertex;
...
{ MOVE_TO_D }
procedure rasterizer_outline_aa.move_to_d(x, y: double);
begin
...
{ LINE_TO_D }
procedure rasterizer_outline_aa.line_to_d(x, y: double);
begin
...
{ RENDER }
procedure rasterizer_outline_aa.render(close_polygon: boolean);
var
  dv: draw_vars;
  v: line_aa_vertex_ptr;
  ...
  if close_polygon then
    ...
    begin
      ...
      if m_accurate_join then
        ...
        dv.flags :=
          unsigned(prev.diagonal_quadrant = dv.curr.diagonal_quadrant) or (unsigned(dv.curr.diagonal_quadrant =
...
{ ADD_VERTEX }
procedure rasterizer_outline_aa.add_vertex(x, y: double; cmd: unsigned);
begin
...
{ ADD_PATH }
procedure rasterizer_outline_aa.add_path(vs: vertex_source_ptr; path_id: unsigned = 0);
var
  x, y: double;
  cmd: unsigned;
...
{ RENDER_ALL_PATHS }
procedure rasterizer_outline_aa.render_all_paths(vs: vertex_source_ptr; colors: aggclr_ptr;
      path_id: unsigned_ptr; num_paths: unsigned);
var
  i: unsigned;
...
{ RENDER_CTRL }
procedure rasterizer_outline_aa.render_ctrl(c: ctrl_ptr);
var
  i: unsigned;
