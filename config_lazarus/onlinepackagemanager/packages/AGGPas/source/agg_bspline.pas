...
{ ADD_POINT }
procedure bspline.add_point(x, y: double);
begin
  if m_num < m_max then
  begin
  ...
  begin
    ...
    begin
      ...
      double_ptr(ptrcomp(s) + k * sizeof(double))^ :=
        (double_ptr(ptrcomp(s) + k * sizeof(double))^ - double_ptr(ptrcomp(r) + k * sizeof(double))^ *
    ...
    begin
      double_ptr(ptrcomp(al) + k * sizeof(double))^ :=
        double_ptr(ptrcomp(al) + k * sizeof(double))^ * double_ptr(ptrcomp(al) + (k + 1) * sizeof(double))^
        + double_ptr(ptrcomp(s) + k * sizeof(double))^;
...
{ GET }
function bspline.get(x: double): double;
var
  i: int;
...
{ GET_STATEFUL }
function bspline.get_stateful(x: double): double;
begin
  if m_num > 2 then
  begin
    // Extrapolation on the left
    ...
    begin
      ...
      if (x < double_ptr(ptrcomp(m_x) + m_last_idx * sizeof(double))^) or
        ...
        if (m_last_idx < m_num - 2) and (x >= double_ptr(ptrcomp(m_x) + (m_last_idx + 1) * sizeof(double))^) and
          (x <= double_ptr(ptrcomp(m_x) + (m_last_idx + 2) * sizeof(double))^) then
          Inc(m_last_idx)
        else
        if (m_last_idx > 0) and (x >= double_ptr(ptrcomp(m_x) + (m_last_idx - 1) * sizeof(double))^) and
...
{ BSEARCH }
procedure bspline.bsearch(n: int; x: double_ptr; x0: double; i: int_ptr);
var
  j, k: int;
...
{ EXTRAPOLATION_LEFT }
function bspline.extrapolation_left(x: double): double;
var
  d: double;
...
begin
  ...
  Result :=
    (-d * double_ptr(ptrcomp(m_am) + sizeof(double))^ / 6 + (double_ptr(ptrcomp(m_y) + sizeof(double))^ - m_y^) / d) * (x - m_x^) + m_y^;

end;

{ EXTRAPOLATION_RIGHT }
function bspline.extrapolation_right(x: double): double;
var
  d: double;

begin
  d :=
    double_ptr(ptrcomp(m_x) + (m_num - 1) * sizeof(double))^ - double_ptr(ptrcomp(m_x) + (m_num - 2) * sizeof(double))^;

  Result :=
    (d * double_ptr(ptrcomp(m_am) + (m_num - 2) * sizeof(double))^ / 6 + (double_ptr(ptrcomp(m_y) + (m_num - 1) * sizeof(double))^
    - double_ptr(ptrcomp(m_y) + (m_num - 2) * sizeof(double))^) / d) * (x - double_ptr(ptrcomp(m_x) + (m_num - 1) * sizeof(double))^) +
    double_ptr(ptrcomp(m_y) + (m_num - 1) * sizeof(double))^;

end;

{ INTERPOLATION }
function bspline.interpolation(x: double; i: int): double;
var
  j: int;
...
begin
  ...
  Result :=
    (double_ptr(ptrcomp(m_am) + j * sizeof(double))^ * r * r * r + double_ptr(ptrcomp(m_am) + i * sizeof(double))^ * h * h * h) /
    6.0 / d + ((double_ptr(ptrcomp(m_y) + j * sizeof(double))^ - double_ptr(ptrcomp(m_am) + j * sizeof(double))^ * p) *
    r + (double_ptr(ptrcomp(m_y) + i * sizeof(double))^ - double_ptr(ptrcomp(m_am) + i * sizeof(double))^ * p) * h) / d;
