...
type
  ...
  scanline_u8 = object(scanline)
    ...
    m_covers: int8u_ptr;
    m_spans, m_cur_span: span_u8_ptr;
...
{ RESET }
procedure scanline_u8.reset(min_x, max_x: int);
var
  max_len: unsigned;
...
{ FINALIZE }
procedure scanline_u8.finalize(y_: int);
begin
  m_y := y_;
...
{ ADD_CELL }
procedure scanline_u8.add_cell(x: int; cover: unsigned);
begin
  Dec(x, m_min_x);
...
{ ADD_CELLS }
procedure scanline_u8.add_cells(x: int; len: unsigned; covers: int8u_ptr);
begin
  Dec(x, m_min_x);
  move(
    covers^,
...
{ ADD_SPAN }
procedure scanline_u8.add_span(x: int; len, cover: unsigned);
begin
  Dec(x, m_min_x);
...
{ NUM_SPANS }
function scanline_u8.num_spans: unsigned;
begin
...
{ BEGIN_ }
function scanline_u8.begin_: pointer;
begin
...
{ SZ_OF_SPAN }
function scanline_u8.sz_of_span: unsigned;
begin
...
{ FINALIZE }
procedure scanline_u8_am.finalize(y_: int);
var
  span: span_u8_ptr;
