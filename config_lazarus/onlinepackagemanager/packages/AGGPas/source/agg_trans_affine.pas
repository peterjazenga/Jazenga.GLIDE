...
type
  ...
  trans_affine = object
    ...
    transform, transform_2x2, inverse_transform: proc_transform;
...
procedure trans_affine.parl_to_parl(src, dst: parallelo_ptr);
var
  m: trans_affine;
...
procedure trans_affine.rect_to_parl(x1, y1, x2, y2: double; parl: parallelo_ptr);
var
  src: parallelogram;
begin
  src[0] := x1;
...
procedure trans_affine.parl_to_rect(parl: parallelo_ptr; x1, y1, x2, y2: double);
var
  dst: parallelogram;
...
procedure trans_affine.multiply(m: trans_affine_ptr);
var
  t0, t2, t4: double;
...
procedure trans_affine.premultiply(m: trans_affine_ptr);
var
  t: trans_affine;
...
procedure trans_affine.multiply_inv(m: trans_affine_ptr);
var
  t: trans_affine;
...
procedure trans_affine.premultiply_inv(m: trans_affine_ptr);
var
  t: trans_affine;
...
procedure trans_affine.store_to(m: parallelo_ptr);
begin
  m^[0] := m0;
  m^[1] := m1;
  m^[2] := m2;
...
procedure trans_affine.load_from(m: parallelo_ptr);
begin
  m0 := m^[0];
  m1 := m^[1];
  m2 := m^[2];
...
function trans_affine.determinant: double;
begin
  try
...
function trans_affine.is_identity(epsilon: double = affine_epsilon): boolean;
begin
  Result :=
    is_equal_eps(m0, 1, epsilon) and is_equal_eps(m1, 0, epsilon) and is_equal_eps(m2, 0, epsilon) and
    is_equal_eps(m3, 1, epsilon) and is_equal_eps(m4, 0, epsilon) and is_equal_eps(m5, 0, epsilon);
end;

function trans_affine.is_equal(m: trans_affine; epsilon: double = affine_epsilon): boolean;
begin
  Result :=
    is_equal_eps(m0, m.m0, epsilon) and is_equal_eps(m1, m.m1, epsilon) and is_equal_eps(m2, m.m2, epsilon) and
    is_equal_eps(m3, m.m3, epsilon) and is_equal_eps(m4, m.m4, epsilon) and is_equal_eps(m5, m.m5, epsilon);

end;

{ rotation }
function trans_affine.rotation: double;
var
  x1, y1, x2, y2: double;
...
procedure trans_affine.translation(dx, dy: double_ptr);
begin
  dx^ := 0;
  dy^ := 0;
...
procedure trans_affine.scaling(sx, sy: double_ptr);
var
...
{ scaling_abs }
procedure trans_affine.scaling_abs(sx, sy: double_ptr);
begin
  sx^ := Sqrt(m0 * m0 + m2 * m2);
  sy^ := Sqrt(m1 * m1 + m3 * m3);
...
{ ASSIGN }
procedure trans_affine.Assign(from: trans_affine_ptr);
begin
  m0 := from^.m0;
  m1 := from^.m1;
  m2 := from^.m2;
...
{ ASSIGN_ALL }
procedure trans_affine.assign_all(from: trans_affine_ptr);
begin
  m0 := from^.m0;
  m1 := from^.m1;
  m2 := from^.m2;
...
constructor trans_affine_rotation.Construct(a: double);
begin
...
constructor trans_affine_translation.Construct(tx, ty: double);
begin
  inherited Construct(1, 0, 0, 1, tx, ty);
end;

constructor trans_affine_skewing.Construct(sx, sy: double);
begin
  inherited Construct(1, Tan(sy), Tan(sx), 1, 0, 0);
end;

constructor trans_affine_line_segment.Construct(x1, y1, x2, y2, dist: double);
var
  dx, dy: double;
