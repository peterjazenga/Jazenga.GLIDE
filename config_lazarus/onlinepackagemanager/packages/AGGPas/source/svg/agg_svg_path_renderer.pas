...
type
  ...
  path_attributes = object
    index: unsigned;
    fill_color, stroke_color: aggclr;
    fill_flag, stroke_flag, even_odd_flag: boolean;
    line_join, line_cap: unsigned;
    miter_limit, stroke_width: double;
  ...
  path_renderer = object(unsigned_list)
    ...
    m_attr_storage, m_attr_stack: pod_deque;
    ...
    procedure render(ras: rasterizer_scanline_ptr; sl: scanline_ptr;
      ren: renderer_scanline_ptr; mtx: trans_affine_ptr; cb: rect_ptr; opacity: double = 1.0);
    // Private
...
constructor conv_count.Construct(vs: vertex_source_ptr);
begin
  m_source := vs;
  m_count := 0;
...
{ COUNT_ }
procedure conv_count.count_(n: unsigned);
begin
  m_count := n;
...
{ _COUNT }
function conv_count._count: unsigned;
begin
  Result := m_count;
...
{ REWIND }
procedure conv_count.rewind(path_id: unsigned);
begin
...
{ VERTEX }
function conv_count.vertex(x, y: double_ptr): unsigned;
begin
  Inc(m_count);
...
{ PARSE_PATH }
procedure path_renderer.parse_path(tok: path_tokenizer_ptr);
var
...
{ MOVE_TO }
procedure path_renderer.move_to(x, y: double; rel: boolean = False);
begin
  if rel then
...
{ LINE_TO }
procedure path_renderer.line_to(x, y: double; rel: boolean = False);
begin
  if rel then
    m_storage.rel_to_abs(@x, @y);
...
{ HLINE_TO }
procedure path_renderer.hline_to(x: double; rel: boolean = False);
var
  x2, y2: double;
...
{ VLINE_TO }
procedure path_renderer.vline_to(y: double; rel: boolean = False);
var
  x2, y2: double;
...
{ ADD_PATH }
procedure path_renderer.add_path(vs: vertex_source_ptr; path_id: unsigned = 0; solid_path: boolean = True);
begin
...
{ VERTEX_COUNT }
function path_renderer.vertex_count: unsigned;
begin
...
{ FILL }
procedure path_renderer.fill(f: aggclr_ptr);
var
...
{ STROKE }
procedure path_renderer.stroke(s: aggclr_ptr);
var
  attr: path_attributes_ptr;
...
{ EVEN_ODD }
procedure path_renderer.even_odd(flag: boolean);
begin
...
{ STROKE_WIDTH }
procedure path_renderer.stroke_width(w: double);
var
  attr: path_attributes_ptr;
...
{ FILL_OPACITY }
procedure path_renderer.fill_opacity(op: double);
begin
...
{ STROKE_OPACITY }
procedure path_renderer.stroke_opacity(op: double);
begin
...
{ LINE_JOIN }
procedure path_renderer.line_join(join: unsigned);
begin
...
{ LINE_CAP }
procedure path_renderer.line_cap(cap: unsigned);
begin
...
{ MITER_LIMIT }
procedure path_renderer.miter_limit(ml: double);
begin
...
{ TRANSFORM }
function path_renderer.transform: trans_affine_ptr;
begin
...
{ EXPAND }
procedure path_renderer.expand(Value: double);
begin
...
{ ARRAY_OPERATOR }
function path_renderer.array_operator(idx: unsigned): unsigned;
begin
  m_transform.assign_all(@path_attributes_ptr(m_attr_storage.array_operator(idx))^.transform);
...
{ BOUNDING_RECT }
procedure path_renderer.bounding_rect(x1, y1, x2, y2: double_ptr);
var
...
begin
  ...
  bounding_rect_ul(@trans, @self, 0, m_attr_storage.size, x1, y1, x2, y2);

end;

{ RENDER }
procedure path_renderer.render(ras: rasterizer_scanline_ptr; sl: scanline_ptr;
                               ren: renderer_scanline_ptr; mtx: trans_affine_ptr;
                               cb: rect_ptr; opacity: double = 1.0);
var
  i: unsigned;
...
{ CUR_ATTR }
function path_renderer.cur_attr: path_attributes_ptr;
begin
  if m_attr_stack.size = 0 then
    raise svg_exception.Construct(PChar('cur_attr : Attribute stack is empty'));

  Result :=
    path_attributes_ptr(m_attr_stack.array_operator(m_attr_stack.size - 1));
