...
{ CONSTRUCT }
constructor span_pattern_resample_rgba_affine.Construct(alloc: span_allocator_ptr;
  src: rendering_buffer_ptr; interpolator: span_interpolator_ptr; filter: image_filter_lut_ptr;
  WX, WY: wrap_mode_ptr; order: order_type);
var
  rgba: aggclr;
...
{ SOURCE_IMAGE_ }
procedure span_pattern_resample_rgba_affine.source_image_(src: rendering_buffer_ptr);
begin
...
{ GENERATE }
function span_pattern_resample_rgba_affine.generate(x, y: int; len: unsigned): aggclr_ptr;
var
  span: aggclr_ptr;
...
{ CONSTRUCT }
constructor span_pattern_resample_rgba.Construct(alloc: span_allocator_ptr; src: rendering_buffer_ptr;
...
{ SOURCE_IMAGE_ }
procedure span_pattern_resample_rgba.source_image_(src: rendering_buffer_ptr);
begin
  inherited source_image_(src);
...
{ GENERATE }
function span_pattern_resample_rgba.generate(x, y: int; len: unsigned): aggclr_ptr;
var
  span: aggclr_ptr;
  intr: span_interpolator_ptr;
  ...
  diameter, filter_size, rx, ry, rx_inv, ry_inv, radius_x, radius_y, maxx, maxy, y_lr, y_hr, total_weight, x_lr_ini
  , x_hr_ini, weight_y, x_lr, x_hr, weight: int;
