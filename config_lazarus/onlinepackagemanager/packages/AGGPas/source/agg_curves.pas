...
type
  ...
  curve3_inc = object(curve)
    m_num_steps, m_step: int;
    m_scale, m_start_x, m_start_y, m_end_x, m_end_y, m_fx, m_fy, m_dfx, m_dfy,
    m_ddfx, m_ddfy, m_saved_fx, m_saved_fy, m_saved_dfx, m_saved_dfy: double;
  ...
  curve3_div = object(curve)
    m_approximation_scale, m_distance_tolerance_square, m_distance_tolerance_manhattan, m_angle_tolerance: double;
    m_count: unsigned;
    m_points: pod_deque;
    constructor Construct; overload;
    constructor Construct(x1, y1, x2, y2, x3, y3: double); overload;
  ...
  curve4_inc = object(curve)
    m_num_steps, m_step: int;
    m_scale, m_start_x, m_start_y, m_end_x, m_end_y, m_fx, m_fy, m_dfx, m_dfy,
    m_ddfx, m_ddfy, m_dddfx, m_dddfy, m_saved_fx, m_saved_fy, m_saved_dfx, m_saved_dfy,
    m_saved_ddfx, m_saved_ddfy: double;
    constructor Construct; overload;
    constructor Construct(x1, y1, x2, y2, x3, y3, x4, y4: double); overload;
  ...
  curve4_div = object(curve)
    m_approximation_scale, m_distance_tolerance_square, m_distance_tolerance_manhattan,
    m_angle_tolerance, m_cusp_limit: double;
    m_count: unsigned;
    m_points: pod_deque;
    constructor Construct; overload;
    constructor Construct(x1, y1, x2, y2, x3, y3, x4, y4: double); overload;
...
begin
  Result :=
    catrom_to_bezier(cp^.array_operator(0), cp^.array_operator(1), cp^.array_operator(2), cp^.array_operator(3),
    cp^.array_operator(4), cp^.array_operator(5), cp^.array_operator(6), cp^.array_operator(7));
...
begin
  Result :=
    ubspline_to_bezier(cp^.array_operator(0), cp^.array_operator(1), cp^.array_operator(2), cp^.array_operator(3)
    , cp^.array_operator(4), cp^.array_operator(5), cp^.array_operator(6), cp^.array_operator(7));
...
begin
  Result :=
    hermite_to_bezier(cp^.array_operator(0), cp^.array_operator(1), cp^.array_operator(2), cp^.array_operator(3)
    , cp^.array_operator(4), cp^.array_operator(5), cp^.array_operator(6), cp^.array_operator(7));

end;

{ INIT3 }
procedure curve.init3(x1, y1, x2, y2, x3, y3: double);
begin
end;

{ INIT4 }
procedure curve.init4(x1, y1, x2, y2, x3, y3, x4, y4: double);
begin
end;
...
{ INIT3 }
procedure curve3_inc.init3(x1, y1, x2, y2, x3, y3: double);
var
  dx1, dy1, dx2, dy2, len, tmpx, tmpy, subdivide_step, subdivide_step2: double;
...
procedure curve3_inc.approximation_method_(v: curve_approximation_method_e);
begin
end;

{ _APPROXIMATION_METHOD }
function curve3_inc._approximation_method: curve_approximation_method_e;
begin
  Result := curve_inc;
...
procedure curve3_inc.approximation_scale_(s: double);
begin
  m_scale := s;
...
function curve3_inc._approximation_scale: double;
begin
  Result := m_scale;
...
{ ANGLE_TOLERANCE_ }
procedure curve3_inc.angle_tolerance_(a: double);
begin
end;

{ _ANGLE_TOLERANCE }
function curve3_inc._angle_tolerance: double;
begin
  Result := 0.0;
...
{ CUSP_LIMIT_ }
procedure curve3_inc.cusp_limit_(v: double);
begin
end;

{ _CUSP_LIMIT }
function curve3_inc._cusp_limit: double;
begin
  Result := 0.0;
...
{ REWIND }
procedure curve3_inc.rewind(path_id: unsigned);
begin
...
{ VERTEX }
function curve3_inc.vertex(x, y: double_ptr): unsigned;
begin
  if m_step < 0 then
  begin
...
{ INIT3 }
procedure curve3_div.init3(x1, y1, x2, y2, x3, y3: double);
begin
  m_points.remove_all;
...
{ APPROXIMATION_METHOD_ }
procedure curve3_div.approximation_method_(v: curve_approximation_method_e);
begin
end;

{ _APPROXIMATION_METHOD }
function curve3_div._approximation_method: curve_approximation_method_e;
begin
  Result := curve_div;
...
{ APPROXIMATION_SCALE_ }
procedure curve3_div.approximation_scale_(s: double);
begin
  if s = 0 then
...
{ _APPROXIMATION_SCALE }
function curve3_div._approximation_scale: double;
begin
...
{ ANGLE_TOLERANCE_ }
procedure curve3_div.angle_tolerance_(a: double);
begin
  m_angle_tolerance := a;
...
{ _ANGLE_TOLERANCE }
function curve3_div._angle_tolerance: double;
begin
...
{ CUSP_LIMIT_ }
procedure curve3_div.cusp_limit_(v: double);
begin
end;

{ _CUSP_LIMIT }
function curve3_div._cusp_limit: double;
begin
  Result := 0.0;
...
{ REWIND }
procedure curve3_div.rewind(path_id: unsigned);
begin
  m_count := 0;
...
{ VERTEX }
function curve3_div.vertex(x, y: double_ptr): unsigned;
var
  p: point_type_ptr;
...
{ BEZIER }
procedure curve3_div.bezier(x1, y1, x2, y2, x3, y3: double);
var
  pt: point_type;
...
{ RECURSIVE_BEZIER }
procedure curve3_div.recursive_bezier(x1, y1, x2, y2, x3, y3: double; level: unsigned);
var
...
{ INIT }
procedure curve4_points.init(x1, y1, x2, y2, x3, y3, x4, y4: double);
begin
  cp[0] := x1;
  cp[1] := y1;
  cp[2] := x2;
...
{ ARRAY_OPERATOR }
function curve4_points.array_operator(i: unsigned): double;
begin
  Result := cp[i];
...
{ ARRAY_OPERATOR_PTR }
function curve4_points.array_operator_ptr(i: unsigned): double_ptr;
begin
  Result := @cp[i];
...
{ INIT4 }
procedure curve4_inc.init4(x1, y1, x2, y2, x3, y3, x4, y4: double);
var
  dx1, dy1, dx2, dy2, dx3, dy3, len, subdivide_step, subdivide_step2, subdivide_step3, pre1, pre2, pre4, pre5
  , tmp1x, tmp1y, tmp2x, tmp2y: double;
...
{ INIT }
procedure curve4_inc.init(cp: curve4_points_ptr);
begin
  init4(
    cp^.array_operator(0),
    cp^.array_operator(1),
...
{ APPROXIMATION_METHOD_ }
procedure curve4_inc.approximation_method_(v: curve_approximation_method_e);
begin
end;

{ _APPROXIMATION_METHOD }
function curve4_inc._approximation_method: curve_approximation_method_e;
begin
  Result := curve_inc;
...
{ APPROXIMATION_SCALE_ }
procedure curve4_inc.approximation_scale_(s: double);
begin
  m_scale := s;
...
{ _APPROXIMATION_SCALE }
function curve4_inc._approximation_scale: double;
begin
  Result := m_scale;
...
{ ANGLE_TOLERANCE_ }
procedure curve4_inc.angle_tolerance_(a: double);
begin
end;

{ _ANGLE_TOLERANCE }
function curve4_inc._angle_tolerance: double;
begin
  Result := 0.0;
...
{ CUSP_LIMIT_ }
procedure curve4_inc.cusp_limit_(v: double);
begin
end;

{ _CUSP_LIMIT }
function curve4_inc._cusp_limit: double;
begin
  Result := 0.0;
...
{ REWIND }
procedure curve4_inc.rewind(path_id: unsigned);
begin
  if m_num_steps = 0 then
  begin
    m_step := -1;
...
{ VERTEX }
function curve4_inc.vertex(x, y: double_ptr): unsigned;
begin
  if m_step < 0 then
  begin
    Result := path_cmd_stop;
...
{ INIT4 }
procedure curve4_div.init4(x1, y1, x2, y2, x3, y3, x4, y4: double);
begin
  m_points.remove_all;
...
{ INIT }
procedure curve4_div.init(cp: curve4_points_ptr);
begin
  init4(
    cp^.array_operator(0),
    cp^.array_operator(1),
...
{ APPROXIMATION_METHOD_ }
procedure curve4_div.approximation_method_(v: curve_approximation_method_e);
begin
end;

{ _APPROXIMATION_METHOD }
function curve4_div._approximation_method: curve_approximation_method_e;
begin
  Result := curve_div;
...
{ APPROXIMATION_SCALE_ }
procedure curve4_div.approximation_scale_(s: double);
begin
  if s = 0 then
    m_approximation_scale := 0.00001
  else
...
{ _APPROXIMATION_SCALE }
function curve4_div._approximation_scale: double;
begin
  Result := m_approximation_scale;
...
{ ANGLE_TOLERANCE_ }
procedure curve4_div.angle_tolerance_(a: double);
begin
  m_angle_tolerance := a;
...
{ _ANGLE_TOLERANCE }
function curve4_div._angle_tolerance: double;
begin
  Result := m_angle_tolerance;
...
{ CUSP_LIMIT_ }
procedure curve4_div.cusp_limit_(v: double);
begin
  if v = 0.0 then
    m_cusp_limit := 0.0
  else
...
{ _CUSP_LIMIT }
function curve4_div._cusp_limit: double;
begin
...
{ REWIND }
procedure curve4_div.rewind(path_id: unsigned);
begin
  m_count := 0;
...
{ VERTEX }
function curve4_div.vertex(x, y: double_ptr): unsigned;
var
  p: point_type_ptr;
...
{ BEZIER }
procedure curve4_div.bezier(x1, y1, x2, y2, x3, y3, x4, y4: double);
var
  pt: point_type;
...
{ RECURSIVE_BEZIER }
procedure curve4_div.recursive_bezier(x1, y1, x2, y2, x3, y3, x4, y4: double; level: unsigned);
var
  x12, y12, x23, y23, x34, y34, x123, y123, x234, y234, x1234, y1234, dx, dy, d2, d3, da1, da2, a23: double;
...
begin
  ...
  case ((int(d2 > curve_collinearity_epsilon) shl 1) + int(d3 > curve_collinearity_epsilon)) of
    // All collinear OR p1==p4
    0:
      if Abs(x1 + x3 - x2 - x2) + Abs(y1 + y3 - y2 - y2) + Abs(x2 + x4 - x3 - x3) + Abs(y2 + y4 - y3 - y3) <=
        m_distance_tolerance_manhattan then
      begin
        pt.x := x1234;
        pt.y := y1234;
...
{ INIT3 }
procedure curve3.init3(x1, y1, x2, y2, x3, y3: double);
begin
...
{ APPROXIMATION_METHOD_ }
procedure curve3.approximation_method_(v: curve_approximation_method_e);
begin
...
{ _APPROXIMATION_METHOD }
function curve3._approximation_method: curve_approximation_method_e;
begin
...
{ APPROXIMATION_SCALE_ }
procedure curve3.approximation_scale_(s: double);
begin
...
{ _APPROXIMATION_SCALE }
function curve3._approximation_scale: double;
begin
...
{ ANGLE_TOLERANCE_ }
procedure curve3.angle_tolerance_(a: double);
begin
...
{ _ANGLE_TOLERANCE }
function curve3._angle_tolerance: double;
begin
...
{ CUSP_LIMIT_ }
procedure curve3.cusp_limit_(v: double);
begin
...
{ _CUSP_LIMIT }
function curve3._cusp_limit: double;
begin
...
{ REWIND }
procedure curve3.rewind(path_id: unsigned);
begin
  if m_approximation_method = curve_inc then
...
{ VERTEX }
function curve3.vertex(x, y: double_ptr): unsigned;
begin
  if m_approximation_method = curve_inc then
...
{ INIT4 }
procedure curve4.init4(x1, y1, x2, y2, x3, y3, x4, y4: double);
begin
  if m_approximation_method = curve_inc then
...
{ INIT }
procedure curve4.init(cp: curve4_points_ptr);
begin
  init4(
    cp^.array_operator(0),
    cp^.array_operator(1),
...
{ APPROXIMATION_METHOD_ }
procedure curve4.approximation_method_(v: curve_approximation_method_e);
begin
  m_approximation_method := v;
...
{ _APPROXIMATION_METHOD }
function curve4._approximation_method: curve_approximation_method_e;
begin
  Result := m_approximation_method;
...
{ APPROXIMATION_SCALE_ }
procedure curve4.approximation_scale_(s: double);
begin
  m_curve_inc.approximation_scale_(s);
  m_curve_div.approximation_scale_(s);
...
{ _APPROXIMATION_SCALE }
function curve4._approximation_scale: double;
begin
  Result := m_curve_inc._approximation_scale;

end;

procedure curve4.angle_tolerance_(a: double);
begin
  m_curve_div.angle_tolerance_(a);

end;

function curve4._angle_tolerance: double;
begin
  Result := m_curve_div._angle_tolerance;

end;

procedure curve4.cusp_limit_(v: double);
begin
  m_curve_div.cusp_limit_(v);

end;

function curve4._cusp_limit: double;
begin
  Result := m_curve_div._cusp_limit;

end;

procedure curve4.rewind(path_id: unsigned);
begin
  if m_approximation_method = curve_inc then
    m_curve_inc.rewind(path_id)
  else
...
function curve4.vertex(x, y: double_ptr): unsigned;
begin
  if m_approximation_method = curve_inc then
    Result := m_curve_inc.vertex(x, y)
  else
