...
type
  ...
  renderer_base = object
    ...
    procedure blend_from_color(src: pixel_formats_ptr; color: aggclr_ptr;
      rect_src_ptr: rect_ptr = nil; dx: int = 0; dy: int = 0; cover: int8u = cover_full);

    procedure blend_from_lut(src: pixel_formats_ptr; color_lut: aggclr_ptr;
      rect_src_ptr: rect_ptr = nil; dx: int = 0; dy: int = 0; cover: int8u = cover_full);
...
begin
  ...
  if (ren_ <> nil) and (ren_^._width > 0) then
    w := ren_^._width - 1
  else
    w := 0;

  if (ren_ <> nil) and (ren_^._height > 0) then
    h := ren_^._height - 1
  else
    h := 0;
...
begin
  Result :=
    (x >= m_clip_box.x1) and (y >= m_clip_box.y1) and (x <= m_clip_box.x2) and (y <= m_clip_box.y2);
...
{ BLEND_SOLID_HSPAN }
procedure renderer_base.blend_solid_hspan(x, y, len: int; c: aggclr_ptr; covers: int8u_ptr);
begin
  if y > _ymax then
    exit;
...
{ BLEND_SOLID_VSPAN }
procedure renderer_base.blend_solid_vspan(x, y, len: int; c: aggclr_ptr; covers: int8u_ptr);
begin
  if x > _xmax then
    exit;
...
{ BLEND_COLOR_HSPAN }
procedure renderer_base.blend_color_hspan(x, y, len: int; colors: aggclr_ptr; covers: int8u_ptr; cover: int8u);
var
  d: int;
...
{ BLEND_COLOR_VSPAN }
procedure renderer_base.blend_color_vspan(x, y, len: int; colors: aggclr_ptr; covers: int8u_ptr; cover: int8u);
var
  d: int;
...
procedure renderer_base.blend_color_hspan_no_clip(x, y, len: int; colors: aggclr_ptr; covers: int8u_ptr; cover: int8u);
begin
  m_ren^.blend_color_hspan(m_ren, x, y, len, colors, covers, cover);
...
procedure renderer_base.blend_color_vspan_no_clip(x, y, len: int; colors: aggclr_ptr; covers: int8u_ptr; cover: int8u);
begin
  m_ren^.blend_color_vspan(m_ren, x, y, len, colors, covers, cover);
...
{ COPY_FROM }
procedure renderer_base.copy_from(src: rendering_buffer_ptr; rect_src_ptr: rect_ptr; dx: int; dy: int);
var
  rsrc, rdst, rc: rect;
...
{ BLEND_FROM }
procedure renderer_base.blend_from(src: pixel_formats_ptr; rect_src_ptr: rect_ptr; dx: int; dy: int; cover: int8u);
var
  rsrc, rdst, rc: rect;
...
{ BLEND_FROM_COLOR }
procedure renderer_base.blend_from_color(src: pixel_formats_ptr; color: aggclr_ptr;
  rect_src_ptr: rect_ptr = nil; dx: int = 0; dy: int = 0; cover: int8u = cover_full);
var
  rsrc, rdst, rc: rect;
...
{ BLEND_FROM_LUT }
procedure renderer_base.blend_from_lut(src: pixel_formats_ptr; color_lut: aggclr_ptr;
  rect_src_ptr: rect_ptr = nil; dx: int = 0; dy: int = 0; cover: int8u = cover_full);
var
  rsrc, rdst, rc: rect;
