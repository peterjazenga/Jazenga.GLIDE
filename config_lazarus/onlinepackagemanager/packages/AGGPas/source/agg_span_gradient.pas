...
type
  ...
  span_gradient = object(span_generator)
    ...
    constructor Construct(alloc: span_allocator_ptr; inter: span_interpolator_ptr;
      gradient_function: gradient_ptr; color_function: array_base_ptr; d1, d2: double); overload;
  ...
  gradient_radial_focus = object(gradient)
    m_radius, m_focus_x, m_focus_y: int;

    m_radius2, m_trivial: double;
...
{ CONSTRUCT }
constructor span_gradient.Construct(alloc: span_allocator_ptr;
  inter: span_interpolator_ptr; gradient_function: gradient_ptr;
  color_function: array_base_ptr; d1, d2: double);
begin
  inherited Construct(alloc);
...
{ _INTERPOLATOR }
function span_gradient._interpolator: span_interpolator_ptr;
begin
...
{ _GRADIENT_FUNCTION }
function span_gradient._gradient_function: gradient_ptr;
begin
...
{ _COLOR_FUNCTION }
function span_gradient._color_function: array_base_ptr;
begin
...
{ _D1 }
function span_gradient._d1: double;
begin
...
{ _D2 }
function span_gradient._d2: double;
begin
...
{ INTERPOLATOR_ }
procedure span_gradient.interpolator_(i: span_interpolator_ptr);
begin
  m_interpolator := i;
...
{ GRADIENT_FUNCTION_ }
procedure span_gradient.gradient_function_(gf: gradient_ptr);
begin
...
{ COLOR_FUNCTION_ }
procedure span_gradient.color_function_(cf: array_base_ptr);
begin
  m_color_function := cf;
...
{ D1_ }
procedure span_gradient.d1_(v: double);
begin
...
{ D2_ }
procedure span_gradient.d2_(v: double);
begin
...
{ GENERATE }
function span_gradient.generate(x, y: int; len: unsigned): aggclr_ptr;
var
  span: aggclr_ptr;
...
{ CONSTRUCT }
constructor gradient_linear_color.Construct(c1, c2: aggclr_ptr; size_: unsigned = 256);
begin
  m_c1 := c1^;
  m_c2 := c2^;
...
{ SIZE }
function gradient_linear_color.size: unsigned;
begin
  Result := m_size;
...
{ ARRAY_OPERATOR }
function gradient_linear_color.array_operator(i: unsigned): pointer;
begin
...
{ COLORS_ }
procedure gradient_linear_color.colors_(c1, c2: aggclr_ptr; size_: unsigned = 256);
begin
  m_c1 := c1^;
  m_c2 := c2^;
...
{ CALCULATE }
function gradient_circle.calculate(x, y, d: int): int;
begin
...
{ CALCULATE }
function gradient_radial.calculate(x, y, d: int): int;
begin
  Result := fast_sqrt(x * x + y * y);
...
{ CALCULATE }
function gradient_radial_d.calculate(x, y, d: int): int;
begin
...
{ INIT }
procedure gradient_radial_focus.init(r, fx, fy: double);
begin
  m_radius := trunc(r * gradient_subpixel_size);
  m_focus_x := trunc(fx * gradient_subpixel_size);
  m_focus_y := trunc(fy * gradient_subpixel_size);
...
{ RADIUS }
function gradient_radial_focus.radius: double;
begin
...
{ FOCUS_X }
function gradient_radial_focus.focus_x: double;
begin
...
{ FOCUS_Y }
function gradient_radial_focus.focus_y: double;
begin
...
{ CALCULATE }
function gradient_radial_focus.calculate(x, y, d: int): int;
var
  solution_x, solution_y, slope, yint, a, b, c, det, int_to_focus, cur_to_focus: double;
...
begin
  ...
  cur_to_focus :=
    (x - m_focus_x) * (x - m_focus_x) + (y - m_focus_y) * (y - m_focus_y);
...
{ CALCULATE }
function gradient_x.calculate(x, y, d: int): int;
begin
  Result := x;
...
{ CALCULATE }
function gradient_y.calculate(x, y, d: int): int;
begin
  Result := y;
...
{ CALCULATE }
function gradient_diamond.calculate(x, y, d: int): int;
var
  ax, ay: int;
...
{ CALCULATE }
function gradient_xy.calculate(x, y, d: int): int;
begin
  if d = 0 then
    Result := 0
  else
...
{ CALCULATE }
function gradient_sqrt_xy.calculate(x, y, d: int): int;
begin
...
{ CALCULATE }
function gradient_conic.calculate(x, y, d: int): int;
begin
...
{ CONSTRUCT }
constructor gradient_repeat_adaptor.Construct(gradient: gradient_ptr);
begin
  m_gradient := gradient;
...
{ CALCULATE }
function gradient_repeat_adaptor.calculate(x, y, d: int): int;
begin
  if d = 0 then
    Result := 0
  else
...
{ CONSTRUCT }
constructor gradient_reflect_adaptor.Construct(gradient: gradient_ptr);
begin
  m_gradient := gradient;
...
{ CALCULATE }
function gradient_reflect_adaptor.calculate(x, y, d: int): int;
var
  d2: int;
