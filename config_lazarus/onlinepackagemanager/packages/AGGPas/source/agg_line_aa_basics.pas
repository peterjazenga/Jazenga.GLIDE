...
{ ORTHOGONAL_QUADRANT }
function line_parameters.orthogonal_quadrant: unsigned;
begin
...
{ DIAGONAL_QUADRANT }
function line_parameters.diagonal_quadrant: unsigned;
begin
...
function line_parameters.same_orthogonal_quadrant(lp: line_parameters_ptr): boolean;
begin
...
function line_parameters.same_diagonal_quadrant(lp: line_parameters_ptr): boolean;
begin
...
begin
  ...
  if intdbl(l2^.x2 - l2^.x1) * intdbl(l2^.y1 - l1^.y1) < intdbl(l2^.y2 - l2^.y1) * intdbl(l2^.x1 - l1^.x1) + 100.0 then
  begin
    tx := tx - ((tx - l2^.x1) * 2.0);
    ty := ty - ((ty - l2^.y1) * 2.0);
...
begin
  d := trunc((intdbl(x^ - lp^.x2) * intdbl(lp^.y2 - lp^.y1) - intdbl(y^ - lp^.y2) * intdbl(lp^.x2 - lp^.x1)) / lp^.len);
