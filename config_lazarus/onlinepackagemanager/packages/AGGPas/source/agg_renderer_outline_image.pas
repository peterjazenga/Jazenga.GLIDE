...
function pixel_source._width_d: double;
begin
 result:=_width;
...
{ _HEIGHT_D }
function pixel_source._height_d: double;
begin
 result:=_height;
...
{ CONSTRUCT }
constructor line_image_scale.Construct(src : pixel_source_ptr; height_ : double );
begin
 m_source:=src;
 m_height:=height_;
...
{ _WIDTH_D }
function line_image_scale._width_d: double;
begin
...
{ _HEIGHT_D }
function line_image_scale._height_d: double;
begin
 result:=m_height;
...
{ PIXEL }
function line_image_scale.pixel(x ,y : int ) : rgba8;
var
 src_y : double;
...
{ CREATE }
procedure line_image_pattern.create(src : pixel_source_ptr );
var
 x ,y ,h : unsigned;
...
{ PATTERN_WIDTH }
function line_image_pattern.pattern_width : int;
begin
 result:=m_width_hr;
...
{ LINE_WIDTH }
function line_image_pattern.line_width : int;
begin
...
{ PIXEL }
procedure line_image_pattern.pixel(p : aggclr_ptr; x ,y : int );
begin
...
{ CREATE }
procedure line_image_pattern_pow2.create(src : pixel_source_ptr );
begin
...
{ PIXEL }
procedure line_image_pattern_pow2.pixel(p : aggclr_ptr; x ,y : int );
begin
 m_filter^.pixel_high_res(
  m_buf.rows ,
  p ,
...
{ INC_X }
procedure distance_interpolator4.inc_x(dy_ : int );
begin
 inc(m_dist ,m_dy );
 inc(m_dist_start ,m_dy_start );
 inc(m_dist_pict ,m_dy_pict );
...
{ DEC_X }
procedure distance_interpolator4.dec_x(dy_ : int );
begin
 dec(m_dist ,m_dy );
 dec(m_dist_start ,m_dy_start );
 dec(m_dist_pict ,m_dy_pict );
...
{ INC_Y }
procedure distance_interpolator4.inc_y(dx_ : int );
begin
 dec(m_dist ,m_dx );
 dec(m_dist_start ,m_dx_start );
 dec(m_dist_pict ,m_dx_pict );
...
{ DEC_Y }
procedure distance_interpolator4.dec_y(dx_ : int );
begin
 inc(m_dist ,m_dx );
 inc(m_dist_start ,m_dx_start );
 inc(m_dist_pict ,m_dx_pict );
...
{ DIST }
function distance_interpolator4.dist : int;
begin
 result:=m_dist;
...
{ DIST_START }
function distance_interpolator4.dist_start : int;
begin
 result:=m_dist_start;
...
{ DIST_PICT }
function distance_interpolator4.dist_pict : int;
begin
 result:=m_dist_pict;
...
{ DIST_END }
function distance_interpolator4.dist_end : int;
begin
 result:=m_dist_end;
...
{ DX }
function distance_interpolator4.dx: int;
begin
 result:=m_dx;
...
{ DY }
function distance_interpolator4.dy: int;
begin
 result:=m_dy;
...
{ DX_START }
function distance_interpolator4.dx_start: int;
begin
 result:=m_dx_start;
...
{ DY_START }
function distance_interpolator4.dy_start: int;
begin
 result:=m_dy_start;
...
{ DX_PICT }
function distance_interpolator4.dx_pict: int;
begin
 result:=m_dx_pict;
...
{ DY_PICT }
function distance_interpolator4.dy_pict: int;
begin
 result:=m_dy_pict;
...
{ DX_END }
function distance_interpolator4.dx_end: int;
begin
 result:=m_dx_end;
...
{ DY_END }
function distance_interpolator4.dy_end: int;
begin
 result:=m_dy_end;
...
{ LEN }
function distance_interpolator4.len: int;
begin
 result:=m_len;
...
{ STEP_HOR }
function line_interpolator_image.step_hor: boolean;
var
...
{ STEP_VER }
function line_interpolator_image.step_ver: boolean;
var
...
{ PATTERN_END }
function line_interpolator_image.pattern_end: int;
begin
...
{ VERTICAL }
function line_interpolator_image.vertical : boolean;
begin
...
{ WIDTH }
function line_interpolator_image.width: int;
begin
 result:=m_width;
...
{ COUNT }
function line_interpolator_image.count: int;
begin
 result:=m_count;
...
{ CONSTRUCT }
constructor renderer_outline_image.Construct(ren : renderer_base_ptr; patt : line_image_pattern_ptr );
begin
 m_ren    :=ren;
 m_pattern:=patt;
 m_start  :=0;
...
{ PATTERN_ }
procedure renderer_outline_image.pattern_(p : line_image_pattern_ptr );
begin
 m_pattern:=p;
...
{ _PATTERN }
function renderer_outline_image._pattern : line_image_pattern_ptr;
begin
 result:=m_pattern;
...
{ SCALE_X_ }
procedure renderer_outline_image.scale_x_(s : double );
begin
 m_scale_x:=s;
...
{ _SCALE_X }
function renderer_outline_image._scale_x : double;
begin
 result:=m_scale_x;
...
{ START_X_ }
procedure renderer_outline_image.start_x_(s : double );
begin
...
{ _START_X }
function renderer_outline_image._start_x : double;
begin
...
{ SUBPIXEL_WIDTH }
function renderer_outline_image.subpixel_width : int;
begin
...
{ _PATTERN_WIDTH }
function renderer_outline_image._pattern_width : int;
begin
...
{ PIXEL }
procedure renderer_outline_image.pixel(p : aggclr_ptr; x ,y : int );
begin
...
{ BLEND_COLOR_HSPAN }
procedure renderer_outline_image.blend_color_hspan(x ,y : int; len : unsigned; colors : aggclr_ptr );
begin
...
{ BLEND_COLOR_VSPAN }
procedure renderer_outline_image.blend_color_vspan(x ,y : int; len : unsigned; colors : aggclr_ptr );
begin
...
{ ACCURATE_JOIN_ONLY }
function renderer_outline_image.accurate_join_only : boolean;
begin
 result:=true;
...
{ SEMIDOT }
procedure renderer_outline_image.semidot(cmp : cmp_function; xc1 ,yc1 ,xc2 ,yc2 : int );
begin
end;

{ LINE0 }
procedure renderer_outline_image.line0(lp : line_parameters_ptr );
begin
end;

{ LINE1 }
procedure renderer_outline_image.line1(lp : line_parameters_ptr; sx ,sy : int );
begin
end;

{ LINE2 }
procedure renderer_outline_image.line2(lp : line_parameters_ptr; ex ,ey : int );
begin
end;

{ LINE3 }
procedure renderer_outline_image.line3(lp : line_parameters_ptr; sx ,sy ,ex ,ey : int );
var
