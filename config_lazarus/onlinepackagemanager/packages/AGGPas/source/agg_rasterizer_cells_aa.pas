...
{ cell_type_ptr_ptr_ptr = ^cell_type_ptr_ptr;
 ...
 cell_type = cell_style_aa; }
  ...
  private
    m_num_blocks, m_max_blocks, m_curr_block, m_num_cells: unsigned;

    m_cells: cell_style_aa_ptr_ptr;

    m_curr_cell_ptr: cell_style_aa_ptr;
    m_sorted_cells, m_sorted_y: pod_vector;
    m_curr_cell, m_style_cell: cell_style_aa;
...
var
  dx, cx, cy, dy, ex1, ex2, ey1, ey2, fy1, fy2, ex, two_fx, area, x_from, x_to, p, rem, mod_, lift, delta, First, incr: int;
...
var
  ...
  limit, base: cell_style_aa_ptr_ptr;
...
begin
  Result := cell_style_aa_ptr_ptr(ptrcomp(m_sorted_cells.Data) + sorted_y_ptr(m_sorted_y.array_operator(y - m_min_y))^.start *
    sizeof(cell_style_aa_ptr));
