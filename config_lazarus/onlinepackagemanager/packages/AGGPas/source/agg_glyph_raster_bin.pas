...
type
  ...
  glyph_raster_bin = object
    ...
    m_bits: int8u_ptr;
    m_glyph_width, m_glyph_byte_width: unsigned;
...
constructor glyph_raster_bin.Construct(font: int8u_ptr);
var
  t: int;
...
{ _FONT }
function glyph_raster_bin._font: int8u_ptr;
begin
  Result := m_font;
...
{ FONT_ }
procedure glyph_raster_bin.font_(f: int8u_ptr);
begin
  m_font := f;
...
{ HEIGHT }
function glyph_raster_bin.Height: double;
begin
...
{ BASE_LINE }
function glyph_raster_bin.base_line: double;
begin
...
{ WIDTH }
function glyph_raster_bin.Width(str: PChar): double;
var
...
begin
  ...
  begin
    ...
    bits :=
      int8u_ptr(ptrcomp(m_font) + 4 + num_chars * 2 + Value(int8u_ptr(ptrcomp(m_font) + 4 + (glyph - start_char) * 2)));
...
{ PREPARE }
procedure glyph_raster_bin.prepare(r: glyph_rect_ptr; x, y: double; glyph: unsigned; flip: boolean);
var
...
begin
  ...
  begin
    r^.y1 :=
      trunc(y) - int8u_ptr(ptrcomp(m_font) + 0 * sizeof(int8u))^ + int8u_ptr(ptrcomp(m_font) + 1 * sizeof(int8u))^;
...
{ SPAN }
function glyph_raster_bin.span(i: unsigned): int8u_ptr;
var
  bits: int8u_ptr;
...
{ VALUE }
function glyph_raster_bin.Value(p: int8u_ptr): int16u;
var
  v: int16u;
