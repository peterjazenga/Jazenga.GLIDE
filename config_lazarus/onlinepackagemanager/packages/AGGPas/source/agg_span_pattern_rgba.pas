...
type
  ...
  span_pattern_rgba = object(span_pattern_base)
    ...
    constructor Construct(alloc: span_allocator_ptr; src: rendering_buffer_ptr;
      offset_x, offset_y: unsigned; WX, WY: wrap_mode_ptr; order: order_type); overload;
...
{ CONSTRUCT }
constructor span_pattern_rgba.Construct(alloc: span_allocator_ptr; src: rendering_buffer_ptr;
...
{ SOURCE_IMAGE_ }
procedure span_pattern_rgba.source_image_(src: rendering_buffer_ptr);
begin
...
{ GENERATE }
function span_pattern_rgba.generate(x, y: int; len: unsigned): aggclr_ptr;
var
  span: aggclr_ptr;
