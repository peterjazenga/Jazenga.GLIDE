...
type
  ...
  bezier_arc = object(vertex_source)
    m_vertex, m_num_vertices: unsigned;
...
procedure bezier_arc.init(x, y, rx, ry, start_angle, sweep_angle: double);
var
  i: int;
  f: double;

  total_sweep, local_sweep, prev_sweep: double;
...
begin
  ...
  repeat
    ...
    arc_to_bezier(
      x, y, rx, ry,
      start_angle, local_sweep, @m_vertices[m_num_vertices - 2]);
...
procedure bezier_arc.rewind(path_id: unsigned);
begin
  m_vertex := 0;

end;

function bezier_arc.vertex(x, y: double_ptr): unsigned;
begin
...
function bezier_arc.num_vertices: unsigned;
begin
  Result := m_num_vertices;

end;

function bezier_arc.vertices: double_26_ptr;
begin
  Result := @m_vertices;
...
procedure bezier_arc_svg.init(x0, y0, rx, ry, angle: double; large_arc_flag, sweep_flag: boolean; x2, y2: double);
var
  i: unsigned;

  v, p, n, sq, x1, y1, cx, cy, ux, uy, vx, vy, dx2, dy2, prx, pry, px1, py1, cx1, cy1, sx2, sy2, sign, coef,
  radii_check, start_angle, sweep_angle, cos_a, sin_a: double;
...
begin
  ...
  if (not sweep_flag) and (sweep_angle > 0) then
    sweep_angle := sweep_angle - pi * 2.0
  else
  if sweep_flag and (sweep_angle < 0) then
    sweep_angle := sweep_angle + pi * 2.0;
...
function bezier_arc_svg.radii_ok: boolean;
begin
  Result := m_radii_ok;
