...
type
  ...
  status = (initial, next_char, start_glyph, glyph);

  //---------------------------------------------------------------gsv_text
  gsv_text_ptr = ^gsv_text;

  gsv_text = object(vertex_source)
    m_x, m_y, m_start_x, m_width, m_height, m_space, m_line_space: double;
    m_chr: array[0..1] of byte;
    m_text, m_text_buf: pointer;
    m_buf_size: unsigned;
    m_cur_chr: int8u_ptr;
    m_font, m_loaded_font: pointer;
    m_loadfont_sz: unsigned;
    m_status: status;
    m_big_endian, m_flip: boolean;
    m_indices: int8u_ptr;
    m_glyphs, m_bglyph, m_eglyph: int8_ptr;
    m_w, m_h: double;
...
{ FONT_ }
procedure gsv_text.font_(font: pointer);
begin
  m_font := font;
...
{ FLIP_ }
procedure gsv_text.flip_(flip_y: boolean);
begin
  m_flip := flip_y;
...
{ LOAD_FONT_ }
procedure gsv_text.load_font_(_file_: shortstring);
var
  fd: file;
  err: integer;
...
{ SIZE_ }
procedure gsv_text.size_(Height: double; Width: double = 0.0);
begin
  m_height := Height;
  m_width := Width;
...
{ SPACE_ }
procedure gsv_text.space_(space: double);
begin
  m_space := space;
...
{ LINE_SPACE_ }
procedure gsv_text.line_space_(line_space: double);
begin
...
{ START_POINT_ }
procedure gsv_text.start_point_(x, y: double);
begin
  m_x := x;
  m_y := y;
...
{ TEXT_ }
procedure gsv_text.text_(Text: PChar);
var
  new_size: unsigned;
...
{ REWIND }
procedure gsv_text.rewind(path_id: unsigned);
var
  base_height: double;
...
{ VERTEX }
function gsv_text.vertex(x, y: double_ptr): unsigned;
var
  idx: unsigned;
...
{ _VALUE }
function gsv_text._value(p: int8u_ptr): int16u;
var
  v: int16u;
...
{ CONSTRUCT }
constructor gsv_text_outline.Construct(Text: gsv_text_ptr; trans: trans_affine_ptr);
begin
...
{ WIDTH_ }
procedure gsv_text_outline.width_(w: double);
begin
  m_polyline.width_(w);
...
{ TRANSFORMER_ }
procedure gsv_text_outline.transformer_(trans: trans_affine_ptr);
begin
...
{ REWIND }
procedure gsv_text_outline.rewind(path_id: unsigned);
begin
...
{ VERTEX }
function gsv_text_outline.vertex(x, y: double_ptr): unsigned;
begin
