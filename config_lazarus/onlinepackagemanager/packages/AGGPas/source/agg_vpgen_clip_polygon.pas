...
type
  ...
  vpgen_clip_polygon = object(vertex_source)
    ...
    m_num_vertices, m_vertex, m_cmd: unsigned;
...
{ CLIP_BOX_ }
procedure vpgen_clip_polygon.clip_box_(x1, y1, x2, y2: double);
begin
  m_clip_box.x1 := x1;
  m_clip_box.y1 := y1;
  m_clip_box.x2 := x2;
...
{ _X1 }
function vpgen_clip_polygon._x1: double;
begin
...
{ _Y1 }
function vpgen_clip_polygon._y1: double;
begin
...
{ _X2 }
function vpgen_clip_polygon._x2: double;
begin
...
{ _Y2 }
function vpgen_clip_polygon._y2: double;
begin
...
{ _AUTO_CLOSE }
function vpgen_clip_polygon._auto_close: boolean;
begin
  Result := True;
...
{ _AUTO_UNCLOSE }
function vpgen_clip_polygon._auto_unclose: boolean;
begin
  Result := False;
...
{ MOVE_TO }
procedure vpgen_clip_polygon.move_to(x, y: double);
begin
  m_vertex := 0;
  m_num_vertices := 0;
...
{ LINE_TO }
procedure vpgen_clip_polygon.line_to(x, y: double);
var
  flags: unsigned;
...
begin
  ...
  if m_clip_flags = flags then
    ...
    m_num_vertices :=
      clip_liang_barsky_d(m_x1, m_y1, x, y, @m_clip_box, @m_x, @m_y);
...
{ VERTEX }
function vpgen_clip_polygon.vertex(x, y: double_ptr): unsigned;
var
  cmd: unsigned;
...
function vpgen_clip_polygon.clipping_flags(x, y: double): unsigned;
begin
