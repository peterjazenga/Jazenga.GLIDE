...
{$ENDIF }
  ...
  private
    ...
    m_fillGradientD1, m_lineGradientD1, m_fillGradientD2, m_lineGradientD2, m_textAngle: double;
    m_textAlignX, m_textAlignY: TAggTextAlignment;
    m_textHints: boolean;
    m_fontHeight, m_fontAscent, m_fontDescent: double;
    ...
    m_fillGradientInterpolator, m_lineGradientInterpolator: span_interpolator_linear;
  ...
  public
    ...
    procedure Viewport(worldX1, worldY1, worldX2, worldY2, screenX1, screenY1, screenX2, screenY2: double;
    ...
    procedure Font(fileName: ansistring; Height: double; bold: boolean = False;
      italic: boolean = False; cache: TAggFontCacheType = AGG_VectorFontCache; angle: double = 0.0);
    ...
    procedure TransformImage(bitmap: TBitmap; imgX1, imgY1, imgX2, imgY2: integer;
      dstX1, dstY1, dstX2, dstY2: double); overload;

    procedure TransformImage(bitmap: TBitmap; dstX1, dstY1, dstX2, dstY2: double); overload;
    ...
    procedure TransformImagePath(bitmap: TBitmap; imgX1, imgY1, imgX2, imgY2: integer;
      dstX1, dstY1, dstX2, dstY2: double); overload;

    procedure TransformImagePath(bitmap: TBitmap; dstX1, dstY1, dstX2, dstY2: double); overload;
...
procedure Agg2DRenderer_render(gr: TAgg2D; renBase: renderer_base_ptr;
  renSolid: renderer_scanline_aa_solid_ptr; fillColor_: boolean); overload;
var
  span: span_gradient;
...
begin
  if (fillColor_ and (gr.m_fillGradientFlag = AGG_Linear)) or (not fillColor_ and (gr.m_lineGradientFlag = AGG_Linear)) then
    if fillColor_ then
    begin
      span.Construct(@gr.m_allocator, @gr.m_fillGradientInterpolator, @gr.m_linearGradientFunction, @gr.m_fillGradient,
        gr.m_fillGradientD1,
        gr.m_fillGradientD2);
    ...
    begin
      span.Construct(@gr.m_allocator, @gr.m_lineGradientInterpolator, @gr.m_linearGradientFunction, @gr.m_lineGradient,
        gr.m_lineGradientD1,
        gr.m_lineGradientD2);
  ...
  if (fillColor_ and (gr.m_fillGradientFlag = AGG_Radial)) or (not fillColor_ and (gr.m_lineGradientFlag = AGG_Radial)) then
    if fillColor_ then
    begin
      span.Construct(@gr.m_allocator, @gr.m_fillGradientInterpolator, @gr.m_radialGradientFunction, @gr.m_fillGradient,
        gr.m_fillGradientD1,
        gr.m_fillGradientD2);
    ...
    begin
      span.Construct(@gr.m_allocator, @gr.m_lineGradientInterpolator, @gr.m_radialGradientFunction, @gr.m_lineGradient,
        gr.m_lineGradientD1,
        gr.m_lineGradientD2);
...
{ AGG2DRENDERER_RENDER }
procedure Agg2DRenderer_render(gr: TAgg2D; renBase: renderer_base_ptr;
  renSolid: renderer_scanline_aa_solid_ptr; ras: gray8_adaptor_type_ptr; sl: gray8_scanline_type_ptr); overload;
var
  span: span_gradient;
  ren: renderer_scanline_aa;
  clr: aggclr;
...
procedure Agg2DRenderer_renderImage(gr: TAgg2D; img: PAggImage; renBase: renderer_base_ptr;
...
begin
  ...
  begin
    ...
    begin
      clr.ConstrInt(0, 0, 0, 0);
      sa.Construct(@gr.m_allocator, @img^.renBuf, @clr,
        interpolator, @gr.m_imageFilterLut,
        rgba_order);
    ...
    begin
      clr.ConstrInt(0, 0, 0, 0);
      sb.Construct(@gr.m_allocator, @img^.renBuf, @clr,
        interpolator,
        rgba_order);
    ...
    begin
      clr.ConstrInt(0, 0, 0, 0);
      s2.Construct(@gr.m_allocator, @img^.renBuf, @clr,
        interpolator, @gr.m_imageFilterLut,
        rgba_order);
    ...
    begin
      clr.ConstrInt(0, 0, 0, 0);
      si.Construct(@gr.m_allocator, @img^.renBuf, @clr,
        interpolator, @gr.m_imageFilterLut,
        rgba_order);
...
begin
  if (m_mode <> AGG_BlendDst) and (m_pixel <> nil) then
  begin{!}
    l2 := len;
    s2 := PAggColor(span);
  ...
  if Assigned(bitmap) and not bitmap.Empty then
    case bitmap.PixelFormat of
      ...
      begin
        ...
        if stride < 0 then
          buffer := fLinkIntfImage.GetDataLineStart(bitmap.Height - 1)
        else
...
{ VIEWPORT }
procedure TAgg2D.Viewport(worldX1, worldY1, worldX2, worldY2, screenX1, screenY1, screenX2, screenY2: double;
  opt: TAggViewportOption = AGG_XMidYMid);
var
  vp: trans_viewport;
  mx: trans_affine;
...
{ FONT }
procedure TAgg2D.Font(fileName: ansistring; Height: double; bold: boolean = False;
  italic: boolean = False; cache: TAggFontCacheType = AGG_VectorFontCache; angle: double = 0.0);
var
  b: int;
...
begin
  ...
  case flag of
    ...
    AGG_StrokeOnly:
      if (m_lineColor.a <> 0) and (m_lineWidth > 0.0) then
      begin
...
{ TRANSFORMIMAGE }
procedure TAgg2D.TransformImage(bitmap: TBitmap; imgX1, imgY1, imgX2, imgY2: integer;
...
{ TRANSFORMIMAGE }
procedure TAgg2D.TransformImage(bitmap: TBitmap; dstX1, dstY1, dstX2, dstY2: double);
var
  parall: array[0..5] of double;
  image: TAggImage;
...
{ TRANSFORMIMAGE }
procedure TAgg2D.TransformImage(bitmap: TBitmap; imgX1, imgY1, imgX2, imgY2: integer; parallelo: PDouble);
var
  image: TAggImage;
...
begin
  ...
  begin
    ...
    LineTo(
      PDouble(ptrcomp(parallelo) + 0 * sizeof(double))^ + PDouble(ptrcomp(parallelo) + 4 * sizeof(double))^
      - PDouble(ptrcomp(parallelo) + 2 * sizeof(double))^,
      PDouble(ptrcomp(parallelo) + 1 * sizeof(double))^ + PDouble(ptrcomp(parallelo) + 5 * sizeof(double))^
      - PDouble(ptrcomp(parallelo) + 3 * sizeof(double))^);
...
{ TRANSFORMIMAGEPATH }
procedure TAgg2D.TransformImagePath(bitmap: TBitmap; imgX1, imgY1, imgX2, imgY2: integer;
  dstX1, dstY1, dstX2, dstY2: double);
var
  parall: array[0..5] of double;
  image: TAggImage;
...
{ TRANSFORMIMAGEPATH }
procedure TAgg2D.TransformImagePath(bitmap: TBitmap; dstX1, dstY1, dstX2, dstY2: double);
var
  parall: array[0..5] of double;
  image: TAggImage;
...
{ TRANSFORMIMAGEPATH }
procedure TAgg2D.TransformImagePath(bitmap: TBitmap; imgX1, imgY1, imgX2, imgY2: integer; parallelo: PDouble);
var
  image: TAggImage;
...
{ COPYIMAGE }
procedure TAgg2D.CopyImage(bitmap: TBitmap; imgX1, imgY1, imgX2, imgY2: integer; dstX, dstY: double);
var
  r: agg_basics.rect;
  image: TAggImage;
...
{ RENDERIMAGE }
procedure TAgg2D.renderImage(img: PAggImage; x1, y1, x2, y2: integer; parl: PDouble);
var
  mtx: trans_affine;
...
begin
  ...
  if Assigned(bitmap) and not bitmap.Empty and (bitmap.PixelFormat = pf32bit) then
  begin
    bitmap.BeginUpdate;
    fIntfImage := bitmap.CreateIntfImage;

    for fcy := 0 to bitmap.Height - 1 do
    begin
      transp := pointer(ptrcomp(fIntfImage.GetDataLineStart(fcy)) + 3);
    ...
    bitmap.LoadFromIntfImage(fIntfImage);
    bitmap.EndUpdate;
