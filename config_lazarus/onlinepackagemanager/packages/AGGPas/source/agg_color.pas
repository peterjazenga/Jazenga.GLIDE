...
//======================== rgba8 ===========================
constructor rgba8.Construct(r_, g_, b_: unsigned; a_: unsigned = base_mask);
begin
  b := int8u(r_);
  g := int8u(g_);
  r := int8u(b_);
...
constructor aggclr.ConstrDbl(r_, g_, b_: double; a_: double = 1.0);
begin
...
constructor aggclr.ConstrPre(r_, g_, b_: double; a_: double = 1.0);
begin
  r_ := r_ * a_;
  g_ := g_ * a_;
  b_ := b_ * a_;
...
constructor aggclr.ConstrMix(r_, g_, b_: unsigned; a_: double = 1.0);
begin
  v := (r_ * 77 + g_ * 150 + b_ * 29) shr 8;
  r := r_;
  g := g_;
...
function aggclr.gradient(c: aggclr_ptr; k: double): aggclr;
var
  ik: unsigned;
...
function aggclr.gradient8(c: aggclr_ptr; k: double): rgba8;
var
  ik: unsigned;
begin
  ik := trunc(k * base_size);
...
function aggclr.opacity_(a_: double): aggclr_ptr;
begin
  if a_ < 0.0 then
    a_ := 0.0;
...
function aggclr._opacity: double;
begin
  Result := a / base_mask;
...
procedure aggclr.apply_gamma_dir(gamma_: gamma_ptr);
begin
