...
type
  ...
  outline_aa = object
    m_num_blocks, m_max_blocks, m_cur_block, m_num_cells: unsigned;

    m_cur_x, m_cur_y, m_min_x, m_min_y, m_max_x, m_max_y: int;
  ...
  rasterizer_scanline_aa = object(rasterizer_scanline)
    ...
    m_clipped_start_x, m_clipped_start_y, m_start_x, m_start_y, m_prev_x, m_prev_y: int;
...
{ MOVE_TO }
procedure outline_aa.move_to(x, y: int);
begin
  if m_sorted then
    reset;
...
{ LINE_TO }
procedure outline_aa.line_to(x, y: int);
begin
...
{ SET_CUR_CELL }
procedure outline_aa.set_cur_cell(x, y: int);
begin
  if (m_cur_cell.x <> x) or (m_cur_cell.y <> y) then
  begin
    add_cur_cell;
...
var
  ...
  limit, base, i, j, pivot: cell_aa_ptr_ptr;
  top: ^cell_aa_ptr_ptr;
...
{ TOTAL_CELLS }
function outline_aa.total_cells: unsigned;
begin
  Result := m_num_cells;
...
{ SCANLINE_NUM_CELLS }
function outline_aa.scanline_num_cells(y: unsigned): unsigned;
begin
...
{ SCANLINE_CELLS }
function outline_aa.scanline_cells(y: unsigned): cell_aa_ptr_ptr;
begin
  Result :=
    cell_aa_ptr_ptr(ptrcomp(m_sorted_cells.m_array) + sorted_y_ptr(ptrcomp(m_sorted_y.m_array) + unsigned(y - m_min_y) *
    m_sorted_y.m_entry_sz)^.start * m_sorted_cells.m_entry_sz);

end;

{ SORTED }
function outline_aa.sorted : boolean;
begin
  Result := m_sorted;
...
{ RENDER_LINE }
procedure outline_aa.render_line (x1 ,y1 ,x2 ,y2 : int );
var
  p, cx, cy, dx, dy, ex, ey1, ey2, fy1, fy2, rem, m_d, x_from, x_to, lift, delta, First, incr, two_fx, area: int;
...
begin
  ...
  if (dx >= dx_limit) or (dx <= -dx_limit) then
  begin
    cx := (x1 + x2) shr 1;
    cy := (y1 + y2) shr 1;
...
{ RENDER_HLINE }
procedure outline_aa.render_hline(ey ,x1 ,y1 ,x2 ,y2 : int );
var
  p, dx, ex1, ex2, fx1, fx2, delta, First, incr, lift, m_d, rem: int;
...
{ _MIN_X }
function outline_aa._min_x: int;
begin
  Result := m_min_x;
...
{ _MIN_Y }
function outline_aa._min_y: int;
begin
  Result := m_min_y;
...
{ _MAX_X }
function outline_aa._max_x: int;
begin
  Result := m_max_x;
...
{ _MAX_Y }
function outline_aa._max_y: int;
begin
  Result := m_max_y;
...
{ CONSTRUCT }
constructor scanline_hit_test.Construct(x: int);
begin
  m_x := x;
  m_hit := False;
...
{ FINALIZE }
procedure scanline_hit_test.finalize(y_: int);
begin
end;

{ ADD_CELL }
procedure scanline_hit_test.add_cell(x: int; cover: unsigned);
begin
  if m_x = x then
    m_hit := True;
...
{ ADD_SPAN }
procedure scanline_hit_test.add_span(x: int; len, cover: unsigned);
begin
  if (m_x >= x) and (m_x < x + len) then
    m_hit := True;

end;

{ NUM_SPANS }
function scanline_hit_test.num_spans: unsigned;
begin
  Result := 1;
...
{ HIT }
function scanline_hit_test.hit: boolean;
begin
  Result := m_hit;
...
begin
  for i := 0 to aa_num - 1 do
    m_gamma[i] :=
      trunc(gamma_function^.func_operator_gamma(i / aa_mask) * aa_mask + 0.5);
...
begin
  if (m_status = status_line_to) and m_auto_close then
...
begin
  repeat
    ...
    begin
      ...
      if (num_cells <> 0) and (cur_cell^.x > x) then
      begin
...
begin
  ...
  if (m_outline.total_cells = 0) or (y < m_outline._min_y) or (y > m_outline._max_y) then
  begin
    Result := False;
