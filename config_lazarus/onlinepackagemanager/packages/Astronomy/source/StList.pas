...
type
  ...
  public
    ...
    function Iterate(Action: TIterateFunc; Up: boolean; OtherData: Pointer): TStListNode;
    {-Call Action for all the nodes, returning the last node visited}

    property Head: TStListNode
    {-Return the head node} read FHead;
    property Tail: TStListNode
    {-Return the tail node} read FTail;
    property Items[Index: longint]: TStListNode
    {-Return the Index'th node, 0-based} read Nth; default;
  end;
...
function FindNode(Container: TStContainer; Node: TStNode; OtherData: Pointer): boolean; far;
begin
  Result := (Node.Data <> OtherData);
end;

function AssignData(Container: TStContainer; Data, OtherData: Pointer): boolean; far;
var
...
procedure TStList.ForEachPointer(Action: TIteratePointerFunc; OtherData: pointer);
var
  N: TStListNode;
  P: TStListNode;
begin
...
{$ENDIF}
    ...
    begin
      ...
      while Assigned(P) do
      begin
        if DoCompare(N.Data, P.Data) < 0 then
        begin
          if not Assigned(P.FPrev) then
          begin
            {New head}
            FHead := N;
          end
...
function TStList.Iterate(Action: TIterateFunc; Up: boolean; OtherData: Pointer): TStListNode;
var
  N: TStListNode;
  P: TStListNode;
begin
...
{$ENDIF}
    if Assigned(L) then
    begin
      if Assigned(P) and (L.Count > 0) then
      begin
...
{$ENDIF}
    ...
    try
      ...
      begin
        ...
        if (not IsOrInheritsFrom(StreamedClass, Self.ClassType)) or (not IsOrInheritsFrom(TStList, StreamedClass)) then
        ...
        if (not IsOrInheritsFrom(StreamedNodeClass, conNodeClass)) or (not IsOrInheritsFrom(TStListNode, StreamedNodeClass)) then
...
{$ENDIF}
    ...
    begin
      ...
      if lsLastI >= 0 then
        ...
        if Index <= lsLastI then
        begin
          if lsLastI - Index < Index then
          begin
            MinI := Index - lsLastI;
            MinP := lsLastP;
          end;
        end
        else if Index - lsLastI < FCount - 1 - Index then
        begin
          MinI := Index - lsLastI;
          MinP := lsLastP;
        end
    ...
    repeat
      ...
      if L <> R then
        ...
        repeat
          ...
          repeat
            while DoCompare(PL.Data, PivotData) < 0 do
            begin
              PL := PL.FNext;
              Dec(Dist);
              Dec(DistR);
            end;
            while DoCompare(PivotData, PR.Data) < 0 do
            begin
              PR := PR.FPrev;
              Dec(Dist);
              Dec(DistL);
            ...
            begin
              ...
              if Assigned(PL.FNext) then
              begin
                PL := PL.FNext;
                Dec(Dist);
                Dec(DistR);
              end;
              if Assigned(PR.FPrev) then
              begin
                PR := PR.FPrev;
                Dec(Dist);
                Dec(DistL);
